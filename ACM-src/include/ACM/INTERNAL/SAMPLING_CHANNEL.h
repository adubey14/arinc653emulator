...
struct DESTINATION_URL {
  ...
  DESTINATION_URL(){};
  DESTINATION_URL(const std::string &name, const unsigned short &port_num)
      : node_name(name), port(port_num) {}
};
...
/*class SAMPLING_PORT_SOCKET_SOURCE
{
        private:
                ...
                char
DataBuffer[sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+SYSTEM_LIMIT_MAX_SAMPLING_MESSAGE_SIZE];
public:
...
private:
  ...
  unsigned int MAX_SOCKET_MESSAGE;
  std::vector<DESTINATION_URL> destination_url_vector;
  char DataBuffer[sizeof(SYSTEM_TIME_TYPE) + sizeof(MESSAGE_SIZE_TYPE) +
...
public:
  ...
  void ADD_SOURCE(std::string Name, MESSAGE_SIZE_TYPE MAX_MESSAGE_SIZE,
                  SYSTEM_TIME_TYPE REFRESH_PERIOD,
                  RETURN_CODE_TYPE *RETURN_CODE);
  void ADD_DESTINATION(std::string Name, MESSAGE_SIZE_TYPE MAX_MESSAGE_SIZE,
                       SYSTEM_TIME_TYPE REFRESH_PERIOD,
                       RETURN_CODE_TYPE *RETURN_CODE);
