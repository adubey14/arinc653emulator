...
void PROCESS_APERIODIC (void){
	...
	BUFFER_ID_TYPE GET_ID;
	//Tests that no error is returned when correctly getting a buffer's ID
	GET_BUFFER_ID(BUFFER_NAME, &GET_ID, &RETURN_CODE);
	if(RETURN_CODE == NO_ERROR){
		LOGGER(APP,"Get buffer ID test Succeeded!");  
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE); 
	}
	else{
		LOGGER(APP,"Get buffer ID test Failed."); 
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);  
	}

	//Tests that the correct code is returned when attempting to get the ID of a nonexistent blackboard
	BUFFER_NAME_TYPE NONEXISTENT_BUFFER_NAME = "nonexistent";
	GET_BUFFER_ID(NONEXISTENT_BUFFER_NAME, &GET_ID, &RETURN_CODE);
