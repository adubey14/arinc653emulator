...
namespace ACM_USER{
	...
	/*!
	The way we will use it is that user will define their own exceptions that will be derived from this.
	...
	*/
	...
	public:
		...
		ACM_BASE_EXCEPTION_TYPE(const ACM_BASE_EXCEPTION_TYPE& ex) :Message(ex.Message){}

		ACM_BASE_EXCEPTION_TYPE(const std::string& _Message):Message(_Message) {
		}
		virtual void setMessage(const std::string& _Message){
			Message=_Message;
		}
	...
	public:
		...
		ACM_TIMING_EXCEPTION_TYPE(std::string Message):ACM_BASE_EXCEPTION_TYPE(Message){
		}
