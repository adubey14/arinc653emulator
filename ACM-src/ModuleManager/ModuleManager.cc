...
namespace ACM_INTERNAL{
	...
	DISCOVERY_PROTOCOL(IOR),
		...
		cpunumber(0),
		DUMMY_PROCESS_ID(0),
		MAX_NUMBER_OF_FRAMES(-1),
		isMAX_NUMBER_OF_FRAMES_SET(false),
		MIN_WAIT_TIME(500),
		isMIN_WAIT_TIME_SET(false),
		NumberOfInitStages(1),	
		PARENT_NAME(""),
		PARENT_HOST(""),
		PARENT_PORT(0),
		CONFIGURATION_PORT_NUM(CONFIGURATION_PORT),
		isSystemModuleManager(false),
		isBroadcast(false),
		isMulticast(false),
		...
		Recv_Socket(0),
		Send_Socket(0),		
		AVE_NETWORK_LATENCY_FROM_PARENT(0),
...

	{
		...
		if(killfile.good() && killfile.is_open())
		{
			...
			killfile<<"ps -ef |grep ModuleManager | grep "<<pid_data<<" && echo \"kill -9 "<<pid_data<<" \" &&  kill -9 "<<pid_data<<std::endl;				
			killfile<<"mkdir -p /dev/mqueue"<<std::endl;
			killfile<<"(! mount |grep mqueue > /dev/null) && mount -t mqueue none /dev/mqueue"<<std::endl;
			killfile.flush();
		}
		...
		{
			LOGGER(FATAL,"PARTITION_VECTOR.size()>MAX_PARTITION_NUM %d",MAX_PARTITION_NUM);
			throw(-6);
		}
		std::sort(SCHEDULING_VECTOR.begin(),SCHEDULING_VECTOR.end(),MODULE_MANAGER::SCHEDULE_COMP);

		//check schedule
		for(unsigned int i =0; i<SCHEDULING_VECTOR.size()-1;i++)
		{
			if (SCHEDULING_VECTOR[i].OFFSET+SCHEDULING_VECTOR[i].DURATION >SCHEDULING_VECTOR[i+1].OFFSET)
			{
				LOGGER(FATAL,"WRONG SCHEDULE: Partition %s Offset %lld is greater than previous Partition %s's Offset %lld + Duration %lld",
					PARTITION_VECTOR[SCHEDULING_VECTOR[i+1].PARTITION_INDEX].Name.c_str(),
					SCHEDULING_VECTOR[i+1].OFFSET,
					PARTITION_VECTOR[SCHEDULING_VECTOR[i].PARTITION_INDEX].Name.c_str(),
					SCHEDULING_VECTOR[i].OFFSET,
					SCHEDULING_VECTOR[i].DURATION);
		...
		if (SCHEDULING_VECTOR[SCHEDULING_VECTOR.size()-1].OFFSET+SCHEDULING_VECTOR[SCHEDULING_VECTOR.size()-1].DURATION> this->MAJOR_FRAME_SIZE)
		{
			LOGGER(FATAL,"WRONG SCHEDULE: Hyperperiod %lld is smaller  than last Partition %s's Offset %lld + Duration %lld",
				this->MAJOR_FRAME_SIZE,					  
				PARTITION_VECTOR[SCHEDULING_VECTOR[SCHEDULING_VECTOR.size()-1].PARTITION_INDEX].Name.c_str(),
	...
	MODULE_MANAGER::~MODULE_MANAGER(){
		...
		for(std::map<std::string,QUEUE_CHANNEL*>::iterator iter=MAP_OF_QUEUEING_CHANNELS.begin();iter!=MAP_OF_QUEUEING_CHANNELS.end();iter++)
		{
		...
		for(std::map<std::string,SAMPLING_CHANNEL*>::iterator iter=MAP_OF_SAMPLING_CHANNELS.begin();iter!=MAP_OF_SAMPLING_CHANNELS.end();iter++)
		{
			if(iter->second !=0 ) delete (iter->second);
		}
		MAP_OF_SAMPLING_CHANNELS.clear();

		for(std::map<std::string,MESSAGE_QUEUE_TYPE*>::iterator iter=MAP_OF_CONFIGURATION_QUEUE.begin();iter!=MAP_OF_CONFIGURATION_QUEUE.end();iter++)
		{
			if(iter->second !=0 ) delete (iter->second);
		}

		if(Recv_Socket!=0) delete (Recv_Socket);
		if (Send_Socket!=0) delete (Send_Socket);
	...
	void MODULE_MANAGER::DOSHUTDOWN_BEFORE_INIT(){
		...
		ACM_INTERNAL::MODULE_MANAGER* partManager =ACM_INTERNAL::MODULE_MANAGER::Instance();
		...
		for(unsigned int i =0; i<partManager->PARTITION_VECTOR.size();i++){
			if(partManager->PARTITION_VECTOR[i].PID==0) continue;
			LOGGER(HMEVENT,"SENDING MESSAGE TO %s (%d)",partManager->PARTITION_VECTOR[i].Name.c_str(),partManager->PARTITION_VECTOR[i].PID);
			//Raise the priority...
			...
			LOGGER(HMEVENT,"%s (%d) killed.",partManager->PARTITION_VECTOR[i].Name.c_str(),partManager->PARTITION_VECTOR[i].PID);
		}
		...
		if(partManager!=0) delete(partManager);
		exit(0);
	...
	void MODULE_MANAGER::DOSHUTDOWN(bool proper){
		ACM_INTERNAL::MODULE_MANAGER* partManager =ACM_INTERNAL::MODULE_MANAGER::Instance();
		if( partManager->DUMMY_PROCESS_ID !=0){
			LOGGER(INFO,"Module Manager, Received Sigint. SENDING SIGINT to dummy Process");
