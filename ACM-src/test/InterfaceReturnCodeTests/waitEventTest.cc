...
void PROCESS_APERIODIC (void){
	...
	//Tests no error is returned when correctly waiting on an event in the up state
	SET_EVENT(SOME_EVENT_ID, &RETURN_CODE);
	START(PROCESS_TIMEOUT_ID, &WAIT_RETURN);
	LOGGER(APP,"Waiting on up event.");  
	START(PROCESS_TIMEOUT_ID, &WAIT_RETURN);
	WAIT_EVENT(SOME_EVENT_ID,convertSecsToNano(1), &RETURN_CODE); //Return code is checked in parallel process	
	LOGGER(APP, "Done waiting.");
	TIMED_WAIT(convertSecsToNano(1.5), &WAIT_RETURN);

	//Tests that the correct code is returned when attempting to wait on an event in the down state
	...
	//Tests that the correct code is returned when attempting to wait on an event that does not exist
