...
// template<typename T> class STATE_VARIABLE;  // pre-declare the template class
// itself
// template<typename T> T operator+ (const STATE_VARIABLE<T>& lhs, const
// STATE_VARIABLE<T>& rhs);
// template<typename T> T operator> (const STATE_VARIABLE<T>& lhs, const
// STATE_VARIABLE<T>& rhs);
// template<typename T> T operator>= (const STATE_VARIABLE<T>& lhs, const
// STATE_VARIABLE<T>& rhs);
...
// template<typename T> std::ostream& operator<< (std::ostream& o, const Foo<T>&
// x);

template <class T> class STATE_VARIABLE {
private:
  struct VALUE_ITEM {
    T VALUE;
    SYSTEM_TIME_TYPE TIME_OF_CREATION;
    VALUE_ITEM(T &DATA)
        : VALUE(DATA), TIME_OF_CREATION(currentTimeInNanoSeconds()) {}
    VALUE_ITEM(T &DATA, SYSTEM_TIME_TYPE &now)
        : VALUE(DATA), TIME_OF_CREATION(now) {}
  };
...
public:
  ...
  /*! TIME_INDEX should always be <=0. 0 implies current sample, -1 imples 1
     previous sample and so on
          Will return INVALID_PARAM if TIME_INDEX >0

  */
  void GET(T *outData, RETURN_CODE_TYPE *RETURN_CODE,
           int TIME_INDEX = 0) const {
    *RETURN_CODE = NO_ERROR;
  ...
  void GET(T *outData, SYSTEM_TIME_TYPE *outTime, RETURN_CODE_TYPE *RETURN_CODE,
           int TIME_INDEX = 0) const {
    *RETURN_CODE = NO_ERROR;
  ...
  void GET_TIMESTAMP(SYSTEM_TIME_TYPE *outData, RETURN_CODE_TYPE *RETURN_CODE,
                     int TIME_INDEX = 0) const {
    *RETURN_CODE = NO_ERROR;
  ...
  SYSTEM_TIME_TYPE GET_TIMESTAMP() { return this->VALUE().TIME_OF_CREATION; }
  ...
  double RATE_OF_CHANGE(RETURN_CODE_TYPE *RETURN_CODE) const {
    *RETURN_CODE = NO_ERROR;
    ...
    double timedelta = (double)(VALUES[size - 1].TIME_OF_CREATION -
                                VALUES[size - 2].TIME_OF_CREATION);
    double timedeltainseconds = timedelta / NANO;

    double rate = ((double)(VALUES[size - 1].VALUE - VALUES[size - 2].VALUE)) /
                  (timedeltainseconds);
    return rate;
  }

  double RATE_OF_CHANGE(T Second, SYSTEM_TIME_TYPE now,
                        RETURN_CODE_TYPE *RETURN_CODE) const {
    *RETURN_CODE = NO_ERROR;
    if (VALUES.size() == 0) {
      *RETURN_CODE = NOT_AVAILABLE;
      return 0;
    ...
    double timedeltainseconds = timedelta / NANO;
    double rate =
        ((double)(Second - VALUES[size - 1].VALUE)) / (timedeltainseconds);
    return rate;
  }
  ...
  // SERIALIZE_TO_BUFFER
  void CHECKPOINT(MESSAGE_ADDR_TYPE MESSAGE_ADDR, MESSAGE_SIZE_TYPE MAX_LENGTH,
                  MESSAGE_SIZE_TYPE *LENGTH_WRITTEN,
    ...
    *LENGTH_WRITTEN = 0;
    ...
    if (MAX_LENGTH <
        sizeof(T) + sizeof(SYSTEM_TIME_TYPE) + sizeof(APEX_INTEGER)) {
    ...
    *LENGTH_WRITTEN = *LENGTH_WRITTEN + sizeof(T);
    memcpy(MESSAGE_ADDR + (*LENGTH_WRITTEN), (void *)&(timeval),
           sizeof(SYSTEM_TIME_TYPE));
    *LENGTH_WRITTEN = *LENGTH_WRITTEN + sizeof(SYSTEM_TIME_TYPE);
    return;
  }

  void RESTORE(MESSAGE_ADDR_TYPE MESSAGE_ADDR, MESSAGE_SIZE_TYPE LENGTH,
               RETURN_CODE_TYPE *RETURN_CODE) {
    this->RESET();
    *RETURN_CODE = NO_ERROR;
    ...
    if (lengthr != LENGTH) {
      // Problem
      LOGGER(ERROR, "Header Length read from buffer not same as length passed "
                    "in the restore API");
      *RETURN_CODE = INVALID_CONFIG;
      return;
    }
    ...
    memcpy((void *)&timeval, MESSAGE_ADDR + (sizeof(APEX_INTEGER) + sizeof(T)),
           sizeof(SYSTEM_TIME_TYPE));
