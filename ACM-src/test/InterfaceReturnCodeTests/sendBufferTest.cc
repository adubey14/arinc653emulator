...
void PROCESS_APERIODIC(void) {
  ...
  // Tests no error is returned when correctly when sending a message in a
  // buffer
  ...
  SEND_BUFFER(SHORT_BUFFER_ID, (APEX_BYTE *)(&myLoad), sizeof(myLoad),
              convertSecsToNano(1), &RETURN_CODE);
  ...
  // range
  SEND_BUFFER(SOME_BUFFER_ID, (APEX_BYTE *)(&myLoad), sizeof(myLoad),
              convertSecsToNano(315569261), &RETURN_CODE);
  if (RETURN_CODE == INVALID_PARAM) {
  ...
  // Tests that the correct code is returned when attempting to send a message
  // in a buffer that does not exist
  BUFFER_ID_TYPE NONEXISTENT_ID = 10;
  SEND_BUFFER(NONEXISTENT_ID, (APEX_BYTE *)(&myLoad), sizeof(myLoad),
              convertSecsToNano(1), &RETURN_CODE);
  if (RETURN_CODE == INVALID_PARAM) {
...
int main() {
  ...
  CREATE_BUFFER(SHORT_BUFFER_NAME, 1, 1 /*number of messages*/, FIFO,
                &SHORT_BUFFER_ID, &RETURN_CODE);
