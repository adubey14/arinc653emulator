...
namespace ACM_INTERNAL{
	...
	public:
		...
		APEX_EVENT_TYPE(std::string name, MUTEX* useMutex =0, bool locked = false, MUTEX::Attribute attr = MUTEX::RECURSIVE);
		bool CreatedMutex;
	public:

		std::string GET_NAME();
		static APEX_EVENT_TYPE* CREATE_EVENT(std::string name, MUTEX* useMutex =0, bool locked = false, MUTEX::Attribute attr = MUTEX::RECURSIVE);
		//	static APEX_EVENT_TYPE* CREATE_EVENT(std::string name, MUTEX* useMutex,  bool locked = false, MUTEX::Attribute attr = MUTEX::RECURSIVE);
		...
		void WAIT_AND_LOCK_EVENT(SYSTEM_TIMESPEC TIME_OUT,RETURN_CODE_TYPE *RETURN_CODE);
