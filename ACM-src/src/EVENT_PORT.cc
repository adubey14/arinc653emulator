...
namespace ACM_USER{
	...
	ACM_EVENT_PORT::DESTINATION_URL::DESTINATION_URL(NAME_TYPE name,const unsigned int port_num)
	{
	...
	void ACM_EVENT_PORT::SET_ONLY_INTER_MODULE(RETURN_CODE_TYPE* RETURN_CODE)
	{
		...
		{
			LOGGER(ERROR,"trying to SET_ONLY_INTER_MODULE in port %s. Ensure  Destination URLS have been added",EVENT_PORT_NAME);
	...
	void ACM_EVENT_PORT::ADD_INTER_MODULE_DESTINATION_URL(NAME_TYPE nodename, unsigned int port_number,RETURN_CODE_TYPE *RETURN_CODE)
	{
		*RETURN_CODE=NO_ERROR;
		if(EVENT_PORT_TYPE==READ)
		{
			LOGGER(ERROR,"Trying to add INTER_MODULE_DESTINATION_URL to a read port %s",EVENT_PORT_NAME);
		...
		{
			LOGGER(ERROR,"Trying to add INTER_MODULE_DESTINATION_URL to an intra-partition port %s",EVENT_PORT_NAME);
			*RETURN_CODE=INVALID_MODE;
			return;
		...
		{
			INTER_MODULE_DATA_BUFFER = new char[sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+EVENT_PORT_TOTAL_SIZE];
			LOGGER(INFO,"Size of INTER_MODULE_DATA_BUFFER %d",sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+EVENT_PORT_TOTAL_SIZE);
		}
	...
	void ACM_EVENT_PORT::ADD_INTER_MODULE_SOURCE_SOCKET(unsigned int port_number,RETURN_CODE_TYPE *RETURN_CODE)
	{
		*RETURN_CODE=NO_ERROR;
		if(EVENT_PORT_TYPE==WRITE)
		{
			LOGGER(ERROR,"Trying to  ADD_INTER_MODULE_SOURCE_SOCKET to a write port %s",EVENT_PORT_NAME);
			*RETURN_CODE=INVALID_MODE;
			return;
		}
		...
		if (EVENT_PORT_IMPL_TYPE==BUFFER || EVENT_PORT_IMPL_TYPE  == BLACKBOARD)
		{
			LOGGER(ERROR,"Trying to  ADD_INTER_MODULE_SOURCE_SOCKET to an intra-partition port %s",EVENT_PORT_NAME);
			*RETURN_CODE=INVALID_MODE;
			return;
		...
		{
			LOGGER(ERROR,"Recv Socket Already initializaed. Trying to add it again: Event Port %s",EVENT_PORT_NAME);
	...
	ACM_EVENT_PORT::ACM_EVENT_PORT(EVENT_PORT_COMM_TYPE COMM_TYPE, 
		...
		bool isCOLOCATED,bool isDESTRUCTIVEREAD ):
	//member variable initialization
	EVENT_PORT_TYPE(COMM_TYPE),
		EVENT_PORT_VALIDITY_PERIOD(VALIDITY_PERIOD),
		EVENT_PORT_PAYLOAD(0),
		EVENT_PORT_IMPL_TYPE(ACM_EVENT_PORT::UNDEFINED),					
		EVENT_PORT_DESTRUCTIVE_READ(isDESTRUCTIVEREAD),
		sender_mySocket(0),
		recv_mySocket(0),
		INTER_MODULE_DATA_BUFFER(0),
		ONLY_INTER_MODULE(false)
	{
		...
		PORT_DIRECTION_TYPE PORT_DIRECTION = (COMM_TYPE==WRITE)?SOURCE:DESTINATION;
		...
		case SAMPLINGPORT:
			...
			CREATE_SAMPLING_PORT(NAME,EVENT_PORT_TOTAL_SIZE,PORT_DIRECTION,VALIDITY_PERIOD,&EVENT_PORT_ID,RETURN_CODE);
		...
		case QUEUEINGPORT:
			...
			CREATE_QUEUING_PORT(NAME,EVENT_PORT_TOTAL_SIZE,MAX_NB_MESSAGE,PORT_DIRECTION,FIFO,&EVENT_PORT_ID,RETURN_CODE);
			//already created, then get the id
			if (*RETURN_CODE == NO_ACTION)
		...
		case BUFFER:
			...
			CREATE_BUFFER(NAME,EVENT_PORT_TOTAL_SIZE,MAX_NB_MESSAGE, FIFO,&EVENT_PORT_ID,RETURN_CODE);
			//already created, then get the id
			if (*RETURN_CODE == NO_ACTION)
			{
	...
	void ACM_EVENT_PORT::BLOCK_AND_TRANSFER_FROM_SOCKET_READ_PORT(RETURN_CODE_TYPE *RETURN_CODE,SYSTEM_TIME_TYPE PORT_TIME_OUT)
	{
		*RETURN_CODE=NO_ERROR;
		if(EVENT_PORT_TYPE==WRITE)
		{
			LOGGER(ERROR,"Trying to  BLOCK_AND_TRANSFER_FROM_SOCKET_READ_PORT to a write port %s",EVENT_PORT_NAME);
			*RETURN_CODE=INVALID_MODE;
			return;
		}
		...
		if (EVENT_PORT_IMPL_TYPE==BUFFER || EVENT_PORT_IMPL_TYPE  == BLACKBOARD)
		{
			LOGGER(ERROR,"Trying to  BLOCK_AND_TRANSFER_FROM_SOCKET_READ_PORT in an intra-partition port %s",EVENT_PORT_NAME);
			*RETURN_CODE=INVALID_MODE;
			return;
		...
		recvMsgSize = recv_mySocket->recvFrom(&(INTER_MODULE_DATA_BUFFER[0]), sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+EVENT_PORT_TOTAL_SIZE, sourceAddress,	sourcePort);
		LOGGER(INFO,"FROM Event Port %s Received %d bytes.",EVENT_PORT_NAME,recvMsgSize);
		...
		case BUFFER:
			break;
		case SAMPLINGPORT:
			WRITE_CONTENTS_FROM_DATA_BUFFER(EVENT_PORT_ID,(MESSAGE_ADDR_TYPE)&INTER_MODULE_DATA_BUFFER[0],sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+EVENT_PORT_TOTAL_SIZE,RETURN_CODE);
			if (*RETURN_CODE!=NO_ERROR) 
			{
				LOGGER(ERROR,"FROM  Event port socket read %s, error while writing to destination queue ",this->EVENT_PORT_NAME);
			}
			break;
		case QUEUEINGPORT:
			MESSAGE_QUEUE_SEND_MESSAGE_FROM_BUFFER(EVENT_PORT_ID, (MESSAGE_ADDR_TYPE)&INTER_MODULE_DATA_BUFFER[0], recvMsgSize, PORT_TIME_OUT, RETURN_CODE);
			if (*RETURN_CODE!=NO_ERROR) 
			{
				if (*RETURN_CODE==TIMED_OUT) 
					LOGGER(ERROR,"FROM Event port socket read %s, TIME OUT while writing to destination queue ",EVENT_PORT_NAME);
				else
					LOGGER(ERROR,"FROM  Event port socket read %s, error while writing to destination queue ",this->EVENT_PORT_NAME);								
			}
			break;
		...
		RETURN_CODE_TYPE *RETURN_CODE)
	{
		...
		case BLACKBOARD:
			...
			DISPLAY_BLACKBOARD (EVENT_PORT_ID, EVENT_PORT_PAYLOAD, NEW_LENGTH,  RETURN_CODE);
			break;

		case BUFFER:

			SEND_BUFFER (EVENT_PORT_ID, EVENT_PORT_PAYLOAD, NEW_LENGTH, TIME_OUT, RETURN_CODE);
			break;

		case SAMPLINGPORT:

			WRITE_SAMPLING_MESSAGE (EVENT_PORT_ID,EVENT_PORT_PAYLOAD,NEW_LENGTH,RETURN_CODE);

			if(sender_mySocket!=0)
			{
				assert(INTER_MODULE_DATA_BUFFER!=0);
				MESSAGE_SIZE_TYPE READ_LENGTH;
				READ_CONTENTS_TO_DATA_BUFFER(EVENT_PORT_ID,(MESSAGE_ADDR_TYPE)&INTER_MODULE_DATA_BUFFER[0],
					sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+EVENT_PORT_TOTAL_SIZE,
					&READ_LENGTH,RETURN_CODE);
				assert(*RETURN_CODE==NO_ERROR);
			LOGGER(INFO,"size of written message to Socket %ld",READ_LENGTH);
			//	LOGGER(INFO,"Read Contents from buffer. Trying to send to destination");

				for(unsigned int counter=0;counter<this->DESTINATIONS_URLS.size();counter++)
				{
					this->DESTINATIONS_URLS[counter].printHere();
					
					this->sender_mySocket->sendTo(INTER_MODULE_DATA_BUFFER, READ_LENGTH, DESTINATIONS_URLS[counter].node_name, DESTINATIONS_URLS[counter].port);  					
				}
			}
		...
		case QUEUEINGPORT:
			...
			{
				SEND_QUEUING_MESSAGE (EVENT_PORT_ID, EVENT_PORT_PAYLOAD, NEW_LENGTH, TIME_OUT, RETURN_CODE);
			}
			...
			if(sender_mySocket!=0)
			{
				...
				for(unsigned int counter=0;counter<this->DESTINATIONS_URLS.size();counter++)
				{
					this->sender_mySocket->sendTo(EVENT_PORT_PAYLOAD, NEW_LENGTH, DESTINATIONS_URLS[counter].node_name, DESTINATIONS_URLS[counter].port);  					
				}
			}
			break;
		...
		RETURN_CODE_TYPE *RETURN_CODE)
	{
		...
		case BLACKBOARD:
			...
			READ_BLACKBOARD (EVENT_PORT_ID,TIME_OUT,EVENT_PORT_PAYLOAD,LENGTH,RETURN_CODE);
		...
		case BUFFER:
			...
			RECEIVE_BUFFER (EVENT_PORT_ID,TIME_OUT,EVENT_PORT_PAYLOAD,LENGTH,RETURN_CODE);
			LOGGER(TRACE,"FINISHED READING BUFFER");
			break;

		case SAMPLINGPORT:

			READ_SAMPLING_MESSAGE (EVENT_PORT_ID,EVENT_PORT_PAYLOAD,LENGTH,&VALIDITY, RETURN_CODE );
			LOGGER(TRACE,"FINISHED READING SAMPLING MESSAGE");
			break;

		case QUEUEINGPORT:

			RECEIVE_QUEUING_MESSAGE (EVENT_PORT_ID,TIME_OUT,EVENT_PORT_PAYLOAD,LENGTH,RETURN_CODE );
			break;
		...
		PARSE_AND_VALIDATE_DATA(MESSAGE_ADDR,LENGTH,VALIDITY,IS_VALID,RETURN_CODE);
	...
	//Possible Problem
	void ACM_EVENT_PORT::PARSE_AND_VALIDATE_DATA(
