/*
Test of the RESET_EVENT service
*/
...
void PROCESS_APERIODIC (void){
	...
	//Tests that no error is returned when correctly resetting an event
	RESET_EVENT(SOME_EVENT_ID, &RETURN_CODE);		
	if(RETURN_CODE == NO_ERROR){
		LOGGER(APP,"Reset event test Succeeded!");  
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);  
	}
	else{
		LOGGER(APP,"Reset event test Failed."); 
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);  
	}

	EVENT_ID_TYPE NONEXISTENT_ID = 10;
	//Tests that the correct code is returned when attempting to rest an event that does not exist
	RESET_EVENT(NONEXISTENT_ID, &RETURN_CODE);
