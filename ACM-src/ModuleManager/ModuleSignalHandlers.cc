namespace ACM_INTERNAL{
	...
	void MODULE_MANAGER::SIGTERM_HANDLER(int sig){
		...
		if( partManager->DUMMY_PROCESS_ID !=0){
			LOGGER(INFO,"Module Manager, Received SigTerm. SENDING SIGTERM to dummy Process");
	...
	void MODULE_MANAGER::SIGINT_HANDLER(int sig){
		...
		{
			if(partManager->PARTITION_VECTOR[i].PID==0) continue;			
			LOGGER(INFO,"SENDING MESSAGE SIGSTOP TO %d",partManager->PARTITION_VECTOR[i].PID);
			kill(partManager->PARTITION_VECTOR[i].PID,SIGSTOP);		
		}
		...
		{
			if(partManager->PARTITION_VECTOR[i].PID==0) continue;			
			LOGGER(INFO,"SENDING MESSAGE SIGCONT and SIGINT TO %d",partManager->PARTITION_VECTOR[i].PID);
			kill(partManager->PARTITION_VECTOR[i].PID,SIGCONT);
			kill(partManager->PARTITION_VECTOR[i].PID,SIGINT);
			//HELPER_CLOCK_NANOSLEEP(convertSecsToNano(10));
			waitpid(partManager->PARTITION_VECTOR[i].PID,NULL,0);
			/*for (unsigned int count=0;count < partManager->PARTITION_VECTOR[i].QUEUE_CHANNEL_SOURCE_VECTOR.size();count++)
			{
			...
			/*for (unsigned int count=0;count<partManager->PARTITION_VECTOR[i].SAMPLING_CHANNEL_SOURCE_VECTOR.size();count++)
			{
			if (partManager->PARTITION_VECTOR[i].SAMPLING_CHANNEL_SOURCE_VECTOR[count]!=0)
