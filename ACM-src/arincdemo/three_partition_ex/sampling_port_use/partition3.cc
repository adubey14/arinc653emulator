...
SAMPLING_PORT_NAME_TYPE SAMPLING_PORT_NAME =
    "SAMPLE_PORT_DEST_3"; // This port will be visible if you list message
...
void PROCESS_PERIODIC(void) {
  ...
  if (LENGTH == sizeof(Payload)) {
    if (VALIDITY == INVALID)
      LOGGER(USER, "Read Invalid message %f at %lld", data.data, data.time);
    if (VALIDITY == VALID)
      LOGGER(USER, "Read Valid message %f at %lld", data.data, data.time);
  } else {
...
int main() {
  ...
  SET_PARTITION_NAME("Partition 3");
  ...
  PROCESS_PERIODIC_ATTR.PERIOD = convertSecsToNano(3); // PERIODIC
