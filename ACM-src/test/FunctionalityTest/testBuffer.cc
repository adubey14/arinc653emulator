...
void testmethod(void){
	...
	while(1)
	{
		...
		LOGGER(APP,"Received length of message %d. Received message %d",LENGTH,j);



		LOGGER( INFO, "From testmethod: OUT OF EVENT. The return code is %d", RETURN_CODE);		
		//sleep(10);
	}
...
void testperiodic(void){
	...
	LOGGER(INFO, "From testperiodic:  Out of event:The return code is %d", RETURN_CODE);
	if (RETURN_CODE!=6)
...
void anotherperiodic(void){
	...
	LOGGER(INFO,"From anotherperiodic:  Finished Work in anotherperiodic:The return code is %d", RETURN_CODE);				
}
...
int main(int argc, char* argv []){
	...
	LOGGER( APP,"Testapp Started. The clock_realtime resolution is %ld sec and %ld nanoseconds",res.tv_sec,res.tv_nsec);
