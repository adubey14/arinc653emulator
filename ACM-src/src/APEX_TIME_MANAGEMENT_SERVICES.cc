...
void APEX::PERIODIC_WAIT(
  ...
  *RETURN_CODE = NO_ERROR;
  // PROCESS_ID_TO_PTHREAD_ID_MAP_TYPE::iterator
  // iter=partition_instance->PROCESS_ID_TO_PTHREAD_ID_MAP.begin();
  ...
  if (!process->isPeriodic()) {
    LOGGER(ERROR, "PROCESS %s is not periodic",
           partition_instance->PROCESS_DESCRIPTOR_VECTOR[PROCESS_ID]
               ->THE_PROCESS->attribute.NAME);
    *RETURN_CODE = INVALID_MODE;
    return;
  }

  //!@abhishek : changing to make sure that the process waits on waiting on
  //!period.
...
void APEX::REPLENISH(/*!in */ SYSTEM_TIME_TYPE BUDGET_TIME,
                     /*!out*/ RETURN_CODE_TYPE *RETURN_CODE) {
  ...
  if (BUDGET_TIME != INFINITE_TIME_VALUE &&
      BUDGET_TIME > (SYSTEM_TIME_TYPE)MAX_TIME_OUT) {
  ...
  SYSTEM_TIMESPEC new_Deadline =
      computeDeadlineTimeSpecs(currentTime(), BUDGET_TIME);
  if (process->isPeriodic()) {
    if (compareTime(partition_instance->PROCESS_DESCRIPTOR_VECTOR[PROCESS_ID]
                        ->ABSOLUTE_NEXT_SCHEDULED_TIME,
                    new_Deadline)) {
      LOGGER(INFO,
             "Absolute next scheduled time is %lld and new deadline is %lld",
             convertTimeSpecToNano(
                 partition_instance->PROCESS_DESCRIPTOR_VECTOR[PROCESS_ID]
                     ->ABSOLUTE_NEXT_SCHEDULED_TIME),
    ...
    // LOGGER(INFO,"Absolute next scheduled time is okay with the new
    // deadline");
  }
  ...
  partition_instance->PROCESS_DESCRIPTOR_VECTOR[PROCESS_ID]->DEADLINE_TIME =
      new_Deadline;
  partition_instance->PROCESS_DESCRIPTOR_VECTOR[PROCESS_ID]
      ->ABSOLUTE_CRITICAL_TIME = new_Deadline;
  partition_instance->APEX_SCHEDULING_EVENT->SET_EVENT(
      RETURN_CODE); // release event. This will cause the unlock
