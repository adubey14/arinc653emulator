...
const NETWORK_PARTITION_PETRI_NET_TOKEN_TYPE NETWORK_PARTITION_PETRI_NET_TOKEN =
    "GO";
...
struct SocketSynchronizationMessageType {
  ...
  SocketSynchronizationMessageType(SyncType sync1, std::string src_name,
                                   std::string dst_name,
                                   SYSTEM_TIME_TYPE WAIT = 0)
...
#define SyncTypeSize                                                           \
  sizeof(SocketSynchronizationMessageType::SyncType) +                         \
      sizeof(ACM_USER::NAME_TYPE) + sizeof(ACM_USER::NAME_TYPE) +              \
      sizeof(SYSTEM_TIME_TYPE) + sizeof(ACM_USER::APEX_LONG_INTEGER)
  ...
  void printout() {
    ...
    LOGGER(INFO, "SyncType:%s, src:%s, dst:%s, wait: %lld ns: seq_id: %lld",
           syncName.c_str(), MESSAGE_SRC_MODULE, MESSAGE_DST_MODULE,
           NANOSECONDS_TO_WAIT, seq_id);
  }

  void increment() { this->seq_id++; }

  static void TO_SERIAL_BUFFER(SERIAL_BUFFER_TYPE &SERIAL_BUFFER,
                               SocketSynchronizationMessageType &message) {
    unsigned int written = 0;
    memcpy(&(SERIAL_BUFFER[0]), &(message.Sync), sizeof(SyncType));
    written += sizeof(SyncType);
    memcpy(&(SERIAL_BUFFER[0]) + written, &(message.MESSAGE_SRC_MODULE[0]),
           sizeof(ACM_USER::NAME_TYPE));
    written += sizeof(ACM_USER::NAME_TYPE);
    memcpy(&(SERIAL_BUFFER[0]) + written, &(message.MESSAGE_DST_MODULE[0]),
           sizeof(ACM_USER::NAME_TYPE));
    written += sizeof(ACM_USER::NAME_TYPE);
    memcpy(&(SERIAL_BUFFER[0]) + written, &(message.NANOSECONDS_TO_WAIT),
           sizeof(SYSTEM_TIME_TYPE));
    written += sizeof(SYSTEM_TIME_TYPE);
    memcpy(&(SERIAL_BUFFER[0]) + written, &(message.seq_id),
           sizeof(APEX_LONG_INTEGER));
    written += sizeof(APEX_LONG_INTEGER);
    return;
  }

  static void FROM_SERIAL_BUFFER(SocketSynchronizationMessageType &message,
                                 SERIAL_BUFFER_TYPE &SERIAL_BUFFER) {
    unsigned int written = 0;
    memcpy(&(message.Sync), &(SERIAL_BUFFER[0]), sizeof(SyncType));
    written += sizeof(SyncType);
    memcpy(&(message.MESSAGE_SRC_MODULE[0]), &(SERIAL_BUFFER[0]) + written,
           sizeof(ACM_USER::NAME_TYPE));
    written += sizeof(ACM_USER::NAME_TYPE);
    memcpy(&(message.MESSAGE_DST_MODULE[0]), &(SERIAL_BUFFER[0]) + written,
           sizeof(ACM_USER::NAME_TYPE));
    written += sizeof(ACM_USER::NAME_TYPE);
    memcpy(&(message.NANOSECONDS_TO_WAIT), &(SERIAL_BUFFER[0]) + written,
           sizeof(SYSTEM_TIME_TYPE));
    written += sizeof(SYSTEM_TIME_TYPE);
    memcpy(&(message.seq_id), &(SERIAL_BUFFER[0]) + written,
           sizeof(APEX_LONG_INTEGER));
    written += sizeof(APEX_LONG_INTEGER);
    return;
  }
...
struct SCHEDULING_TUPLE {
  ...
  void printDescriptor() {
    LOGGER(INFO, "PARTITION_INDEX:%d OFFSET:%lld,DURATION:%lld",
           PARTITION_INDEX, OFFSET, DURATION);
  }
};

struct PROXY_PORT_DESCRIPTORS {
  enum PROXY_PORT_DESCRIPTORS_TYPE { SAMPLING, QUEUEING };
  ...
  void printout() {
    LOGGER(INFO, " proxy Port "
                 "Name:%s,Module_name:%s,port_number:%u,MAX_MESSAGE_SIZE:%d,"
                 "MAX_NB_MESSAGE:%d,PORT_DIRECTION:%s",
           Name.c_str(), hostName.c_str(), portNumber, MAX_MESSAGE_SIZE,
           MAX_NB_MESSAGE,
           (PORT_DIRECTION == SOURCE) ? "SOURCE" : "DESTINATION");
  }
};
...
struct QUEUING_PORT_DESCRIPTOR {
  ...
  PORT_DIRECTION_TYPE PORT_DIRECTION;
  void printout() {
    LOGGER(INFO, " Queuing Port "
                 "Name:%s,MAX_MESSAGE_SIZE:%d,MAX_NB_MESSAGE:%d,PORT_DIRECTION:"
                 "%s",
           Name.c_str(), MAX_MESSAGE_SIZE, MAX_NB_MESSAGE,
           (PORT_DIRECTION == SOURCE) ? "SOURCE" : "DESTINATION");
  }
};
...
struct SAMPLING_PORT_DESCRIPTOR {
  ...
  PORT_DIRECTION_TYPE PORT_DIRECTION;
  void printout() {
    LOGGER(INFO, "Sampling Port "
                 "Name:%s,MAX_MESSAGE_SIZE:%d,REFRESH_PERIOD:%lld,PORT_"
                 "DIRECTION:%s",
           Name.c_str(), MAX_MESSAGE_SIZE, REFRESH_PERIOD,
           (PORT_DIRECTION == SOURCE) ? "SOURCE" : "DESTINATION");
  }
};
...
struct PARTITION {
  ...
  inline bool CheckIfChannelExists(std::string &name) {
    ...
    for (unsigned int i = 0; i < SAMPLING_CHANNEL_SOURCE_VECTOR.size(); i++) {
      if (SAMPLING_CHANNEL_SOURCE_VECTOR[i].compare(name) == 0)
        return true;
    }

    for (unsigned int i = 0; i < QUEUE_CHANNEL_SOURCE_VECTOR.size(); i++) {
      if (QUEUE_CHANNEL_SOURCE_VECTOR[i].compare(name) == 0)
        return true;
    }

    for (unsigned int i = 0; i < QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR.size();
         i++) {
      if (QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[i].compare(name) == 0)
        return true;
    }

    for (unsigned int i = 0;
         i < SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR.size(); i++) {
      if (SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR[i].compare(name) == 0)
        return true;
    }
  ...
  void printDescriptor() {
    if (isNetworkPartition) {
      ...
      LOGGER(INFO, "Partition: %s, Executable: %s", Name.c_str(),
             Executable.c_str());
    }
    for (unsigned int i = 0; i < QUEUING_PORTS.size(); i++)
      LOGGER(INFO, "Partition: %s, Queuing Port "
                   "Name:%s,MAX_MESSAGE_SIZE:%d,MAX_NB_MESSAGE:%d,PORT_"
                   "DIRECTION:%s",
             Name.c_str(), QUEUING_PORTS[i].Name.c_str(),
             QUEUING_PORTS[i].MAX_MESSAGE_SIZE, QUEUING_PORTS[i].MAX_NB_MESSAGE,
             (QUEUING_PORTS[i].PORT_DIRECTION == SOURCE) ? "SOURCE"
                                                         : "DESTINATION");

    for (unsigned int i = 0; i < SAMPLING_PORTS.size(); i++)
      LOGGER(INFO, "Partition: %s, Sampling Port "
                   "Name:%s,MAX_MESSAGE_SIZE:%d,REFRESH_PERIOD:%lld,PORT_"
                   "DIRECTION:%s",
             Name.c_str(), SAMPLING_PORTS[i].Name.c_str(),
             SAMPLING_PORTS[i].MAX_MESSAGE_SIZE,
             SAMPLING_PORTS[i].REFRESH_PERIOD,
             (SAMPLING_PORTS[i].PORT_DIRECTION == SOURCE) ? "SOURCE"
                                                          : "DESTINATION");

    for (unsigned int i = 0; i < QUEUE_CHANNEL_SOURCE_VECTOR.size(); i++)
      LOGGER(INFO, "Partition: %s, Queuing Channel Name: %s", Name.c_str(),
             QUEUE_CHANNEL_SOURCE_VECTOR[i].c_str());

    for (unsigned int i = 0; i < SAMPLING_CHANNEL_SOURCE_VECTOR.size(); i++)
      LOGGER(INFO,
             "Partition: %s, SAMPLING_CHANNEL_SOURCE_VECTOR Channel Name: %s",
             Name.c_str(), SAMPLING_CHANNEL_SOURCE_VECTOR[i].c_str());

    for (unsigned int i = 0; i < QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR.size();
         i++)
      LOGGER(INFO, "Partition: %s, QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR "
                   "Channel Name: %s",
             Name.c_str(), QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[i].c_str());

    // for (unsigned int i=0;i<SAMPLING_CHANNEL_SOURCE_VECTOR.size();i++)
    // LOGGER(INFO,"Partition: %s, Sampling Channel Name: %s, Channel Source %s,
    // Destination Names:
    // %s",Name.c_str(),SAMPLING_CHANNEL_SOURCE_VECTOR[i]->GET_NAME().c_str(),SAMPLING_CHANNEL_SOURCE_VECTOR[i]->GET_SOURCE_NAME().c_str(),SAMPLING_CHANNEL_SOURCE_VECTOR[i]->GET_DESTINATION_NAMES().c_str());

    // for (unsigned int i=0;i<QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR.size();i++)
    //{
    // LOGGER(INFO,"Partition: %s, Queuing Channel Name: %s, Channel Source: %s,
    // Destination Names:
    // %s",Name.c_str(),QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[i]->GET_NAME().c_str(),QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[i]->GET_SOURCE_NAME().c_str(),QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[i]->GET_DESTINATION_NAMES().c_str());

    //}
    // for (unsigned int
    // i=0;i<SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR.size();i++)
    //{
    // LOGGER(INFO,"Partition: %s, Sampling Channel Name: %s, Channel Source %s,
    // Destination Names:
    // %s",Name.c_str(),SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR[i]->GET_NAME().c_str(),SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR[i]->GET_SOURCE_NAME().c_str(),SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR[i]->GET_DESTINATION_NAMES().c_str());
...
// says that the maximum number of partitions is already reached, no more
...
public:
  static bool SCHEDULE_COMP(const SCHEDULING_TUPLE &first,
                            const SCHEDULING_TUPLE &second) {
...
public:
  ...
  int FIND_PARTITION_FROM_QUEUING_PORT(std::string &PORT_NAME,
                                       QUEUING_PORT_DESCRIPTOR &descriptor);
  int FIND_PARTITION_FROM_SAMPLING_PORT(std::string &PORT_NAME,
  ...
  // by sending SIGUSR1 the partition will notify the contoller that it
  // initialized itself
...
public:
  ...
  // destructor
  ~MODULE_MANAGER();
  // we need to have a dummy process with priority level between the active and
  // sleeping worker processec
  ...
  // int CreatePartition( SYSTEM_TIME_TYPE duration, const char* name, const
  // char* configfile );
  ...
  inline void RUN_PARTITION(unsigned int &PARTITION_INDEX,
                            SYSTEM_TIMESPEC &ABSOLUTE_DURATION_TIME) {

    RETURN_CODE_TYPE RETURN_CODE;
    if (!PARTITION_VECTOR[PARTITION_INDEX].isNetworkPartition) {

      for (unsigned int j = 0;
           j < PARTITION_VECTOR[PARTITION_INDEX]
                   .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR.size();
           j++) {
        this->MAP_OF_QUEUEING_CHANNELS
            [PARTITION_VECTOR[PARTITION_INDEX]
                 .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[j]]
                ->MOVE_MESSAGES(&RETURN_CODE);
        if (RETURN_CODE != NO_ERROR)
          LOGGER(HMEVENT, "ERROR while writing to %s",
                 PARTITION_VECTOR[PARTITION_INDEX]
                     .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[j]
                     .c_str());
      ...
      for (unsigned int j = 0;
           ...
           j++) {
        this->MAP_OF_SAMPLING_CHANNELS
            [PARTITION_VECTOR[PARTITION_INDEX]
                 .SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR[j]]
                ->MOVE_MESSAGES(&RETURN_CODE);
        if (RETURN_CODE != NO_ERROR)
          LOGGER(HMEVENT, "ERROR while writing to %s",
                 PARTITION_VECTOR[PARTITION_INDEX]
    ...
    if (PARTITION_VECTOR[PARTITION_INDEX].isNetworkPartition) {
      if (this->MAP_OF_CONFIGURATION_QUEUE.find(
              PARTITION_VECTOR[PARTITION_INDEX].CONFIGURATION_QUEUE) ==
          this->MAP_OF_CONFIGURATION_QUEUE.end())
        LOGGER(ERROR, "CONFIGURATION QUEUE FOR NETWORK PAR is null %s",
               PARTITION_VECTOR[PARTITION_INDEX].Name.c_str());

      else if (this->MAP_OF_CONFIGURATION_QUEUE
                   [PARTITION_VECTOR[PARTITION_INDEX].CONFIGURATION_QUEUE] ==
               0) {
        LOGGER(ERROR, "CONFIGURATION QUEUE FOR NETWORK PAR is null %s",
               PARTITION_VECTOR[PARTITION_INDEX].Name.c_str());
        ...
        RETURN_CODE_TYPE return_code;
        this->MAP_OF_CONFIGURATION_QUEUE[PARTITION_VECTOR[PARTITION_INDEX]
                                             .CONFIGURATION_QUEUE]
            ->SEND_MESSAGE(
                (MESSAGE_ADDR_TYPE)&NETWORK_PARTITION_PETRI_NET_TOKEN[0],
                sizeof(NETWORK_PARTITION_PETRI_NET_TOKEN_TYPE), 0,
                &return_code);
        if (return_code != NO_ERROR) {
          LOGGER(ERROR, "Failed sending message to CONFIGURATION QUEUE FOR "
                        "NETWORK part %s",
                 PARTITION_VECTOR[PARTITION_INDEX].Name.c_str());
        }
      }
    ...
    if (!PARTITION_VECTOR[PARTITION_INDEX].isNetworkPartition) {
      for (unsigned int j = 0;
           j <
           PARTITION_VECTOR[PARTITION_INDEX].QUEUE_CHANNEL_SOURCE_VECTOR.size();
           j++) {
        this->MAP_OF_QUEUEING_CHANNELS[PARTITION_VECTOR[PARTITION_INDEX]
                                           .QUEUE_CHANNEL_SOURCE_VECTOR[j]]
            ->MOVE_MESSAGES(&RETURN_CODE);
        if (RETURN_CODE != NO_ERROR)
          LOGGER(HMEVENT, "ERROR while writing to %s",
                 PARTITION_VECTOR[PARTITION_INDEX]
      ...
      for (unsigned int j = 0; j < PARTITION_VECTOR[PARTITION_INDEX]
                                       .SAMPLING_CHANNEL_SOURCE_VECTOR.size();
           j++) {
        this->MAP_OF_SAMPLING_CHANNELS[PARTITION_VECTOR[PARTITION_INDEX]
                                           .SAMPLING_CHANNEL_SOURCE_VECTOR[j]]
            ->MOVE_MESSAGES(&RETURN_CODE);
        if (RETURN_CODE != NO_ERROR)
          LOGGER(HMEVENT, "ERROR while writing to %s",
                 PARTITION_VECTOR[PARTITION_INDEX]
...
public:
  ...
  void resetchildrenModules() {
    for (std::map<std::string, child_url>::iterator iter =
             childrenModules.begin();
         iter != childrenModules.end(); iter++)
