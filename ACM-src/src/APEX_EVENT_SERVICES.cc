...
*
*/
void APEX::CREATE_EVENT(/*!in */ EVENT_NAME_TYPE EVENT_NAME,
  ...
  if (partition_instance->CURRENT_OPERATING_MODE == IDLE ||
    ...
    LOGGER(ERROR, "From CREATE_EVENT: Trying to create event %s while parition "
                  "is either in IDLE or NORMAL state",
           EVENT_NAME);
  ...
  for (unsigned int i = 0; i < partition_instance->EVENT_VECTOR.size(); i++) {
    if (partition_instance->EVENT_VECTOR[i]->GET_NAME() ==
...
void APEX::SET_EVENT(EVENT_ID_TYPE EVENT_ID, RETURN_CODE_TYPE *RETURN_CODE) {
  ...
  if (partition_instance->EVENT_VECTOR.size() <= EVENT_ID) {
    *RETURN_CODE = INVALID_PARAM;
    ...
    partition_instance->EVENT_VECTOR[EVENT_ID]->SET_EVENT(
        &temp_RETURN_CODE_TYPE);
...
void APEX::RESET_EVENT(
  ...
  if (partition_instance->EVENT_VECTOR.size() <= EVENT_ID) {
    *RETURN_CODE = INVALID_PARAM;
    ...
    RETURN_CODE_TYPE temp_RETURN_CODE_TYPE;
    partition_instance->EVENT_VECTOR[EVENT_ID]->RESET_EVENT(
        &temp_RETURN_CODE_TYPE);
    *RETURN_CODE = temp_RETURN_CODE_TYPE;
  }
}
  ...
  if (partition_instance->EVENT_VECTOR.size() <= EVENT_ID) {
    *RETURN_CODE = INVALID_PARAM;
    ...
    if (TIME_OUT == INFINITE_TIME_VALUE) {
      partition_instance->EVENT_VECTOR[EVENT_ID]->WAIT_EVENT(
          &temp_RETURN_CODE_TYPE);
    } else {
      partition_instance->EVENT_VECTOR[EVENT_ID]->WAIT_EVENT(
          TIME_OUT, &temp_RETURN_CODE_TYPE);
    }
    *RETURN_CODE = temp_RETURN_CODE_TYPE;
  }
}
  ...
  if (partition_instance->EVENT_VECTOR.size() <= EVENT_ID) {
    *RETURN_CODE = INVALID_PARAM;
    ...
    EVENT_STATUS->EVENT_STATE =
        partition_instance->EVENT_VECTOR[EVENT_ID]->GET_STATE();
    EVENT_STATUS->WAITING_PROCESSES =
        partition_instance->EVENT_VECTOR[EVENT_ID]->GET_COUNT();
    *RETURN_CODE = NO_ERROR;
    return;
  }
}
