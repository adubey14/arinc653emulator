...
void PROCESS_PERIODIC(void) {
  ...
  // Tests that no error is returned when setting the partition to the normal
  // mode
  ...
  // Tests that the correct code is returned when setting the partition to the
  // normal mode when it is already in the normal state
  ...
  // Tests that the correct code is returned when setting the partition to an
  // undefined mode
  ...
  // Tests that no error is returned when setting the partition to the idle mode
  // - should cause an error since we only support the normal mode
  if (RETURN_CODE == NO_ERROR) {
