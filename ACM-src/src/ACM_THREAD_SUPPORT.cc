...
AUTO_LOCK::AUTO_LOCK(MUTEX &m) : mymutex(m) { mymutex.lock(); }
AUTO_LOCK::~AUTO_LOCK() { mymutex.unlock(); }
...
COND_VAR::COND_VAR(MUTEX *mut) : mutex(mut) {
  int result = 0;

  pthread_condattr_t cattr;
#ifdef HAVE_PTHREAD_CONDATTR_SETCLOCK
  result = pthread_condattr_setclock(
      &cattr, CLOCK_REALTIME); // all conditions will use the clock_realtime
  assert(!result);
#endif
...
COND_VAR::~COND_VAR() {
  ...
  case EBUSY:
    ...
    break;
  case EINVAL:
    // LOGGER(INFO,"In COND_VAR Destructor: Return Code from Destroy is
    // EINVAL");
    break;
  default:
    break;
...
int result;
assert((result = pthread_cond_timedwait(&cond, &mutex->mutex, &timeout)) !=
EINVAL);
...
int COND_VAR::timed_wait(SYSTEM_TIME_TYPE TIME_OUT) {
  ...
  // std::cout <<" time spec "<<timeout.tv_sec << "."<< timeout.tv_nsec
  // <<std::endl;
  assert((result = pthread_cond_timedwait(&cond, &mutex->mutex, &timeout)) !=
         EINVAL);
  return (result);
...
int COND_VAR::wait() { return (pthread_cond_wait(&cond, &mutex->mutex)); }

void COND_VAR::broadcast() { pthread_cond_broadcast(&cond); }

void COND_VAR::signal() { pthread_cond_signal(&cond); }
...
void RW_LOCK::unlock() { pthread_rwlock_unlock(&rwlock); }
...
SEMAPHORE::SEMAPHORE(SEMAPHORE_VALUE_TYPE value, SEMAPHORE_VALUE_TYPE MAX_VALUE)
    : mutex(false, MUTEX::ERRORCHECK), condition(&mutex), counter(value),
      SEMAPHORE_NAME("NO_NAME"), maximumvalue(MAX_VALUE),
      APEX_UNSIGNED_WAITING_COUNTER(0),
      APEX_UNSIGNED_WAITING_COUNTER_MUTEX(false, MUTEX::NORMAL) {}
SEMAPHORE::SEMAPHORE(SEMAPHORE_NAME_TYPE name, SEMAPHORE_VALUE_TYPE value,
                     SEMAPHORE_VALUE_TYPE MAX_VALUE)
    : mutex(false, MUTEX::ERRORCHECK), condition(&mutex), counter(value),
      SEMAPHORE_NAME(name), maximumvalue(MAX_VALUE),
      APEX_UNSIGNED_WAITING_COUNTER(0),
      APEX_UNSIGNED_WAITING_COUNTER_MUTEX(false, MUTEX::NORMAL) {}
SEMAPHORE::~SEMAPHORE() {}
