...
int MODULE_MANAGER::LOAD_AND_INITIALIZE_PARTITION_EXECUTABLE(
    unsigned int index) {
  LOGGER(INFO, "Entered Load Executable");

  if (index > PARTITION_VECTOR.size()) {
    LOGGER(INFO, "LOAD_AND_INITIALIZE_PARTITION_EXECUTABLE index %d > "
                 "PARTITION_VECTOR.size() %d ",
           index, PARTITION_VECTOR.size());
    return -1;
  }
  ...
  if (newPartitionPID == -1) {
    LOGGER(INFO, "Process creation failed (fork) for process %s",
           partitionToCreate.Name.c_str());
    return -1;
  }
  LOGGER(INFO, "Process fork succeeded for process %s",
         partitionToCreate.Name.c_str());
  ...
  if (newPartitionPID == 0) {
    ...
    /* 11-26-11 AD: Adding to remove partitions from MM group. This will block
     * automatic SIGINTS to be sent to partitions.
    */
    LOGGER(INFO, "Partition %s is initialized with PID: %d and priority: %d",
           partitionToCreate.Name.c_str(), getpid(), sp_child.sched_priority);
    LOGGER(DEBUG, "Creating a new group session for parititon %s",
           partitionToCreate.Name.c_str());
    setpgrp();
    LOGGER(DEBUG, "LAUNCHING PARTITION %s with group id %d",
           partitionToCreate.Name.c_str(), getpgrp());
    ...
    if (partitionToCreate.isNetworkPartition) {
      result =
          execl(partitionToCreate.Executable.c_str(),
                partitionToCreate.Executable.c_str(), this->fileName.c_str(),
                partitionToCreate.Name.c_str(), (char *)NULL);
    } else {
      // is a regular partition
      if (this->DISCOVERY_PROTOCOL == NAMINGSERVICE) {
        char namingservicebuffer[500];
        snprintf(namingservicebuffer, 500,
                 "NameService=corbaloc::%s:%d/NameService",
                 this->NS_HOST_IP.c_str(), this->NS_PORT_NUMBER);
        result = execl(partitionToCreate.Executable.c_str(),
                       partitionToCreate.Executable.c_str(), "-ORBInitRef",
                       namingservicebuffer, (char *)NULL);
      } else {
        // result = execv( partitionToCreate.Executable.c_str(), NULL );
        result = execl(partitionToCreate.Executable.c_str(),
                       partitionToCreate.Executable.c_str(), (char *)NULL);
      }
    }
    if (result == -1) {
      // This failure happens when the user is not a root.	//The framework
      // should log this error and EXIT.
      LOGGER(FATAL, "Error when trying to execv( %s) : %s",
             partitionToCreate.Executable.c_str(), strerror(errno));
    }

    // executable cannot be started
    LOGGER(FATAL,
           "Unable to launch process: %s (PID: %d), killing partition...",
           partitionToCreate.Executable.c_str(), getpid());
    ...
    if (killfile.good() && killfile.is_open()) {
      ...
      killfile << "ps -ef |grep `basename \""
               << PARTITION_VECTOR[index].Executable << "\"` | grep "
               << newPartitionPID << " && echo \"kill -9 " << newPartitionPID
               << " \" && kill -9 " << newPartitionPID << std::endl;
    ...
    // it will be unlocked when partition send SIGUSR1 or the partition process
    // cannot be started
    ...
    if (mutexRetVal == ETIMEDOUT) {
      // store the it in the list
      LOGGER(HMEVENT, "DID Not RECEIVE HANDSHAKE SIGNAL from %s",
             PARTITION_VECTOR[index].Name.c_str());
...
int MODULE_MANAGER::Synchronize() {
  ...
  for (unsigned int index = 0; index < PARTITION_VECTOR.size(); index++) {
    ...
    if (mutexRetVal == ETIMEDOUT) {
      // store the it in the list
      LOGGER(HMEVENT, "DID Not RECEIVE 2nd STAGE HANDSHAKE SIGNAL from %s",
             PARTITION_VECTOR[index].Name.c_str());
      LOGGER(HMEVENT, "STARTING SHUTDOWN PROCEDURE");
      DOSHUTDOWN_BEFORE_INIT();
      // throw(-1);
