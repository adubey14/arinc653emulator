/* This example tests the use of an API used only by the Module Manager and Network Partition */
...
void doSomething()
{
	...
	READ_CONTENTS_TO_DATA_BUFFER(SAMPLING_PORT_ID_SOURCE,(MESSAGE_ADDR_TYPE)&DataBuffer[0],
		sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+11,&LENGTH,&RETURN_CODE);
	assert(RETURN_CODE==NO_ERROR);


	WRITE_CONTENTS_FROM_DATA_BUFFER(	SAMPLING_PORT_ID_DEST,
		(MESSAGE_ADDR_TYPE)&DataBuffer[0],sizeof(SYSTEM_TIME_TYPE)+sizeof(MESSAGE_SIZE_TYPE)+11,&RETURN_CODE);
...
int main(){
	...
	PERIODIC_ID = APEX_HELPER_CREATE_PROCESS(convertSecsToNano(5), //SYSTEM_TIME_TYPE PERIOD,
