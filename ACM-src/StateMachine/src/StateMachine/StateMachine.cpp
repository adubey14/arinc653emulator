...
State::State(Region* pRegion, State* pState,  string& name, StateType t, STATEID sid):
m_pRegion(pRegion),m_pState(pState),m_type(t),m_name(name),m_id(sid),m_ds(0), m_DefaultActionSet(false)
{
...
m_pState(pState),m_name(name),m_priority(priorityID), 
m_initState(0), m_currentState(0),m_historyEnabled(false), m_isDeepHistory(false)
{
	m_childStates.clear();
}
...
Transition::Transition(State* source, State* target,  int priorityID, TRANSID id, bool setHistory):
m_SourceState(source),m_TargetState(target),m_priority(priorityID), m_id(id),m_ds(0),
m_TransitionBetweenParallelMachines(false), m_setHistory(setHistory),m_evalTimeOut(false)
{
	m_StateToExit = 0;
