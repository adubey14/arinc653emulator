...
void PROCESS_PERIODIC(void) {
  ...
  // Tests that "NO_ACTION" is returned when the specified process is not
  // suspended
  RESUME(PROCESS_APERIODIC_ID, &PERIODIC_RETURN);
  ...
  // Tests if the right return code is given when a process tries to resume a
  ...
  // Tests if the right return code is given when a process tries to suspend
  // another process that is in the dormant state
...
void PROCESS_APERIODIC(void) {
  ...
  // Tests if the right return code is given when a process tries to resume
  // itself
