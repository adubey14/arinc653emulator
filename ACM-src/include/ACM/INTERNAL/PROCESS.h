...
public:
  ...
  */
  ...
  /*!
  *
  * This enum is internal to PROCESS and will be mapped to external state as
  * defined by PROCESS_STATE_TYPE
  */
  enum PROCESS_state {
    DORMANT,
    READY,
  ...
  */
  ...
  const PROCESS_ID_TYPE process_id;  //!< A unique id used by Arinc Partition
                                     //!Controller to identify the process
  PROCESS_ATTRIBUTE_TYPE attribute;  //!< PROCESS_ATTRIBUTE_TYPE has to be
                                     //!initialized at the time of construction.
  const Detach_flag thread_detached; //!< Thread attached or detached?. Thread
                                     //!is always joinable
  ERROR_TYPE
      start_error; //!< Used to carry starting error from ctor to start method
  PROCESS_state state;
  MUTEX mutex_op_critical; //!< Critical section mutex. Any change to the state
                           //!must first aquire this mutex. This will happen in
                           //!the setState method.
  SEMAPHORE sem_state;     //!< State semaphore
  // MUTEX notificationMUTEX; //< Used by APC (Arinc Partition Controller) to
  // get notified about the completion of this process
  // COND_VAR notificationCOND_VAR; //< Used by APC (Arinc Partition Controller)
  // to get notified about the completion of this process
  ...
  //@{
  PROCESS(PROCESS_ATTRIBUTE_TYPE &attr,
          PROCESS_ID_TYPE unique_process_id); //!< PROCESS_attribute has to be
  ...
  */
  PROCESS_ID_TYPE get_process_id() { return this->process_id; }
  ...
  */
  ...
  /*!
  *\code
  After Calling stop you must decide if you want to call start again on the
  process
  \endcode
  */
  ...
  */
  static void create_key(Thread_key &key, void (*__cleanup)(void *) = 0);
  ...
  */
  static void delete_key(Thread_key &key);
  /*!
  ...
  */
  static void *get_specific(const Thread_key &key);
  /*!
  ...
  */
  static void set_specific(const Thread_key &key, void *value);
  ...
  */
  static pid_t tid() { return syscall(SYS_gettid); }
  ...
  */
  inline ThreadID thread_id() const { return id; }
  ...
  */
  ...
  /*!
  * This method is used to compare if the PROCESS is equivalent i.e. its thread
  * id is same to the other PROCESS
  */
  virtual bool operator==(const PROCESS &aep) const;

  /*! \name Methods for running and creating threads
  * This Method is used to pass an address of a function pointer to the
  * pthread_create call
  *
  */
  ...
  */
  ...
  inline bool isPeriodic() { return (attribute.PERIOD != INFINITE_TIME_VALUE); }
