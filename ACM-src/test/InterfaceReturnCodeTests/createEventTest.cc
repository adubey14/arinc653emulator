...
void PROCESS_APERIODIC(void) {
  ...
  USE_INFINITE_LOOP_PROTECTION; // Always put this statement in infinite loops.
                                // It creates a thread cancellation point which
                                // is required in order to stop the process if
                                // necessary.
  CREATE_EVENT(EVENT_NAME, &APERIODIC_EVENT_ID, &APERIODIC_RETURN);
  // Tests that invalid mode is returned when attempting to create an event in
  // the normal partition state
