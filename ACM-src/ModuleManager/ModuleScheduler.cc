...
void MODULE_MANAGER::SET_FIFO_POLICY() {
  ...
  if (policy != ACM_SCHED_POLICY) {
    ...
    if (schedRetValue == -1) {
      ...
      // should log this error and throw.
...
int MODULE_MANAGER::RUN_SYSTEM_MODULE_MANAGER() {
  ...
  while ((this->isMAX_NUMBER_OF_FRAMES_SET &&
          NumberofIterations < this->MAX_NUMBER_OF_FRAMES) ||
         (!this->isMAX_NUMBER_OF_FRAMES_SET)) {
    ...
    // SYSTEM_TIMESPEC
    // NEXT_HYPER_PERIOD=addTimeNano(START_HYPER_PERIOD,this->MAJOR_FRAME_SIZE);
    for (unsigned int i = 0; i < SCHEDULING_VECTOR.size(); i++) {
      SYSTEM_TIMESPEC ABSOLUTE_OFFSET_TIME =
          addTimeNano(START_HYPER_PERIOD, SCHEDULING_VECTOR[i].OFFSET);
      SYSTEM_TIME_TYPE CRITICAL_INSTANCE =
          SCHEDULING_VECTOR[i].OFFSET + SCHEDULING_VECTOR[i].DURATION;
      SYSTEM_TIMESPEC ABSOLUTE_DURATION_TIME =
          addTimeNano(START_HYPER_PERIOD, CRITICAL_INSTANCE);
      int clockresult = 1;
      // you can spuriously get out of nanosleep
      struct timespec rem = {0, 0};
      while (clockresult != 0) {
        clockresult = clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME,
                                      &ABSOLUTE_OFFSET_TIME, &rem);
      }
      RUN_PARTITION(SCHEDULING_VECTOR[i].PARTITION_INDEX,
                    ABSOLUTE_DURATION_TIME);
    }
    LOGGER(INFO, "Waiting for next Hyperperiod");
    // SYSTEM_TIME_TYPE
    // FRAME_OFFSET=this->MAJOR_FRAME_SIZE*(SYSTEM_TIME_TYPE)(NumberofIterations);
    START_HYPER_PERIOD = START_HYPER_PERIOD + this->MAJOR_FRAME_SIZE;
    struct timespec rem = {0, 0};
    int clockresult = 1;
    while (clockresult != 0) {
      clockresult = clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME,
                                    &START_HYPER_PERIOD, &rem);
    }
  }
...
int MODULE_MANAGER::RUN_MODULE_MANAGER() {
  ...
  GET_READY_MM_FOR_HYPERPERIOD(); // This will be Used for Hyper period
                                  // synchronization...

  // HELPER_CLOCK_NANOSLEEP(convertSecsToNano(1)); //Ensure all have reached
  // this point

  if ((!this->isSystemModuleManager) &&
      (!this->isrunningUnderSystemModuleManager))
...
void MODULE_MANAGER::CREATE_DUMMY_PROCESS(void) {
  ...
  if (newPid == -1) {
    ...
    if (newPid == 0) {
      ...
      int schedRetValue =
          sched_setscheduler(0, ACM_SCHED_POLICY, &sp_fakeDummy);
      if (schedRetValue == -1) {
        // This failure happens when the user is not a root.	//The framework
