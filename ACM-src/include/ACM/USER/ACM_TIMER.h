...
struct SYSTEM_TIMESPEC_COMP
    : public std::binary_function<SYSTEM_TIMESPEC, SYSTEM_TIMESPEC, bool> {
public:
...
class ACM_TIMER {
  ...
  enum TIMER_STATE { WORKING, STOPPED };

private:
  std::string name;
  ACM_INTERNAL::APEX_EVENT_TYPE
      internalevent_; // Intial state of This is locked;
