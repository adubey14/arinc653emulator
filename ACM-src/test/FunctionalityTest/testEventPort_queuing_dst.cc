...
void runTest()
{
	...
	RETURN_CODE_TYPE RETURN_CODE;
		...
		READ_EV_PORT->READ_FROM_EVENT_PORT((APEX_BYTE*)readData,&length, &isValid,INFINITE_TIME_VALUE, &RETURN_CODE);
		LOGGER(APP,"Return code from READ_EV_PORTING FROM EMPTY EVENTPORT - BBOARD is %d",RETURN_CODE);
		assert(RETURN_CODE==NO_ERROR);		
...
PROCESS_ID_TYPE APERIODIC_PROCESS_ID = APEX_HELPER_CREATE_PROCESS(INFINITE_TIME_VALUE, //SYSTEM_TIME_TYPE PERIOD,
...
PROCESS_ID_TYPE APERIODIC_PROCESS_ID2 = APEX_HELPER_CREATE_PROCESS(INFINITE_TIME_VALUE, //SYSTEM_TIME_TYPE PERIOD,
                           INFINITE_TIME_VALUE, //SYSTEM_TIME_TYPE TIME_CAPACITY,
