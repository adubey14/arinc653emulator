...
void MODULE_MANAGER::INIT_PARTITION_PROPERTY_MAP(std::string filename) {
  ...
  for (std::map<std::string, std::string>::iterator iter = PropertyMap.begin();
       iter != PropertyMap.end(); iter++) {
...
void MODULE_MANAGER::READ_NUMBER_OF_INIT_STAGES() {
  ...
  if (searchPair.first == searchPair.second) {
    ...
    sprintf(bufstr, " %s not set. Will  use 1",
  ...
  stringTrim(stages, " ");
  if (!from_string<unsigned int>(this->NumberOfInitStages, stages, std::dec) ||
      this->NumberOfInitStages > 2) {
    LOGGER(FATAL, "Incorrect%s %s. Legal values =1 or 2",
           PRIMARY_KEY_NUMBER_OF_INITIAL_STAGES, stages.c_str());
    throw(-2);
  }
}
void MODULE_MANAGER::CREATE_SAMPLING_CHANNELS() {
  RETURN_CODE_TYPE RETURN_CODE;
  for (unsigned int counter = 0; counter < this->PARTITION_VECTOR.size();
       counter++) {
    for (unsigned int k = 0;
         k < this->PARTITION_VECTOR[counter].SAMPLING_PORTS.size(); k++) {
      if (this->PARTITION_VECTOR[counter].SAMPLING_PORTS[k].PORT_DIRECTION ==
          ACM_USER::DESTINATION)
        continue;
      CHANNEL_MAP_TYPE::iterator source_iterator;
      for (CHANNEL_MAP_TYPE::iterator iter = CHANNEL_MAP.begin();
           iter != CHANNEL_MAP.end(); iter++) {
        if (iter->second.source.compare(
                this->PARTITION_VECTOR[counter].SAMPLING_PORTS[k].Name) == 0) {
          // SAMPLING_CHANNEL * channel = new SAMPLING_CHANNEL(iter->first);
          // //channel name
          if (CanAddChannel(iter->first)) {
            int currentsize = this->PARTITION_VECTOR[counter]
                                  .SAMPLING_CHANNEL_SOURCE_VECTOR.size();
            ...
            temp_channel->ADD_SOURCE(
                this->PARTITION_VECTOR[counter].SAMPLING_PORTS[k].Name,
                this->PARTITION_VECTOR[counter]
                    .SAMPLING_PORTS[k]
                    .MAX_MESSAGE_SIZE,
                this->PARTITION_VECTOR[counter]
                    .SAMPLING_PORTS[k]
            ...
            LOGGER(INFO, "Destiantion size %d",
                   iter->second.destination.size());
            for (unsigned int i = 0; i < iter->second.destination.size(); i++) {
              ...
              int j = FIND_PARTITION_FROM_SAMPLING_PORT(destination_name,
                                                        descriptor);
              LOGGER(INFO, "output of FIND_PARTITION_FROM_SAMPLING_PORT %d", j);
              if (j == -1) {

                // check if name exists in a proxy_port and the direction of
                // proxy port is destination...

                for (; sizevec < this->PROXY_PORTS.size(); sizevec++) {
                  if (PROXY_PORTS[sizevec].Name.compare(destination_name) ==
                      0) {
                    foundinproxy = true;
                    break;
                  }
                ...
                if (!foundinproxy) {
                  LOGGER(FATAL, "Problem creating channel with destination %s",
                         destination_name.c_str());
                  throw(-1);
                } else {
                  if (PROXY_PORTS[sizevec].PORT_DIRECTION !=
                      ACM_USER::DESTINATION) {
                    LOGGER(FATAL,
                           "Problem creating channel with destination %s",
                           destination_name.c_str());
              ...
              if (foundinproxy) {
                RETURN_CODE_TYPE return_code;
                if (temp_channel->GET_COMMUNICATION_TYPE() !=
                    ACM_INTERNAL::INTER_MODULE_SRC) {
                  temp_channel->SET_COMMUNICATION_TYPE_ENUM(
                      ACM_INTERNAL::INTER_MODULE_SRC, &RETURN_CODE);
                  ENSURE_CONDITION(RETURN_CODE == NO_ERROR);
                }
                temp_channel->ADD_DESTINATION_URL(
                    PROXY_PORTS[sizevec].hostName,
                    PROXY_PORTS[sizevec].portNumber, &return_code);
                ENSURE_CONDITION(RETURN_CODE == NO_ERROR);
              } else if (j >= 0) {
                ...
                descriptor.printout();
                temp_channel->ADD_DESTINATION(
                    descriptor.Name, descriptor.MAX_MESSAGE_SIZE,
                    descriptor.REFRESH_PERIOD, &RETURN_CODE);
                ENSURE_CONDITION(RETURN_CODE == NO_ERROR ||
                                 RETURN_CODE == NO_ACTION);
              }
            }
            ...
            this->PARTITION_VECTOR[counter]
                .SAMPLING_CHANNEL_SOURCE_VECTOR.push_back(
                    temp_channel->GET_NAME());
            this->MAP_OF_SAMPLING_CHANNELS[temp_channel->GET_NAME()] =
                temp_channel;
  ...
  for (; sizevec < this->PROXY_PORTS.size(); sizevec++) {
    if (PROXY_PORTS[sizevec].PORT_DIRECTION == ACM_USER::DESTINATION ||
        PROXY_PORTS[sizevec].PROXY_TYPE == PROXY_PORT_DESCRIPTORS::QUEUEING)
      continue;
    ...
    for (CHANNEL_MAP_TYPE::iterator iter = CHANNEL_MAP.begin();
      ...
      if (iter->second.source.compare(this->PROXY_PORTS[sizevec].Name) == 0) {
        if (CanAddChannel(iter->first)) {
          ...
          SAMPLING_CHANNEL *channel = new SAMPLING_CHANNEL(
              std::string(iter->first), INTER_MODULE_DST,
              PROXY_PORTS[sizevec].portNumber); // channel name
          channel->SET_MAX_SIZE_OF_MESSAGE(
              PROXY_PORTS[sizevec].MAX_MESSAGE_SIZE, &RETURN_CODE);
          channel->SET_MAX_SOCKET_MESSAGE(PROXY_PORTS[sizevec].MAX_NB_MESSAGE,
                                          &RETURN_CODE);
          ENSURE_CONDITION(RETURN_CODE == NO_ERROR);
          ...
          for (unsigned int i = 0; i < iter->second.destination.size(); i++) {
            ...
            int j =
                FIND_PARTITION_FROM_SAMPLING_PORT(destination_name, descriptor);
            if (j == -1) // check if name exists in a proxy_port
            {
              // check if name exists in a proxy_port and the direction of proxy
              // port is destination...
              LOGGER(FATAL, "Problem creating channel with destination %s",
                     destination_name.c_str());
              throw(-1);
            ...
            if (j >= 0) {
              ...
              channel->ADD_DESTINATION(descriptor.Name,
                                       descriptor.MAX_MESSAGE_SIZE,
                                       descriptor.REFRESH_PERIOD, &RETURN_CODE);

              bool alreadyInpartition = false;
              for (unsigned int counter = 0;
                   counter <
                   this->PARTITION_VECTOR[j]
                       .SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR.size();
                   counter++) {
                if (this->PARTITION_VECTOR[j]
                        .SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR[counter]
                        .compare(channel->GET_NAME()) == 0) {
              ...
              if (!alreadyInpartition)
                this->PARTITION_VECTOR[j]
                    .SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR.push_back(
                        channel->GET_NAME());
...
bool MODULE_MANAGER::CanAddChannel(std::string name) {
  ...
  for (unsigned int counter = 0; counter < this->PARTITION_VECTOR.size();
       counter++) {
    if (!(this->PARTITION_VECTOR[counter].isNetworkPartition))
      continue;
    ...
    if (net_partition->name.compare(this->PARTITION_VECTOR[counter].Name) != 0)
      continue;

    if (this->PARTITION_VECTOR[counter].CheckIfChannelExists(
            name)) // exists in  a network partition
    {
      LOGGER(INFO, "Can add Channel %s in %s", name.c_str(),
             this->PARTITION_VECTOR[counter].Name.c_str());
      return true;
    }
  }
  ...
  for (unsigned int counter = 0; counter < this->PARTITION_VECTOR.size();
       counter++) {
    LOGGER(INFO, "Checking in Partition %s",
           this->PARTITION_VECTOR[counter].Name.c_str());
...
void MODULE_MANAGER::CREATE_QUEUEING_CHANNELS() {
  ...
  for (unsigned int counter = 0; counter < this->PARTITION_VECTOR.size();
       counter++) {
    for (unsigned int k = 0;
         k < this->PARTITION_VECTOR[counter].QUEUING_PORTS.size(); k++) {
      if (this->PARTITION_VECTOR[counter].QUEUING_PORTS[k].PORT_DIRECTION ==
          ACM_USER::DESTINATION)
        continue;
      CHANNEL_MAP_TYPE::iterator source_iterator;
      for (CHANNEL_MAP_TYPE::iterator iter = CHANNEL_MAP.begin();
           iter != CHANNEL_MAP.end(); iter++) {
        if (iter->second.source.compare(
                this->PARTITION_VECTOR[counter].QUEUING_PORTS[k].Name) == 0) {
          if (CanAddChannel(iter->first)) {
            QUEUE_CHANNEL *channel = new QUEUE_CHANNEL(); // channel name
            int currentsize = this->PARTITION_VECTOR[counter]
                                  .QUEUE_CHANNEL_SOURCE_VECTOR.size();
            LOGGER(INFO,
                   "Partition %s, QUEUE CHANNEL NUMBER %d. Channel name %s ",
                   this->PARTITION_VECTOR[counter].Name.c_str(), currentsize,
                   iter->first.c_str());
            ...
            channel->ADD_SOURCE_QUEUE(
                this->PARTITION_VECTOR[counter].QUEUING_PORTS[k].Name,
                this->PARTITION_VECTOR[counter]
                    .QUEUING_PORTS[k]
                    .MAX_MESSAGE_SIZE,
            ...
            for (unsigned int i = 0; i < iter->second.destination.size(); i++) {
              ...
              unsigned int sizevec = 0;
              bool foundinproxy = false;
              int j = FIND_PARTITION_FROM_QUEUING_PORT(destination_name,
                                                       descriptor);
              if (j == -1) // check if name exists in a proxy_port
              {
              ...
              if (foundinproxy) {
                RETURN_CODE_TYPE return_code;
                if (channel->GET_COMMUNICATION_TYPE() !=
                    ACM_INTERNAL::INTER_MODULE_SRC) {
                  channel->SET_COMMUNICATION_TYPE_ENUM(
                      ACM_INTERNAL::INTER_MODULE_SRC, &RETURN_CODE);
                  ENSURE_CONDITION(RETURN_CODE == NO_ERROR);
                }
                channel->ADD_DESTINATION_URL(PROXY_PORTS[sizevec].hostName,
                                             PROXY_PORTS[sizevec].portNumber,
                                             &return_code);
                ENSURE_CONDITION(RETURN_CODE == NO_ERROR);
              } else if (j >= 0) {
                if (descriptor.PORT_DIRECTION != ACM_USER::DESTINATION) {
                ...
                channel->ADD_DESTINATION_QUEUE(
                    descriptor.Name, descriptor.MAX_MESSAGE_SIZE,
                    descriptor.MAX_NB_MESSAGE, &RETURN_CODE);
                ENSURE_CONDITION(RETURN_CODE == NO_ERROR);
              }
            }
            // create the channel
            ...
            this->PARTITION_VECTOR[counter]
                .QUEUE_CHANNEL_SOURCE_VECTOR.push_back(channel->GET_NAME());
  ...
  for (; sizevec < this->PROXY_PORTS.size(); sizevec++) {
    if (PROXY_PORTS[sizevec].PORT_DIRECTION == ACM_USER::DESTINATION ||
        PROXY_PORTS[sizevec].PROXY_TYPE == PROXY_PORT_DESCRIPTORS::SAMPLING)
      continue;
    ...
    for (CHANNEL_MAP_TYPE::iterator iter = CHANNEL_MAP.begin();
      ...
      if (iter->second.source.compare(this->PROXY_PORTS[sizevec].Name) == 0) {
        ...
        if (CanAddChannel(iter->first)) {
          ...
          QUEUE_CHANNEL *channel = new QUEUE_CHANNEL(
              std::string(iter->first), INTER_MODULE_DST,
              PROXY_PORTS[sizevec].portNumber); // channel name
          ...
          for (unsigned int i = 0; i < iter->second.destination.size(); i++) {
            ...
            int j =
                FIND_PARTITION_FROM_QUEUING_PORT(destination_name, descriptor);
            if (j == -1) // check if name exists in a proxy_port
            {
              // check if name exists in a proxy_port and the direction of proxy
              // port is destination...
            ...
            if (j >= 0) {
              ...
              bool alreadyInpartition = false;
              for (unsigned int counter = 0;
                   counter < this->PARTITION_VECTOR[j]
                                 .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR.size();
                   counter++) {

                if (this->PARTITION_VECTOR[j]
                        .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[counter]
                        .compare(channel->GET_NAME()) == 0) {
                  alreadyInpartition = true;
                  break;
                }
              ...
              if (!alreadyInpartition)
                this->PARTITION_VECTOR[j]
                    .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR.push_back(
                        channel->GET_NAME());
              ENSURE_CONDITION(RETURN_CODE == NO_ERROR);
            }
          }
...
void MODULE_MANAGER::SET_MAX_NUMBER_OF_FRAMES() {
  ...
  if (!from_string<int>(this->MAX_NUMBER_OF_FRAMES, maxiterationstring,
                        std::dec)) {
    LOGGER(FATAL, "Incorrect MAXITERATIONS VALUE %s",
           maxiterationstring.c_str());
    throw(-2);
  }
...
void MODULE_MANAGER::SET_MIN_WAIT_TIME() {
  ...
  if (searchPair.first == searchPair.second) {
    char bufstr[100];
    sprintf(bufstr, "No PARTITION_INIT_TIMEOUT Limit. Will wait for  %d",
            this->MIN_WAIT_TIME);
    LOGGER(INFO, bufstr);
    return;
  }
  ...
  if (!from_string<int>(this->MIN_WAIT_TIME, minwaittimestring, std::dec)) {
    LOGGER(FATAL, "Incorrect PARTITION_INIT_TIMEOUT VALUE %s",
           minwaittimestring.c_str());
    throw(-2);
  }
...
void MODULE_MANAGER::SET_DISCOVERY_PROTOCOL() {
  ...
  if (searchPair.first == searchPair.second) {
    ...
    sprintf(bufstr, "No %s set. Will  use %s", PRIMARY_KEY_DISCOVERY_PROTOCOL,
  ...
  if (discoveryprotocol.compare(PRIMARY_KEY_DISCOVERY_PROTOCOL_IOR) == 0) {
    sprintf(bufstr, "Setting %s to %s", PRIMARY_KEY_DISCOVERY_PROTOCOL,
            PRIMARY_KEY_DISCOVERY_PROTOCOL_IOR);
    LOGGER(INFO, bufstr);
    DISCOVERY_PROTOCOL = IOR;
    return;
  } else if (discoveryprotocol.compare(PRIMARY_KEY_DISCOVERY_PROTOCOL_NS) ==
             0) {
    ...
    if (searchPair1.first == searchPair1.second) {
      char bufstr[100];
      sprintf(bufstr, "You have set %s to %s but not provided value for %s",
              PRIMARY_KEY_DISCOVERY_PROTOCOL, PRIMARY_KEY_DISCOVERY_PROTOCOL_NS,
      ...
      LOGGER(INFO, "Setting %s to %s", PRIMARY_KEY_DISCOVERY_PROTOCOL_NS_HOST,
             this->NS_HOST_IP.c_str());
    }
    ...
    if (searchPair1.first == searchPair1.second) {
      ...
      if (!from_string<int>(this->NS_PORT_NUMBER, port, std::dec)) {
        LOGGER(FATAL, "Incorrect%s %s", PRIMARY_KEY_DISCOVERY_PROTOCOL_NS_PORT,
               port.c_str());
        throw(-2);
      }
      ...
      LOGGER(INFO, "Setting %s to %d", PRIMARY_KEY_DISCOVERY_PROTOCOL_NS_PORT,
             this->NS_PORT_NUMBER);
    ...
    LOGGER(FATAL, "Wrong choice %s for %s. Correct choices are %s and %s",
           discoveryprotocol.c_str(), PRIMARY_KEY_DISCOVERY_PROTOCOL,
           PRIMARY_KEY_DISCOVERY_PROTOCOL_NS,
           PRIMARY_KEY_DISCOVERY_PROTOCOL_IOR);
    throw(-3);
  }
...
void MODULE_MANAGER::CREATE_PROXY_PORT_DESCRIPTORS() {
  ...
  for (PropertyMapIteratorType iter = searchPair.first;
       iter != searchPair.second; iter++) {
    ...
    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        ModuleName_PROPERTY_SEARCH;
    ...
    if (proxytypetmp.compare(PROXY_TYPE_SAMPLING) == 0) {
      ...
      LOGGER(FATAL, "Cannot resolve %s=%s", proxytypestr.c_str(),
             proxytypetmp.c_str());
      throw(-3);
    }

    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        Portnum_PROPERTY_SEARCH;
    ...
    if (!from_string<unsigned int>(PROXY_PORT_DESCRIPTOR.portNumber,
                                   Portnumtemp, std::dec)) {
      LOGGER(FATAL, "Incorrect Port number %s for proxyPort %s",
             Portnumtemp.c_str(), PROXY_NAME.c_str());
      throw(-3);
    }

    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        messagemaxsize_PROPERTY_SEARCH;
    messagemaxsize_PROPERTY_SEARCH =
        this->PropertyMap.equal_range(messagemaxsize);
    if (messagemaxsize_PROPERTY_SEARCH.first ==
        messagemaxsize_PROPERTY_SEARCH.second) {
      LOGGER(FATAL, "Cannot find %s", messagemaxsize.c_str());
      throw(-3);
    }

    messagemaxsize = messagemaxsize_PROPERTY_SEARCH.first->second;
    if (!from_string<MESSAGE_SIZE_TYPE>(PROXY_PORT_DESCRIPTOR.MAX_MESSAGE_SIZE,
                                        messagemaxsize, std::dec)) {
      LOGGER(FATAL, "Incorrect max message size %s for proxyPort %s",
             messagemaxsize.c_str(), PROXY_NAME.c_str());
      throw(-3);
    }
    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        messagemax_num_PROPERTY_SEARCH;
    messagemax_num_PROPERTY_SEARCH =
        this->PropertyMap.equal_range(messagemax_num);
    if (messagemax_num_PROPERTY_SEARCH.first ==
        messagemax_num_PROPERTY_SEARCH.second) {
      LOGGER(FATAL, "Cannot find %s", messagemax_num.c_str());
      throw(-3);
    }

    messagemax_num = messagemax_num_PROPERTY_SEARCH.first->second;
    if (!from_string<MESSAGE_RANGE_TYPE>(PROXY_PORT_DESCRIPTOR.MAX_NB_MESSAGE,
                                         messagemax_num, std::dec)) {
      LOGGER(FATAL, "Incorrect max message size %s for proxyPort %s",
             messagemax_num.c_str(), PROXY_NAME.c_str());
      throw(-3);
    }

    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        Direction_PROPERTIES_SEARCH_PAIR;
    Direction_PROPERTIES_SEARCH_PAIR =
        this->PropertyMap.equal_range(DIRECTION_String);
    if (Direction_PROPERTIES_SEARCH_PAIR.first ==
        Direction_PROPERTIES_SEARCH_PAIR.second) {
    ...
    if (DIRECTION_data.compare("SOURCE") == 0) {
      ...
      LOGGER(ERROR, "WRONG TOken %s in proxy port %s_DIRECTION",
             DIRECTION_data.c_str(), PROXY_NAME.c_str());
      throw(-3);
    }
...
void MODULE_MANAGER::CREATE_CHANNEL_DSCRIPTOR() {
  ...
  for (PropertyMapIteratorType iter = searchPair.first;
    ...
    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        SOURCE_STRING_PROPERTY_SEARCH;
    SOURCE_STRING_PROPERTY_SEARCH =
        this->PropertyMap.equal_range(SOURCE_STRING);

    if (SOURCE_STRING_PROPERTY_SEARCH.first ==
        SOURCE_STRING_PROPERTY_SEARCH.second) {
    ...
    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        PORT_PROPERTIES_SEARCH_PAIR =
            this->PropertyMap.equal_range(DIRECTION_String);

    if (PORT_PROPERTIES_SEARCH_PAIR.first ==
        PORT_PROPERTIES_SEARCH_PAIR.second) {
      LOGGER(FATAL, "Cannot find %s", DIRECTION_String.c_str());
      throw(-3);
    }
    ...
    if (DIRECTION_data.compare("SOURCE") != 0) {
      LOGGER(FATAL, "The port configured for %s should have SOURCE DIRECTION",
             SOURCE_STRING.c_str());
      throw(-3);
    }
    ...
    SOURCE_STRING_PROPERTY_SEARCH =
        this->PropertyMap.equal_range(DESTINATION_STRING);
    if (SOURCE_STRING_PROPERTY_SEARCH.first ==
        SOURCE_STRING_PROPERTY_SEARCH.second) {
    ...
    for (PropertyMapIteratorType iter_inner =
             SOURCE_STRING_PROPERTY_SEARCH.first;
         iter_inner != SOURCE_STRING_PROPERTY_SEARCH.second; iter_inner++) {
      ...
      if (DIRECTION_data.compare("DESTINATION") != 0) {
        LOGGER(
            FATAL,
            "The port %s configured for %s should have DESTINATION DIRECTION",
            temp.c_str(), DESTINATION_STRING.c_str());
        throw(-3);
      }
    }
  ...
  for (CHANNEL_MAP_TYPE::iterator iter = CHANNEL_MAP.begin();
       iter != CHANNEL_MAP.end(); iter++) {
    for (unsigned int i = 0; i < iter->second.destination.size(); i++) {
      LOGGER(INFO, "Channel %s, SourcePort:%s, DestinationPort:%s",
             iter->first.c_str(), iter->second.source.c_str(),
             iter->second.destination[i].c_str());
    }
  }
}
void MODULE_MANAGER::CREATE_NETWORK_PARTITION_VECTOR() {
  std::pair<PropertyMapIteratorType, PropertyMapIteratorType> searchPair;
  searchPair =
      this->PropertyMap.equal_range(PRIMARY_KEY_NETWORK_PARTITION_NAME);
  ...
  for (PropertyMapIteratorType iter = searchPair.first;
    ...
    snprintf(QUEUING_PORT_NAME, MAX_NAME_LENGTH, "%s_config",
             this->PARTITION_VECTOR[tempPartitionIndex].Name.c_str());

    MESSAGE_QUEUE_TYPE *temp_queue = new MESSAGE_QUEUE_TYPE(
        QUEUING_PORT_NAME, sizeof(NETWORK_PARTITION_PETRI_NET_TOKEN_TYPE), 10,
        PORT_SOURCE_DEST, &queuing_return_code, O_CREAT | O_EXCL);
    if (queuing_return_code != NO_ERROR) {
      LOGGER(FATAL, "Error in Creating Queue %s. Please clean /dev/mqeueue",
             QUEUING_PORT_NAME);
      LOGGER(FATAL,
             "To Mount: mkdir /dev/mqueue; mount -t mqueue none /dev/mqueue");
      throw(-3);
    }
    ENSURE_CONDITION(queuing_return_code == NO_ERROR);

    this->PARTITION_VECTOR[tempPartitionIndex].CONFIGURATION_QUEUE =
        QUEUING_PORT_NAME;
    this->MAP_OF_CONFIGURATION_QUEUE[this->PARTITION_VECTOR[tempPartitionIndex]
                                         .CONFIGURATION_QUEUE] = temp_queue;
#endif

    LOGGER(INFO, "**********Created %s", iter->second.c_str());
    // Channels;

    std::string channel_name_search_string =
        this->PARTITION_VECTOR[tempPartitionIndex].Name;
    channel_name_search_string.append(ADDITVE_KEY_CHANNEL);
    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        CHANNEL_SEARCH_PAIR;
    CHANNEL_SEARCH_PAIR =
        this->PropertyMap.equal_range(channel_name_search_string);

    for (PropertyMapIteratorType PropertyMapIterator =
             CHANNEL_SEARCH_PAIR.first;
         PropertyMapIterator != CHANNEL_SEARCH_PAIR.second;
         PropertyMapIterator++) {
      ...
      std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
          search_port_string_SEARCH_PAIR;
      std::string search_port_string = source_port + ADDITIVE_KEY__TYPE;
      search_port_string_SEARCH_PAIR =
          this->PropertyMap.equal_range(search_port_string);

      if (search_port_string_SEARCH_PAIR.first ==
          search_port_string_SEARCH_PAIR.second) {
        ...
        if (search_port_string_SEARCH_PAIR.first ==
            search_port_string_SEARCH_PAIR.second) {
          this->PARTITION_VECTOR[tempPartitionIndex]
              .QUEUE_CHANNEL_SOURCE_VECTOR.push_back(channel_name);
        } else {

          this->PARTITION_VECTOR[tempPartitionIndex]
              .SAMPLING_CHANNEL_SOURCE_VECTOR.push_back(channel_name);
        }
      } else {
        ...
        if (value.compare(PROXY_TYPE_SAMPLING) == 0) {
          ...
          LOGGER(ERROR, "Incorrect %s=%s", search_port_string.c_str(),
                 value.c_str());
          throw(-3);
        }
      }
    ...
    // SCHEDULE
    std::string sched_name_string =
        this->PARTITION_VECTOR[tempPartitionIndex].Name;
    sched_name_string.append(ADDITIVE_KEY__SCHEDULE);
    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        SCHEDULE_SEARCH_PAIR;
    ...
    for (PropertyMapIteratorType PropertyMapIterator =
             SCHEDULE_SEARCH_PAIR.first;
         PropertyMapIterator != SCHEDULE_SEARCH_PAIR.second;
         PropertyMapIterator++) {
      ...
      if (tokens.size() != 2) {
        LOGGER(FATAL, "Problem with %s=%s", sched_name_string.c_str(),
      ...
      LOGGER(DEBUG, "TUPLE OFFSET is %lld, TUPLE DURATION is DURATION %lld",
             tuple.OFFSET, tuple.DURATION);
    ...
    this->PARTITION_VECTOR[tempPartitionIndex].Executable.append(
        "//bin//NetworkPartition");
...
void MODULE_MANAGER::CREATE_PARTITION_VECTOR() {
  ...
  for (PropertyMapIteratorType iter = searchPair.first;
    ...
    this->PARTITION_VECTOR.push_back(temporaryPartition);
    this->PARTITION_VECTOR[tempPartitionIndex].Name = iter->second;
    std::string exec_name_string =
        this->PARTITION_VECTOR[tempPartitionIndex].Name;
    exec_name_string.append(ADDITIVE_KEY__EXECUTABLE);
    PropertyMapIteratorType PropertyMapIterator =
        this->PropertyMap.find(exec_name_string);
    if (PropertyMapIterator == this->PropertyMap.end()) {
      LOGGER(FATAL, "Cannot find %s", exec_name_string.c_str());
      throw(-3);
    }
    this->PARTITION_VECTOR[tempPartitionIndex].Executable =
        PropertyMapIterator->second;

    // Check if the partition Executable exists and is readable.

    if (!fexists(
            this->PARTITION_VECTOR[tempPartitionIndex].Executable.c_str())) {
      LOGGER(FATAL, "Cannot access %s",
             this->PARTITION_VECTOR[tempPartitionIndex].Executable.c_str());
      throw(-3);
    }
    ...
    for (PropertyMapIterator = SCHEDULE_SEARCH_PAIR.first;
         PropertyMapIterator != SCHEDULE_SEARCH_PAIR.second;
         PropertyMapIterator++) {
      if (PropertyMapIterator == this->PropertyMap.end()) {
        LOGGER(FATAL, "Cannot find %s", sched_name_string.c_str());
      ...
      LOGGER(DEBUG,
             "Before Sending to Convert to NanoSec OFFSET Value is %4.9f",
             OFFSET);
      tuple.OFFSET = convertSecsToNano(OFFSET);
      ...
      LOGGER(DEBUG, "Read Duration Value is %4.9f", DURATION);
      tuple.DURATION = convertSecsToNano(DURATION);
      LOGGER(INFO, "TUPLE OFFSET is %lld, TUPLE DURATION is DURATION %lld",
             tuple.OFFSET, tuple.DURATION);
      tuple.PARTITION_INDEX = tempPartitionIndex;
      this->SCHEDULING_VECTOR.push_back(tuple);
    }
    //////////////////////////////////////SAMPLING_PORT////////////////////////////
    std::string sampling_string =
        this->PARTITION_VECTOR[tempPartitionIndex].Name;
    sampling_string.append(ADDITIVE_KEY__SAMPLINGPORT);
    std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
        SAMPLING_SEARCH_PAIR;
    SAMPLING_SEARCH_PAIR = this->PropertyMap.equal_range(sampling_string);
    // LOGGER(INFO,"sampling_string is %s",sampling_string.c_str());
    // exit(-1);
    for (PropertyMapIteratorType iter = SAMPLING_SEARCH_PAIR.first;
         iter != SAMPLING_SEARCH_PAIR.second; iter++) {
      ...
      std::string MAXMESSAGESIZE_String =
          sampling_port_name + ADDITIVE_KEY__MAXMESSAGESIZE;
      std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
          SAMPLING_PROPERTIES_SEARCH_PAIR;
      SAMPLING_PROPERTIES_SEARCH_PAIR =
          this->PropertyMap.equal_range(MAXMESSAGESIZE_String);
      if (SAMPLING_PROPERTIES_SEARCH_PAIR.first ==
          SAMPLING_PROPERTIES_SEARCH_PAIR.second) {
        LOGGER(FATAL, "Cannot find %s", MAXMESSAGESIZE_String.c_str());
        throw(-3);
      }
      std::string max_message_data =
          SAMPLING_PROPERTIES_SEARCH_PAIR.first->second;

      if (!from_string<MESSAGE_SIZE_TYPE>(
              temp_SAMPLING_PORT_DESCRIPTOR.MAX_MESSAGE_SIZE, max_message_data,
              std::dec)) {
        LOGGER(INFO, "ERROR parsing %s to int", max_message_data.c_str());
        throw(-4);
      }

      std::string REFRESHPERIOD_String =
          sampling_port_name + ADDITIVE_KEY__REFRESHPERIOD;
      SAMPLING_PROPERTIES_SEARCH_PAIR =
          this->PropertyMap.equal_range(REFRESHPERIOD_String);
      if (SAMPLING_PROPERTIES_SEARCH_PAIR.first ==
          SAMPLING_PROPERTIES_SEARCH_PAIR.second) {
      ...
      temp_SAMPLING_PORT_DESCRIPTOR.REFRESH_PERIOD =
          convertSecsToNano(refresh_period);
      std::string DIRECTION_String =
          sampling_port_name + ADDITIVE_KEY__DIRECTION;
      SAMPLING_PROPERTIES_SEARCH_PAIR =
          this->PropertyMap.equal_range(DIRECTION_String);
      if (SAMPLING_PROPERTIES_SEARCH_PAIR.first ==
          SAMPLING_PROPERTIES_SEARCH_PAIR.second) {
      ...
      if (DIRECTION_data.compare("SOURCE") == 0) {
        ...
        LOGGER(ERROR, "WRONG TOken %s in sampling port %s_DIRECTION",
               DIRECTION_data.c_str(),
               temp_SAMPLING_PORT_DESCRIPTOR.Name.c_str());
        throw(-4);
      }
...

      {
        ...
        if (killfile.good() && killfile.is_open()) {
          ...
          killfile << "rm -f /dev/shm/" << temp_SAMPLING_PORT_DESCRIPTOR.Name
                   << std::endl;
          killfile.flush();
        }

        SAMPLING_PORT_HELPER tmpPort(
            temp_SAMPLING_PORT_DESCRIPTOR.Name,
            temp_SAMPLING_PORT_DESCRIPTOR.MAX_MESSAGE_SIZE, PORT_SOURCE_DEST,
            temp_SAMPLING_PORT_DESCRIPTOR.REFRESH_PERIOD, &sampling_return_code,
            O_CREAT | O_EXCL);

        if (sampling_return_code != NO_ERROR) {
          LOGGER(FATAL, "Error in creating sampling port %s",
                 temp_SAMPLING_PORT_DESCRIPTOR.Name.c_str());
          throw(-4);
        }
...
O_CREAT);*/
      ...
      this->PARTITION_VECTOR[tempPartitionIndex].SAMPLING_PORTS.push_back(
          temp_SAMPLING_PORT_DESCRIPTOR);
    ...
    for (PropertyMapIteratorType iter = QUEUING_SEARCH_PAIR.first;
         iter != QUEUING_SEARCH_PAIR.second; iter++) {
      ...
      std::string MAXMESSAGESIZE_String =
          queuing_port_name + ADDITIVE_KEY__MAXMESSAGESIZE;
      std::pair<PropertyMapIteratorType, PropertyMapIteratorType>
          QUEUING_PROPERTIES_SEARCH_PAIR;
      QUEUING_PROPERTIES_SEARCH_PAIR =
          this->PropertyMap.equal_range(MAXMESSAGESIZE_String);
      if (QUEUING_PROPERTIES_SEARCH_PAIR.first ==
          QUEUING_PROPERTIES_SEARCH_PAIR.second) {
        LOGGER(FATAL, "Cannot  find %s", MAXMESSAGESIZE_String.c_str());
        throw(-3);
      }
      std::string max_message_data =
          QUEUING_PROPERTIES_SEARCH_PAIR.first->second;

      // std::cout<<max_message_data<<std::endl;

      if (!from_string<MESSAGE_SIZE_TYPE>(
              temp_QUEUING_PORT_DESCRIPTOR.MAX_MESSAGE_SIZE, max_message_data,
              std::dec)) {
        LOGGER(INFO, "ERROR parsing %s to int", max_message_data.c_str());
        throw(-4);
      }
      std::string MAXNUMBEROFMESSAGES_String =
          queuing_port_name + ADDITIVE_KEY__MAXNUMBEROFMESSAGES;

      // std::cout<<"MAXNUMBEROFMESSAGES_String
      // "<<MAXNUMBEROFMESSAGES_String<<std::endl;
      QUEUING_PROPERTIES_SEARCH_PAIR =
          this->PropertyMap.equal_range(MAXNUMBEROFMESSAGES_String);
      ...
      std::string MAXNUMBEROFMESSAGES_String_data =
          QUEUING_PROPERTIES_SEARCH_PAIR.first->second;
      // std::cout<<MAXNUMBEROFMESSAGES_String_data <<"
      // MAXNUMBEROFMESSAGES_String_data" <<std::endl;

      if (!from_string<MESSAGE_RANGE_TYPE>(
              temp_QUEUING_PORT_DESCRIPTOR.MAX_NB_MESSAGE,
              MAXNUMBEROFMESSAGES_String_data, std::dec)) {
        LOGGER(INFO, "ERROR parsing %s to int",
               MAXNUMBEROFMESSAGES_String_data.c_str());
        throw(-4);
      }

      std::string DIRECTION_String =
          queuing_port_name + ADDITIVE_KEY__DIRECTION;
      ...
      if (DIRECTION_data.compare("SOURCE") == 0) {
        ...
        LOGGER(ERROR, "WRONG TOken %s in sampling port %s_DIRECTION",
               DIRECTION_data.c_str(),
               temp_QUEUING_PORT_DESCRIPTOR.Name.c_str());
        throw(-4);
      }

      {
        ...
        if (killfile.good() && killfile.is_open()) {
          killfile << "rm -f /dev/mqueue/" << temp_QUEUING_PORT_DESCRIPTOR.Name
                   << std::endl;
          killfile.flush();
        }
        MESSAGE_QUEUE_TYPE tmpPort(
            temp_QUEUING_PORT_DESCRIPTOR.Name,
            temp_QUEUING_PORT_DESCRIPTOR.MAX_MESSAGE_SIZE,
            temp_QUEUING_PORT_DESCRIPTOR.MAX_NB_MESSAGE, PORT_SOURCE_DEST,
            &queuing_return_code, O_CREAT | O_EXCL);
        if (queuing_return_code != NO_ERROR) {
          LOGGER(FATAL, "Error in Creating Queue %s. Please clean /dev/mqeueue",
                 temp_QUEUING_PORT_DESCRIPTOR.Name.c_str());
          LOGGER(
...
O_CREAT);*/
      ...
      this->PARTITION_VECTOR[tempPartitionIndex].QUEUING_PORTS.push_back(
          temp_QUEUING_PORT_DESCRIPTOR);
    }
  }
  this->CREATE_QUEUEING_CHANNELS();
  this->CREATE_SAMPLING_CHANNELS();
}
int MODULE_MANAGER::FIND_PARTITION_FROM_SAMPLING_PORT(
    std::string &PORT_NAME, SAMPLING_PORT_DESCRIPTOR &descriptor) {
  int return_value = -1;

  for (unsigned int counter = 0; counter < this->PARTITION_VECTOR.size();
       counter++) {
    for (unsigned int i = 0;
         i < this->PARTITION_VECTOR[counter].SAMPLING_PORTS.size(); i++) {
      if (this->PARTITION_VECTOR[counter].SAMPLING_PORTS[i].Name.compare(
              PORT_NAME) == 0) {
        // std::cout<< "          FOUUUUND "<<i<<" "<<PORT_NAME<<std::endl;
        descriptor = this->PARTITION_VECTOR[counter].SAMPLING_PORTS[i];
        // std::cout<<descriptor.Name << descriptor.MAX_MESSAGE_SIZE
        // <<descriptor.PORT_DIRECTION<<descriptor.REFRESH_PERIOD<<std::endl;
        return counter;
      }
    }
  }
...
int MODULE_MANAGER::FIND_PARTITION_FROM_QUEUING_PORT(
    std::string &PORT_NAME, QUEUING_PORT_DESCRIPTOR &descriptor) {
  int return_value = -1;

  for (unsigned int counter = 0; counter < this->PARTITION_VECTOR.size();
       counter++) {
    for (unsigned int i = 0;
         i < this->PARTITION_VECTOR[counter].QUEUING_PORTS.size(); i++) {
      if (this->PARTITION_VECTOR[counter].QUEUING_PORTS[i].Name.compare(
              PORT_NAME) == 0) {
...
void MODULE_MANAGER::SET_CPU_NUMBER() {
  ...
  if (searchPair.first == searchPair.second) {
    LOGGER(INFO, "MODULE WILL SET Its Affinity to Default CPU %d",
           this->cpunumber);
    ...
    LOGGER(INFO, "MODULE WILL SET Its Affinity to Specified CPU %d",
           this->cpunumber);
  }
