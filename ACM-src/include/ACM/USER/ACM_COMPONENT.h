...
namespace ACM_USER {
//! All components used in ACM arinc emulation will inherit from ACMComponent.
//! The functionality of this component is still to be decided.
...
protected:
  ...
  inline void readlock(SYSTEM_TIME_TYPE TIMEOUT,
                       RETURN_CODE_TYPE *RETURN_CODE) {
  ...
  inline void writelock(SYSTEM_TIME_TYPE TIMEOUT,
                        RETURN_CODE_TYPE *RETURN_CODE) {
...
//	private:
//
//		pthread_rwlock_t	rwlock;	//!< The pthread read/write
//lock // A process belonging to a component must obtain either a read lock or
//write lock to the component before running
//		// Any interface defined with const in end will only require a
//read lock to be able to run.
//
//
//	public:
//		ACM_COMPONENT_TYPE(){
...
//			{
//			case EBUSY:
//				LOGGER(INFO,"In ~ACM_COMPONENT_TYPE Destructor: Return
//Code from Destroy is EBUSY");
//				break;
//			case EINVAL:
//				LOGGER(INFO,"In ~ACM_COMPONENT_TYPE Destructor: Return
//Code from Destroy is EINVAL");
//				break;
//			default:
//				break;
//			}
