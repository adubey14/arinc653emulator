...
ACM_EVENT_PORT *createReadEventPort(RETURN_CODE_TYPE *RETURN_CODE) {
  ACM_EVENT_PORT *ep =
      new ACM_EVENT_PORT(READ, BLACKBOARD_NAME, 200,
                         convertSecsToNano(4), // VALIDITY_PERIOD
                         1, RETURN_CODE, true, false);
  assert(*RETURN_CODE == NO_ERROR);
  return ep;
}
...
ACM_EVENT_PORT *createWriteEventPort(RETURN_CODE_TYPE *RETURN_CODE) {
  ACM_EVENT_PORT *ep =
      new ACM_EVENT_PORT(WRITE, BLACKBOARD_NAME, 200,
                         convertSecsToNano(4), // VALIDITY_PERIOD
                         1, RETURN_CODE, true, false);
  assert(*RETURN_CODE == NO_ERROR);
  return ep;
...
void runTest(ACM_EVENT_PORT *read, ACM_EVENT_PORT *write) {
  ...
  try {
    ...
    *RETURN_CODE);
    ...
    LOGGER(APP, "Return code from READING FROM EMPTY EVENTPORT - BBOARD is %d",
           RETURN_CODE);
    ...
    read->READ_FROM_EVENT_PORT((APEX_BYTE *)readData, &length, &isValid,
                               convertSecsToNano(5), &RETURN_CODE);
    LOGGER(APP, "Return code from Re-READING EVENTPORT - BBOARD is %d",
           RETURN_CODE);
    RETURN_CODE = NO_ERROR;
    ...
    int j;
    RETURN_CODE = NO_ERROR;
    write->WRITE_TO_EVENT_PORT((APEX_BYTE *)(&i), sizeof(int),
                               convertSecsToNano(5), &RETURN_CODE);
    read->READ_FROM_EVENT_PORT((APEX_BYTE *)(&j), &length, &isValid,
                               convertSecsToNano(5), &RETURN_CODE);
