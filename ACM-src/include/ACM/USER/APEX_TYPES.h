...
#define PARTITION_MANAGER_PRIORITY                                             \
  sched_get_priority_max(                                                      \
      SCHED_FIFO) //!< General definition for partition MANAGER. Usually 99
#define MODULE_MANAGER_PRIORITY                                                \
  PARTITION_MANAGER_PRIORITY //!< Module Manager is the new name for partition
                             //!manager
#define CPU_CORE_TO_USE                                                        \
  0 //!<By default a partition uses core 0. This can be specifically changed
    //!using module manager configuration file.
#define CONTROLLER_PROCESS_PRIORITY                                            \
  97 //!<This is the highest priority in a partition (Scheduler)
#define PARTITION_PROCESS_HIGHPRIORITY                                         \
  96 //!<All high priority processes run at this level.
#define HMPRIORITY                                                             \
  PARTITION_PROCESS_HIGHPRIORITY //!<All high priority processes run at this
                                 //!level.
#define PARTITION_ORB_PROCESS_HIGH_PRIORITY                                    \
  PARTITION_PROCESS_HIGHPRIORITY //!<All high priority processes run at this
                                 //!level.

#define PARTITION_PROCESS_LOWPRIORITY 83 //!<Lowest priority in a partition
#define DUMMY_PROCESS_PRIORITY                                                 \
  (PARTITION_PROCESS_LOWPRIORITY + 1) //!< priority for the dummy process.
                                      //!Anything less than the dummy process
                                      //!priority will be automaticall
                                      //!suspended.
...
// priority levels are from 0-9 which means effective priority will be from
// 85-94
//#define WORKER_THREAD_BASE_PRIORITY 85

#define WORKER_THREAD_RESTART_PRIORITY                                         \
  95 //!<restart priority level for worker thread.when a worker thread has to be
     //!restarted, it's priority will be raised to this level to boost up the
     //!restart process
...
*
*/
typedef APEX_LONG_INTEGER SYSTEM_TIME_TYPE; //!<SYSTEM_TIME_TYPE is defined
                                            //!using a signed 64-bit value with
...
//	typedef APEX_LONG_INTEGER SYSTEM_TIME_TYPE; //!< 64-bit signed integer
//with a 1 nanosecond LSB
...
enum PROCESS_STATE_TYPE { DORMANT, READY, RUNNING, WAITING };
...
struct PROCESS_ATTRIBUTE_TYPE {
  ...
  SYSTEM_TIME_TYPE
      TIME_CAPACITY; //!< Worst Case Execution Time. Defined in Nanoseconds
  SYSTEM_ADDRESS_TYPE ENTRY_POINT; //!<System address point is the entry
                                   //!function i.e. code to be executed
  STACK_SIZE_TYPE STACK_SIZE;      //!< We do not use it currently
  PRIORITY_TYPE BASE_PRIORITY;     //!< Should be between
                                   //!PARTITION_PROCESS_HIGHPRIORITY and
                                   //!PARTITION_PROCESS_LOWPRIORITY
  DEADLINE_TYPE DEADLINE;          //! Deadline type, can be SOFT or HARD
  PROCESS_NAME_TYPE
      NAME; //! Name of the process. This name should be unique in a partition.

  PROCESS_ATTRIBUTE_TYPE(){
      // constructed
  };
  PROCESS_ATTRIBUTE_TYPE(const PROCESS_ATTRIBUTE_TYPE &copy)
      : PERIOD(copy.PERIOD), TIME_CAPACITY(copy.TIME_CAPACITY),
        ENTRY_POINT(copy.ENTRY_POINT), STACK_SIZE(copy.STACK_SIZE),
        BASE_PRIORITY(copy.BASE_PRIORITY), DEADLINE(copy.DEADLINE)
...
*
*/
enum OPERATING_MODE_TYPE { IDLE, COLD_START, WARM_START, NORMAL };
