...
void PROCESS_APERIODIC (void){
	...
	//Tests that the correct code is returned when the current value is already equal to the max value for that semaphore
	SIGNAL_SEMAPHORE (
		/*!in */ mysemid,
		/*!out*/ &RETURN_CODE );
	...
	//Tests that the correct code is returned when attempting to signal a nonexistent semaphore
