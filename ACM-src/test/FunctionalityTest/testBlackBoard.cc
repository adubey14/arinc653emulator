...
int main() {
  ...
  READ_BLACKBOARD(my_board, convertSecsToNano(5), (APEX_BYTE *)readData,
                  &length, &RETURN_CODE);

  LOGGER(APP, "Return code from READING FROM EMPTY BBOARD is %s",
         RETURN_CODE_TO_STRING(RETURN_CODE));
  DISPLAY_BLACKBOARD(my_board, (APEX_BYTE *)message, 200 * sizeof(char),
                     &RETURN_CODE);
