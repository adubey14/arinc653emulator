...
inline std::string GET_PARTITION_NAME()
{
	return "MM";
}
...
public:
	~REPORTER()
	{
			LOGGER(APP,"Reported Terminated");
	}
};
...
void REPORT(const char* message, ACM_USER::RETURN_CODE_TYPE *RETURN_CODE)
{
	...
	ACM_INTERNAL::AUTO_LOCK lock(REPORTER::mymutex);
	snprintf(REPORTER::my_instance->buffer1,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%s|%0.9Lf|APP|%s",REPORTER::my_instance->Partition_Name.c_str(),sec,message);
	try{

		REPORTER::my_instance->sendSocket.sendTo(REPORTER::my_instance->buffer1,
				strlen(REPORTER::my_instance->buffer1)+1,
				REPORTER::my_instance->hostname,
				REPORTER::my_instance->portnum);
	}
...
void LOG_INIT()
{
  ...
  if ( LOGGER_BUFFER::log_buffer !=0)
    {
      printf("Logger entries already configured, ignoring the environment parameter\n");
      return;
    }
      ...
      if (i>1)
        {
          ...
          for (long j=0;j<i;j++)
            {
              LOGGER_BUFFER::log_buffer[j]=(char*) malloc(((sizeof(char))* LOGGER_BUFFER::LOG_ENTRY_SIZE )+1);
            }
...
void LOG2FILE(){
	...
	/*printf("------------------------------------------------------------------------------------------------------------\n");
		printf("%11s|%20s|%7s|%s\n","PID:NAME","SECOND.NANO ","LEVEL","MESSAGE");
		printf("------------------------------------------------------------------------------------------------------------\n");*/

	unsigned int num_iterations = (log_overflow)?LOGGER_BUFFER::MAX_LOG_ENTRIES:log_index;
	if (log_overflow)
...
void LOGGER_IDENT(char* NAME,LOG_LEVEL LOGLEVEL, const char * format,...)
{
	...
	case  INFO:
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|INF|[From %s]",pid_data,Partition_Name.c_str(),sec, NAME);
	}
	break;

	case  ERROR :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|ERR|[From %s]",pid_data,Partition_Name.c_str(),sec, NAME);
	}
	break;
	case  DEBUG :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|DEB|[From %s]",pid_data,Partition_Name.c_str(),sec,NAME);
	}
	break;
	case  FATAL :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|FTL|[From %s]",pid_data,Partition_Name.c_str(),sec,NAME);
	}
	break;
	case  TRACE :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|TRC|[From %s]",pid_data,Partition_Name.c_str(),sec,NAME);
	}
	break;
	case  HMEVENT :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|HME|[From %s]",pid_data,Partition_Name.c_str(),sec,NAME);
	}
	break;
	case  APP :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|APP|[From %s]",pid_data,Partition_Name.c_str(),sec,NAME);
	}
	break;
	case  USER :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|USR|[From %s]",pid_data,Partition_Name.c_str(),sec,NAME);
	}
	break;	
	case  WARNING:
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|WRN|[From %s]",pid_data,Partition_Name.c_str(),sec,NAME);
	}
	}
	va_list args;
	...
	if (LOGGER_BUFFER::MAX_LOG_ENTRIES>1 && LOGGER_BUFFER::log_buffer!=0)
	{
		...
		strncpy(LOGGER_BUFFER::log_buffer[log_index++],buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE);
...
void LOGGER( LOG_LEVEL LOGLEVEL, const char * format,...)
{
	...
	case  INFO:
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|INF|",pid_data,Partition_Name.c_str(),sec);
	}
	break;

	case  ERROR :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|ERR|",pid_data,Partition_Name.c_str(),sec);
	}
	break;
	case  DEBUG :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|DEB|",pid_data,Partition_Name.c_str(),sec);
	}
	break;
	case  FATAL :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|FTL|",pid_data,Partition_Name.c_str(),sec);
	}
	break;
	case  TRACE :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|TRC|",pid_data,Partition_Name.c_str(),sec);
	}
	break;
	case  HMEVENT :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|HME|",pid_data,Partition_Name.c_str(),sec);
	}
	break;
	case  APP :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|APP|",pid_data,Partition_Name.c_str(),sec);
	}
	break;
	case  USER :
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|USR|",pid_data,Partition_Name.c_str(),sec);
	}
	break;	
	case  WARNING:
	{
		snprintf(buffer2,LOGGER_BUFFER::LOG_ENTRY_SIZE,"%d:%s|%0.9Lf|WRN|",pid_data,Partition_Name.c_str(),sec);
	}
	}
	va_list args;
	va_start (args, format);
	...
	{
		ACM_INTERNAL::AUTO_LOCK lock (LOG_MUTEX);
		strncpy(LOGGER_BUFFER::log_buffer[log_index++],buffer2,LOGGER_BUFFER::LOGGER_BUFFER::LOG_ENTRY_SIZE);
		if (log_index >= LOGGER_BUFFER::MAX_LOG_ENTRIES)
		{
			log_overflow=true;
			log_index=0;
