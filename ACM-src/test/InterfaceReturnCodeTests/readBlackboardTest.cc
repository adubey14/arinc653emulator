...
void PROCESS_APERIODIC (void){
	...
	DISPLAY_BLACKBOARD (SOME_BLACKBOARD_ID,(APEX_BYTE*)(&myLoad),sizeof(Payload),&RETURN_CODE);
	//Tests no error is returned when correctly when reading from a blackboard
	READ_BLACKBOARD(SOME_BLACKBOARD_ID, convertSecsToNano(1), (APEX_BYTE*)(&readLoad), &length, &RETURN_CODE);
	...
	READ_BLACKBOARD(SOME_BLACKBOARD_ID, convertSecsToNano(0), (APEX_BYTE*)&readLoad, &length, &RETURN_CODE);
	...
	DISPLAY_BLACKBOARD (SOME_BLACKBOARD_ID,(APEX_BYTE*)(&myLoad),sizeof(myLoad),&RETURN_CODE);
	READ_BLACKBOARD(SOME_BLACKBOARD_ID, convertSecsToNano(315569261), (APEX_BYTE*)&readLoad, &length, &RETURN_CODE);
	...
	//Tests that the correct code is returned when attempting to read from a blackboard that does not exist
