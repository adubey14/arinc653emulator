/*
Test of the CLEAR_BLACKBOARD service
*/
...
void PROCESS_APERIODIC (void){
	...
	//Tests that no error is returned when correctly clear a blackboard
	CLEAR_BLACKBOARD (BB_ID, &RETURN_CODE);
	if(RETURN_CODE == NO_ERROR){
		LOGGER(APP,"Clear blackboard test Succeeded!");  
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE); 
	}
	else{
		LOGGER(APP,"Clear blackboard test Failed."); 
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);  
	}

	//Tests that the correct code is returned when attempting to clear a nonexistent blackboard
	BLACKBOARD_ID_TYPE NONEXISTENT_BB_ID = 10;
	CLEAR_BLACKBOARD (NONEXISTENT_BB_ID, &RETURN_CODE);
