...
void APEX::CREATE_BLACKBOARD(
  ...
  // LOGGER(INFO,"From CREATE_BLACKBOARD: Trying to create blackboard %s
  // ",BLACKBOARD_NAME );

  // Can Initialize only if the state is COLD_START or WARM_START
  if (partition_instance->CURRENT_OPERATING_MODE == NORMAL) {

    LOGGER(ERROR, "From CREATE_BLACKBOARD: Trying to create blackboard %s "
                  "while partition is in NORMAL state",
           BLACKBOARD_NAME);
  ...
  for (unsigned int i = 0; i < partition_instance->BLACKBOARD_VECTOR.size();
       i++) {
    if (partition_instance->BLACKBOARD_VECTOR[i]->BLACKBOARD_NAME ==
        std::string(BLACKBOARD_NAME)) {
      LOGGER(ERROR, "From CREATE_BLACKBOARD: NAME %s is not unique",
             BLACKBOARD_NAME);
  ...
  *BLACKBOARD_ID = partition_instance->BLACKBOARD_VECTOR.size();
  ...
  APEX_BLACKBOARD_TYPE *APEX_BLACKBOARD = new APEX_BLACKBOARD_TYPE(
      BLACKBOARD_NAME, *BLACKBOARD_ID, MAX_MESSAGE_SIZE);
...
void APEX::DISPLAY_BLACKBOARD(
  ...
  APEX_BLACKBOARD_TYPE *BBOARD =
      partition_instance->BLACKBOARD_VECTOR[BLACKBOARD_ID];
...
void APEX::CLEAR_BLACKBOARD(
  ...
  partition_instance->BLACKBOARD_VECTOR[BLACKBOARD_ID]->ERASE_BLACKBOARD(
      RETURN_CODE);
...
void APEX::GET_BLACKBOARD_STATUS(
  ...
  BLACKBOARD_STATUS->EMPTY_INDICATOR =
      (BBOARD->CURRENT_MESSAGE_SIZE == 0) ? EMPTY : OCCUPIED;
  BLACKBOARD_STATUS->MAX_MESSAGE_SIZE = BBOARD->MAX_MESSAGE_SIZE;
  EVENT_STATUS_TYPE EVENT_STATUS;
  APEX::GET_EVENT_STATUS(BBOARD->BLACKBOARD_EVENT_ID, &EVENT_STATUS,
                         RETURN_CODE);
