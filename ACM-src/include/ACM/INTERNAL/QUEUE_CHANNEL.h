...
namespace ACM_INTERNAL{
	...
	public:
		...
		QUEUE_CHANNEL(std::string NAME,COMMUNICATION_TYPE_ENUM COMMUNICATION_TYPE_VAL=INTRA_MODULE,unsigned int srcportNumber=0);		
		QUEUE_CHANNEL(COMMUNICATION_TYPE_ENUM COMMUNICATION_TYPE_VAL=INTRA_MODULE,unsigned int srcportNumber=0);
		...
		std::string GET_SOURCE_NAME()
		{
			...
			{
				char buffer[30];
				snprintf(buffer,30,"InterModulePort:%d",this->recv_mySocket->getLocalPort());
				return buffer;
			}
			else
				return("");
		...
		void ADD_SOURCE_QUEUE(std::string Name,MESSAGE_SIZE_TYPE MAX_MESSAGE_SIZE,MESSAGE_RANGE_TYPE MAX_NB_MESSAGE,RETURN_CODE_TYPE *RETURN_CODE);
		void ADD_DESTINATION_QUEUE(std::string Name,MESSAGE_SIZE_TYPE MAX_MESSAGE_SIZE,MESSAGE_RANGE_TYPE MAX_NB_MESSAGE,RETURN_CODE_TYPE *RETURN_CODE);
