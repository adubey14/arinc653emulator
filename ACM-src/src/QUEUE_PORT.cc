...
namespace ACM_INTERNAL{
		...
		case ACM_USER::PORT_SOURCE:
			{
				...
				this->Q_MQDES=mq_open (REAL_PLATFORM_NAME.c_str(), O_RDWR/*O_WRONLY*/ |FLAGS, 0664, &attr);
		...
		case ACM_USER::PORT_DEST:
			{
				...
				this->Q_MQDES=mq_open (REAL_PLATFORM_NAME.c_str(), /*O_RDONLY*/O_RDWR | FLAGS, 0664, &attr);
				LOGGER(INFO,"%d,%s\n",errno,strerror(errno));
	...
	void MESSAGE_QUEUE_TYPE::SEND_MESSAGE(MESSAGE_ADDR_TYPE msg_ptr,MESSAGE_SIZE_TYPE LENGTH, SYSTEM_TIME_TYPE TIME_OUT,RETURN_CODE_TYPE* RETURN_CODE)
	{
		...
		*RETURN_CODE=NO_ERROR;
		...
		else
		{
			...
			LOGGER(INFO,"*****Result of timed send on %s %d %s*******",this->Q_NAME.c_str(),result,strerror(errno));
			if (result==-1)
			{
				if (errno==ETIMEDOUT){
					*RETURN_CODE=TIMED_OUT;
	...
	void MESSAGE_QUEUE_TYPE::SEND_MESSAGE_FROM_BUFFER_READ_FROM_SOCKET(MESSAGE_ADDR_TYPE msg_ptr,MESSAGE_SIZE_TYPE LENGTH, SYSTEM_TIME_TYPE TIME_OUT,RETURN_CODE_TYPE* RETURN_CODE)
	{
		if (!READY_OR_NOT){
			*RETURN_CODE=INVALID_MODE;
			return;
		...
		*RETURN_CODE=NO_ERROR;
		...
		{
			LOGGER(ACM_USER::ERROR,"Queue %s is to be used only for sending message. Cannot write to it from a socket",this->Q_NAME.c_str());
			*RETURN_CODE=INVALID_MODE;
			return;
		}
		if (TIME_OUT==0 && MESSAGE_LIMIT_EXCEEDED())
