...
// send the signal to the partition controller that this partition is ready with
// initialization
// signal is SIGUSR1
...
void APEX::sigIntHandler(int sig) {
  ...
  if (APEX::sigIntCount > 1) {
    LOGGER(ERROR, "There was some problem in clean termination. You asked for "
...
void APEX::sigsegvHandler(int sig) {
  LOGGER(FATAL,
         "Received sigsegv in the Partition. Segementation Violation Happened");
...
void APEX::SET_FIFO_POLICY() throw(ACM_BASE_EXCEPTION_TYPE) {
  ...
  if (retVal != 0) {
    LOGGER(ERROR, "pthread_getschedparam at %d failed with %d", __LINE__,
           retVal);
  }
  ...
  if (policy != ACM_SCHED_POLICY) {
    ...
    if (schedRetValue == -1) {
      // This failure happens when the user is not a root.	//The framework
      // should log this error and EXIT.
      LOGGER(FATAL, "Error when trying to set FIFO scheduling: %s",
             strerror(errno));
      char buffer[100];
      snprintf(buffer, 100, "Error when trying to set FIFO scheduling: %s",
               strerror(errno));
      throw ACM_BASE_EXCEPTION_TYPE(buffer);
    }
    // after setting FIFO scheduling, get controllers ABSOLUTE priority
    sched_getparam(0, &sp_Controller);
    LOGGER(ACM_USER::DEBUG, "Controller's ABSOLUTE priority: %d",
           sp_Controller.sched_priority);
    // get thread priority
    retVal = 0;
    ...
    if (sp_Process.sched_priority < CONTROLLER_PROCESS_PRIORITY) {
      LOGGER(ACM_USER::DEBUG, "setting main thread priority to %d",
             CONTROLLER_PROCESS_PRIORITY);
...
void APEX::SET_PARTITION_PRIORITY() {
  ...
  // set CPU affinity to bind this process it to processor 1
  // the setting will be inherited by fork, execv, and to threads
  LOGGER(INFO, "Running With Module Manager, Setting Controller priority to "
               "CONTROLLER_PROCESS_PRIORITY");
  ...
  if (policy != ACM_SCHED_POLICY) {
    LOGGER(ACM_USER::ERROR,
           "Partition FIFO Policy is not set - Running with MM");
    exit(-1);
  } else {
    struct sched_param sp_Controller;
    sched_getparam(0, &sp_Controller);
...
/*====================================Scheduler Initialization and Scheduling
 * services==================================================*/

int APEX::InitPartitionScheduler(/*!out*/ RETURN_CODE_TYPE *RETURN_CODE) {
  // This method is invoked from set_partition_mode before the mode is set to
  // normal
  // set_partition_mode ensures that this method is not invoked if the partition
  // is already in mode = normal.
...
// signal to the parent
  ...
  if (entries != NULL) {
    ...
    if (init_wait_time > 0) {
      ...
      LOGGER(APP,
             "*** Setting Init Wait Time to %ld and starting the wait phase",
             init_wait_time);
      SYSTEM_TIMESPEC actualtime = currentTime();
      SYSTEM_TIMESPEC absolute_time =
          addTimeNano(actualtime, convertSecsToNano(init_wait_time));
...
void APEX::RunPartitionScheduler() {
  ...
  HARD_DEADLINE_PROCESS_RESPONSE.ERROR_RESPONSE.ERROR_CODE =
      ACM_USER::DEADLINE_MISSED;
  HARD_DEADLINE_PROCESS_RESPONSE.ERROR_RESPONSE.HM_MESSAGE_LENGTH = 0;
  HARD_DEADLINE_PROCESS_RESPONSE.ERROR_RESPONSE.HM_ACTION =
      HM_RESPONSE_TYPE::STOP;
  //{PROCESS_RESPONSE_TYPE::HARD_DEADLINE_MISSED,0,
  //false,PROCESS_RESPONSE_TYPE::PARTITIONSCHEDULER,"violation"};
  // PROCESS_RESPONSE_TYPE SOFT_DEADLINE_PROCESS_RESPONSE
  // {PROCESS_RESPONSE_TYPE::SOFT_DEADLINE_MISSED,0,
  // false,PROCESS_RESPONSE_TYPE::PARTITIONSCHEDULER,"violation"};
  APEX *partition_instance = APEX::_pinstance;
  ...
  assert(partition_instance->PROCESS_TABLE_MUTEX.try_lock_without_locking() ==
         MUTEX::NO_ERROR);
...
// initialize first schedule
  ...
  for (unsigned int i = 0;
    ...
    partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]->NEXT_SCHEDULING_INSTANCE =
        PROCESS_DESCRIPTOR::RELEASE_PROCESS; // set the next schedule to release
    ...
    if (status == PROCESS::WAITING_PARTITION_NOT_NORMAL) {
      ...
      if (partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]
        ...
        partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]
            ->ABSOLUTE_NEXT_SCHEDULED_TIME = currentTime();
      } else {
        partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]
            ->ABSOLUTE_NEXT_SCHEDULED_TIME =
            // currentTime(); // compute next release point
            addTimeNano(currentTime(),
                        partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]
                            ->INITIAL_DELAY_TIME);
      }

      partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]->ABSOLUTE_CRITICAL_TIME =
          partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]
              ->ABSOLUTE_NEXT_SCHEDULED_TIME;
      ...
      if (process->isPeriodic()) // set process state based on the nature of the
        ...
        process->set_state(PROCESS::READY); // set process state to ready for
                                            // aperiodic process.
    }
  }
  ...
  //		LOGGER(INFO,"In RunScheduler::after unlocking , the process table
  //mutex status is %ld",
  //partition_instance->PROCESS_TABLE_MUTEX.try_lock_without_locking());

  while (partition_instance->CURRENT_OPERATING_MODE == NORMAL &&
         !partition_instance->flagToTerminate) {
    ...
    if ((partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR.size() == 0) ||
        (partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR[0]
             ->THE_PROCESS->get_state() == PROCESS::DORMANT) ||
        ((partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR[0]
              ->THE_PROCESS->attribute.TIME_CAPACITY == INFINITE_TIME_VALUE) &&
         (partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR[0]
              ->NEXT_SCHEDULING_INSTANCE ==
          PROCESS_DESCRIPTOR::CHECK_DEADLINE))) {
      ...
      APEX_SCHEDULER_WAIT_ABSOLUTE_TIME(next_sleep_time); // May be wait forever
                                                          // and get triggered
                                                          // if there is
                                                          // something to do.
      continue;
    ...
    // partition_instance->PROCESS_TABLE_MUTEX.try_lock(); //try if it is not
    // locked
    // partition_instance->sortProcessDescriptorVector(); //locks and sorts the
    // vector

    // Do the scheduling loop
    // SYSTEM_TIMESPEC actualtime=currentTime();

    // LOGGER( DEBUG, "Size of PROCESS_SCHEDULING_SORTED_VECTOR is
    // %d",partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR.size());
    ...
    if (partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR.size() > 0) {
      next_sleep_time = partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR[0]
    ...
    for (unsigned int count = 0;
         count < partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR.size();
         count++) {

      // if absolute critical time is less than or equal to current time
      if (compareTime(
              partition_instance->PROCESS_SCHEDULING_SORTED_VECTOR[count]
                  ->ABSOLUTE_CRITICAL_TIME,
              current_time)) {
        ...
        case PROCESS_DESCRIPTOR::RELEASE_PROCESS: {
          ...
          // Check that the current state should be DORMANT or waiting on pERIOD
          // or WAITING on Partition not Normal
          PROCESS::PROCESS_state current_process_state =
              process_descriptor->THE_PROCESS->get_state();

          // current_process_state == PROCESS::DORMANT ||
          if (current_process_state == PROCESS::WAITING_ON_PERIOD ||
              current_process_state == PROCESS::READY) {
            process_descriptor->THE_PROCESS->set_priority(
                process_descriptor->THE_PROCESS->attribute.BASE_PRIORITY);
            process_descriptor->CURRENT_PRIORITY =
                process_descriptor->THE_PROCESS->attribute.BASE_PRIORITY;

            // LOGGER( DEBUG, "State of process %s is
            // %d",process_descriptor->THE_PROCESS->attribute.NAME,current_process_state);

            process_descriptor->THE_PROCESS
                ->INITIALIZE_PROCESS_RESPONSE(); //!<This will initialize the
                                                 //!process response back to
                                                 //!default
            process_descriptor->THE_PROCESS->set_state(PROCESS::READY);
            LOGGER(DEBUG, "Releasing process %s",
                   process_descriptor->THE_PROCESS->attribute.NAME);
            LOGGER(DEBUG, " ABSOLUTE_CRITICAL_TIME of %s is %ld.%ld",
                   process_descriptor->THE_PROCESS->attribute.NAME,
                   process_descriptor->ABSOLUTE_CRITICAL_TIME.tv_sec,
                   process_descriptor->ABSOLUTE_CRITICAL_TIME.tv_nsec);
            process_descriptor->THE_PROCESS->start();

            process_descriptor->ABSOLUTE_PREV_START_TIME =
                current_time; // set previous schedule time
...

            {
              ...
              process_descriptor->ABSOLUTE_NEXT_SCHEDULED_TIME = addTimeNano(
                  process_descriptor->ABSOLUTE_CRITICAL_TIME,
                  process_descriptor->THE_PROCESS->attribute.PERIOD);
              // std::cout<<"tick\n";
            }

            // process_descriptor->DEADLINE_TIME =
            // computeDeadlineTimeSpecs(current_time,process_descriptor->THE_PROCESS->attribute.TIME_CAPACITY);
            process_descriptor->DEADLINE_TIME = computeDeadlineTimeSpecs(
                process_descriptor->ABSOLUTE_CRITICAL_TIME,
                process_descriptor->THE_PROCESS->attribute.TIME_CAPACITY);
            process_descriptor->ABSOLUTE_CRITICAL_TIME =
                process_descriptor->DEADLINE_TIME; // set absolute critical time
            // process_descriptor->CRITICAL_TIME=process_descriptor->THE_PROCESS->attribute.TIME_CAPACITY;
            // //set critical time
            process_descriptor->NEXT_SCHEDULING_INSTANCE =
                PROCESS_DESCRIPTOR::CHECK_DEADLINE; // set next scheduling
                                                    // action
          }
        } break;
        ...
        case PROCESS_DESCRIPTOR::CHECK_DEADLINE: {
          ...
          LOGGER(ACM_USER::DEBUG, "Checking Deadline for %s",
                 process_descriptor->THE_PROCESS->attribute.NAME);

          PROCESS::PROCESS_state current_process_state =
              process_descriptor->THE_PROCESS->get_state();
          if (current_process_state == PROCESS::DORMANT ||
              current_process_state == PROCESS::WAITING_ON_PERIOD) {

            // Deadline not violated
            LOGGER(ACM_USER::DEBUG, "No Deadline Violation for %s",
                   process_descriptor->THE_PROCESS->attribute.NAME);

            if (current_process_state ==
                PROCESS::WAITING_ON_PERIOD) // set the next release time for
                                            // periodic process
              process_descriptor->ABSOLUTE_CRITICAL_TIME =
                  process_descriptor->ABSOLUTE_NEXT_SCHEDULED_TIME;

            process_descriptor->NEXT_SCHEDULING_INSTANCE =
                PROCESS_DESCRIPTOR::RELEASE_PROCESS; // set the next action to
                                                     // release
            ...
            if (process_descriptor->THE_PROCESS->attribute.DEADLINE == HARD) {
              ...
              LOGGER(HMEVENT, "DETECT HARD DEADLINE VIOLATION for %s:%s ",
                     process_descriptor->THE_PROCESS->attribute.NAME,
                     partition_instance->PARTITION_NAME.c_str());
              process_descriptor->THE_PROCESS->set_priority(
                  WORKER_THREAD_RESTART_PRIORITY);
              process_descriptor->THE_PROCESS->stop(true);
              process_descriptor->THE_PROCESS->set_priority(
                  process_descriptor->THE_PROCESS->attribute.BASE_PRIORITY);
              process_descriptor->NEXT_SCHEDULING_INSTANCE =
                  PROCESS_DESCRIPTOR::RELEASE_PROCESS;

              // Inform the health Manager of this process.
              LOGGER(HMEVENT, "RAISE_ERROR for %s: %s",
                     process_descriptor->THE_PROCESS->get_name().c_str(),
                     partition_instance->PARTITION_NAME.c_str());
              RETURN_CODE_TYPE RETURN_CODE;
              APEX::NOTIFY_PROCESS_FINISHED_STATUS(
                  process_descriptor->THE_PROCESS->process_id,
                  &HARD_DEADLINE_PROCESS_RESPONSE, &RETURN_CODE);
              APEX::RAISE_APPLICATION_ERROR(
                  process_descriptor->THE_PROCESS->process_id, DEADLINE_MISSED,
                  0, 0, &RETURN_CODE);

            } else {
              // If ever we get into the situation where anaperiodic process
              // with has soft deadline and
              // it is suspended, the scheduler will go into running at every
              // second to monitor the deadline of the aperiodic process.
              // we will still continue to monitor this for deadline violation.
              // Setting a default critical time of 1 second
              LOGGER(HMEVENT, "WARNING, Soft Deadline Violation for %s: %s ",
                     process_descriptor->THE_PROCESS->attribute.NAME,
                     partition_instance->PARTITION_NAME.c_str());
              // we will still continue to monitor this for deadline violation
              ...
              //!!! TEST Added by abhishek to get rid of the soft deadline
              //!violation bug
              ...
              {
                //! Required to ensure that the process starts in phase even
                //! when it misses the deadline.
                while (compareTime(
                    process_descriptor->ABSOLUTE_NEXT_SCHEDULED_TIME,
                    process_descriptor->ABSOLUTE_CRITICAL_TIME))
                  // process_descriptor->ABSOLUTE_NEXT_SCHEDULED_TIME=addTimeNano(current_time,process_descriptor->THE_PROCESS->attribute.PERIOD);
              ...
              //!violation bug
              // Inform the health Manager of this process.
              LOGGER(HMEVENT, "Informing HM of %ld of deadline violation",
                     process_descriptor->THE_PROCESS->process_id);
              RETURN_CODE_TYPE RETURN_CODE;
        ...
        break; // @nag : This loop probably should not continue if there is more
               // time for the processes to launch
  ...
  LOGGER(ACM_USER::DEBUG,
         "Terminate Flag is Set. Getting out of Run Partition Scheduler");
}
...
//	if ( partition_instance->PRIORITY_LEVEL == LOW_LEVEL ) // THis is a flag
//that indicates wether the partition is waking up or going to sleep
//	{
//		//the priorities are already low, we have to raise them back =
...
//			schedRetValue = pthread_setschedparam( pthread_self(),
//ACM_SCHED_POLICY, &sp_Thread);
//		}

//		//AUTO_LOCK lock (partition_instance->PROCESS_TABLE_MUTEX);

//		//raise each thread priority to the original value (only which is
//still in use)
//		for(unsigned int i =
//0;i<partition_instance->PROCESS_DESCRIPTOR_VECTOR.size();i++)
//		{
//			PROCESS* process =
//partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]->THE_PROCESS;
...
//	else
//	{
//		//the priorities are already high, we have to lower them back =
//this partition is not running
//		//lower each thread priority to the LOW_PRIORITY VALUE
//		{
//			//AUTO_LOCK lock
//(partition_instance->PROCESS_TABLE_MUTEX);

//			//lower each thread priority to the original value (only
...
//			struct sched_param sp_Thread;
//			sp_Thread.sched_priority =
//PARTITION_PROCESS_LOWPRIORITY;
//			int schedRetValue = 99;
//			//checking which thread's priority to change
//			schedRetValue = pthread_setschedparam( pthread_self(),
//ACM_SCHED_POLICY, &sp_Thread);
...
//	partition_instance->PRIORITY_LEVEL =(partition_instance->PRIORITY_LEVEL
//== LOW_LEVEL) ? HIGH_LEVEL : LOW_LEVEL ;
