...
void PROCESS_READER1 (void)
{
	...
	while (true)
	{
		...
		{
			LOGGER(APP,"Read Data %f. Time %lld nanoseconds since Jan 1 1970",readLoad.data,readLoad.time);
		}
	}
...
int main ()
{
	...
	CREATE_BUFFER(BUFFER_NAME,sizeof(Payload),10 /*number of messages*/,FIFO,&TEST_BUFFER_ID,&RETURN_CODE);
	assert(RETURN_CODE==NO_ERROR);
