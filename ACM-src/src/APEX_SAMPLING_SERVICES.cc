...
void APEX::CREATE_SAMPLING_PORT(
  ...
  if (partition_instance->CURRENT_OPERATING_MODE == NORMAL) {
    LOGGER(ERROR, "From CREATE_SAMPLING_PORT: Trying to %s while partition is "
                  "in NORMAL state",
           SAMPLING_PORT_NAME);
    *RETURN_CODE = INVALID_MODE;
    return;
  }
  if (partition_instance->SAMPLING_PORT_VECTOR.size() >=
      SYSTEM_LIMIT_NUMBER_OF_SAMPLING_PORTS) {
    LOGGER(ERROR,
           "From CREATE_SAMPLING_PORT: exceed sampling port number limit");
    *RETURN_CODE = INVALID_CONFIG;
    return;
  }
  // We do not check the max message size right now(MAX_MESSAGE_SIZE> (int)
  // ACM_USER::SYSTEM_LIMIT_MAX_SAMPLING_MESSAGE_SIZE)

  if (MAX_MESSAGE_SIZE == 0 ||
      MAX_MESSAGE_SIZE > SYSTEM_LIMIT_MAX_SAMPLING_MESSAGE_SIZE) {
    // LOGGER(  ERROR, "From CREATE_SAMPLING_PORT: %s: MAX_MESSAGE_SIZE %d is
    // either 0 or > SYSTEM_LIMIT_MAX_SAMPLING_MESSAGE_SIZE
    // %d",SAMPLING_PORT_NAME,MAX_MESSAGE_SIZE,ACM_USER::SYSTEM_LIMIT_MAX_SAMPLING_MESSAGE_SIZE
    // );
    *RETURN_CODE = INVALID_CONFIG;
    return;
  }
  if (REFRESH_PERIOD == 0 || REFRESH_PERIOD > MAX_TIME_OUT) {
    LOGGER(ERROR, "From CREATE_SAMPLING_PORT: %s: "
                  "REFRESH_PERIOD==0||REFRESH_PERIOD > MAX_TIME_OUT",
           SAMPLING_PORT_NAME);
    *RETURN_CODE = INVALID_CONFIG;
    return;
  }
  // Ensure that the SAMPLING_PORT_NAME is unique
  for (unsigned int i = 0; i < partition_instance->SAMPLING_PORT_VECTOR.size();
       i++) {
    if (partition_instance->SAMPLING_PORT_VECTOR[i]->S_NAME ==
        std::string(SAMPLING_PORT_NAME)) {
      LOGGER(ERROR, "From CREATE_SAMPLING_PORT: NAME %s is not unique",
             SAMPLING_PORT_NAME);
  ...
  *SAMPLING_PORT_ID = partition_instance->SAMPLING_PORT_VECTOR.size();
  ...
  SAMPLING_PORT_HELPER *sampling_port = new SAMPLING_PORT_HELPER(
      SAMPLING_PORT_NAME, MAX_MESSAGE_SIZE, new_port_direction, REFRESH_PERIOD,
      RETURN_CODE, O_CREAT);
  if (*RETURN_CODE == NO_ERROR)
    partition_instance->SAMPLING_PORT_VECTOR.push_back(sampling_port);
  return;
}
void APEX::WRITE_SAMPLING_MESSAGE(
  ...
  if ((unsigned int)SAMPLING_PORT_ID >=
      partition_instance->SAMPLING_PORT_VECTOR.size()) {
    *RETURN_CODE = INVALID_PARAM;
    return;
  }

  SAMPLING_PORT_HELPER *sampling_port =
      partition_instance->SAMPLING_PORT_VECTOR[SAMPLING_PORT_ID];
...
void APEX::READ_SAMPLING_MESSAGE(
  ...
  SAMPLING_PORT_HELPER *sampling_port =
      partition_instance->SAMPLING_PORT_VECTOR[SAMPLING_PORT_ID];
  sampling_port->READ_SAMPLING_MESSAGE(MESSAGE_ADDR, LENGTH, VALIDITY,
                                       RETURN_CODE);
  return;
}
...
void APEX::WRITE_CONTENTS_FROM_DATA_BUFFER(
...
void APEX::READ_CONTENTS_TO_DATA_BUFFER(SAMPLING_PORT_ID_TYPE SAMPLING_PORT_ID,
  ...
  SAMPLING_PORT_HELPER *sampling_port =
      partition_instance->SAMPLING_PORT_VECTOR[SAMPLING_PORT_ID];
  sampling_port->READ_CONTENTS_TO_DATA_BUFFER(msg_ptr, MAX_LENGTH, READ_LENGTH,
                                              RETURN_CODE);
}
}
#endif