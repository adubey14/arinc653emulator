...
*/
	...
	void APEX::PRINT_CREATED_OBJECT_LIST(/*!out*/ RETURN_CODE_TYPE *RETURN_CODE )
	{
		...
		*RETURN_CODE=NO_ERROR;
		if(partition_instance->CURRENT_OPERATING_MODE==IDLE || partition_instance->CURRENT_OPERATING_MODE==NORMAL){
			LOGGER( ACM_USER::WARNING,"From PRINT_CREATED_OBJECT_LIST: Violated: PARTITION STATE== IDLE || NORMAL");
		...
		*RETURN_CODE=INVALID_MODE;
		...
		for (unsigned int count=0; count<partition_instance->PROCESS_DESCRIPTOR_VECTOR.size();count++){
			PROCESS* p = partition_instance->PROCESS_DESCRIPTOR_VECTOR[count]->THE_PROCESS;
			LOGGER(INFO,"%d. ProcessName: %s,ProcessID: %ld: Period(ns):%lld Deadline(ns):%lld : Deadline Type:%s", count, p->attribute.NAME,p->process_id,p->attribute.PERIOD,p->attribute.TIME_CAPACITY,
				(p->attribute.DEADLINE==SOFT)?"SOFT DEADLINE":"HARD DEADLINE");
		}
	//	printf("===========LIST OF RW_LOCK_SEMAPHORE ==================\n");
		for (unsigned int count=0; count<partition_instance->RW_LOCK_SEM_VECTOR.size();count++){
			LOGGER(INFO,"%d. RW_LOCK_SEMAPHORE %s", count, partition_instance->RW_LOCK_SEM_VECTOR[count]->GET_NAME().c_str());
		}

		//printf("===========LIST OF SEMAPHORE ==================\n");
		for (unsigned int count=0; count<partition_instance->SEMAPHORE_VECTOR.size();count++){
			LOGGER(INFO,"%d. SEMAPHORE: %s", count, partition_instance->SEMAPHORE_VECTOR[count]->GET_NAME().c_str());
		}

	//	printf("===========LIST OF EVENT ==================\n");
		for (unsigned int count=0; count<partition_instance->EVENT_VECTOR.size();count++){
			LOGGER(INFO,"%d. EVENT: %s", count, partition_instance->EVENT_VECTOR[count]->GET_NAME().c_str());
		}
		//printf("===========LIST OF BLACKBOARD ==================\n");
		for (unsigned int count=0; count<partition_instance->BLACKBOARD_VECTOR.size();count++){
			LOGGER(INFO,"%d. BLACKBOARD: %s: MAX_MESSAGE_SIZE %ld BYTES", count, partition_instance->BLACKBOARD_VECTOR[count]->BLACKBOARD_NAME.c_str(), partition_instance->BLACKBOARD_VECTOR[count]->MAX_MESSAGE_SIZE);
		}
		//printf("===========LIST OF BUFFER ==================\n");
		for (unsigned int count=0; count<partition_instance->BUFFER_VECTOR.size();count++){
			LOGGER(INFO,"%d. BUFFER: %s MAX_MESSAGE_SIZE %ld BYTES: MAX_NUM_MESSAGES:%ld", count, partition_instance->BUFFER_VECTOR[count]->GET_NAME().c_str(),
		...
		for (unsigned int count=0; count<partition_instance->SAMPLING_PORT_VECTOR.size();count++){
			LOGGER(INFO,"%d. SAMPLING_PORT: %s :MAX_MESSAGE_SIZE %ld BYTES", count, partition_instance->SAMPLING_PORT_VECTOR[count]->GET_NAME().c_str(),partition_instance->SAMPLING_PORT_VECTOR[count]->S_MAX_MESSAGE_SIZE);
		}
		//printf("===========LIST OF QUEUING_PORT ==================\n");
		for (unsigned int count=0; count<partition_instance->QUEUING_PORT_VECTOR.size();count++){
			LOGGER(INFO,"%d. QUEUING_PORT: %s: MAX_MESSAGE_SIZE %ld BYTES: MAX_NUM_MESSAGES:%ld", count, partition_instance->QUEUING_PORT_VECTOR[count]->GET_NAME().c_str(),
	...
	ACM_INTERNAL::MUTEX APEX::singletonInstanceMutex (false, ACM_INTERNAL::MUTEX::NORMAL);
	...
	std::string APEX::GET_PARTITION_NAME()  
	{
		return APEX::PARTITION_NAME;
	}
	...
	APEX::APEX():	PROCESS_TABLE_MUTEX(false, MUTEX::ERRORCHECK),
		CURRENT_OPERATING_MODE(ACM_USER::COLD_START),flagToTerminate(false)
	{

	  LOG_INIT();
	  //LOGGER(INFO,"INSIDE APEX_CONSTRUCTOR");
		APEX_SCHEDULING_EVENT = APEX_EVENT_TYPE::CREATE_EVENT("APEX_SCHEDULING_EVENT",&(this->PROCESS_TABLE_MUTEX));
	...
	void APEX::CLEAR_VECTORS()
	{
		...
		if (partition_instance ==NULL)  return;
		//clear events
		for (unsigned int count=0; count<_pinstance->RW_LOCK_SEM_VECTOR.size();count++){
		...
		for (unsigned int count=0; count<_pinstance->SEMAPHORE_VECTOR.size();count++){
		...
		for (unsigned int count=0; count<_pinstance->EVENT_VECTOR.size();count++){
			APEX_EVENT_TYPE* apex_event = _pinstance->EVENT_VECTOR[count];				
			delete(apex_event); // this will call the terminate and then wait for termination;
		}
		LOGGER(ACM_USER::DEBUG,"_pinstance->EVENT_VECTOR.clear();");
		_pinstance->EVENT_VECTOR.clear();			

		for (unsigned int count=0; count<_pinstance->BLACKBOARD_VECTOR.size();count++){
		...
		for (unsigned int count=0; count<_pinstance->SAMPLING_PORT_VECTOR.size();count++){
		...
		for (unsigned int count=0; count<_pinstance->QUEUING_PORT_VECTOR.size();count++){
		...
		for (unsigned int count=0; count<_pinstance->BUFFER_VECTOR.size();count++){
	...
	//!Destructor
	void APEX::TERMINATE(){
		AUTO_LOCK lock (singletonInstanceMutex);//<!Cannot create a singleton instance anymore
		if(_pinstance==0)
		{
		...
		for (unsigned int count=0; count<_pinstance->PROCESS_DESCRIPTOR_VECTOR.size();count++){
			PROCESS* p = _pinstance->PROCESS_DESCRIPTOR_VECTOR[count]->THE_PROCESS;
			p->set_priority(WORKER_THREAD_RESTART_PRIORITY);
			delete(_pinstance->PROCESS_DESCRIPTOR_VECTOR[count]);// this will call the terminate and then wait for termination for the process, delete the attribute
		...
		LOGGER(ACM_USER::DEBUG,"_pinstance->PROCESS_SCHEDULING_SORTED_VECTOR.clear();");
	...
	//This code doesnot take into consideration the case when a process could not run, because the necessary locks are not available.
	// If this portion of the code needs to be added as per the ARINC specs, the PROCESS should probably handle this in the set_status method 
	// upon trying to set the state of process to ready. Setting the status to ready should fail and the scheduler code should be updated to 
	// take this into account.

	/*bool APEX::GetNextCriticalTime(int i, long long& rem_ns)//process is in state - waiting to schedule
	{
	rem_ns = 0;
	...
	{
	rem_ns = subtractTime2(partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]->ABSOLUTE_NEXT_SCHEDULED_TIME, actualTime);
	char message[100];
	LOGGER(message," Computed next launch time = %lld \n",rem_ns);
	LOGGER(message);
	return true;
	}
	else if ((status == PROCESS::RUNNING) || (status == PROCESS::WAITING_ON_RESOURCE))
	{
	rem_ns = subtractTime2(partition_instance->PROCESS_DESCRIPTOR_VECTOR[i]->DEADLINE_TIME, actualTime);
	char message[100];
	...
	void APEX::DoSchedulabilityAnalysis()
	{
		...
		if ( !schedulable ){
			LOGGER( ERROR, "Schedulability analysis failed for the given process definitions");
			throw ACM_BASE_EXCEPTION_TYPE( "Schedulability analysis failed for the given process definitions" );
		}
	}
		...
		/*!out*/ RETURN_CODE_TYPE *RETURN_CODE )
	{
		...
		*RETURN_CODE = NO_ERROR;
		...
		if (OPERATING_MODE == NORMAL) 
		{ 
			//Transition to mode - normal, implies all the partition initializations are completed.
			//The partition is initialized for scheduling and then the mode is swithced to normal.
