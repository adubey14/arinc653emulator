...
namespace ACM_INTERNAL{
	...
	APEX_BLACKBOARD_TYPE::APEX_BLACKBOARD_TYPE(BLACKBOARD_NAME_TYPE NAME,BLACKBOARD_ID_TYPE ID,MESSAGE_SIZE_TYPE size):  BLACKBOARD_EVENT_ID(0),
																										BLACKBOARD_NAME(NAME),
																										BLACKBOARD_ID(ID),
		...
		APEX_BLACKBOARD_TYPE::~APEX_BLACKBOARD_TYPE(){
			if(PAYLOAD!=0){
				//LOGGER(ACM_USER::DEBUG,"Deleting payload for black board %s",BLACKBOARD_NAME.c_str());
			free(PAYLOAD);
			//LOGGER(ACM_USER::DEBUG,"Deleted payload for black board %s",BLACKBOARD_NAME.c_str());
		...
		void APEX_BLACKBOARD_TYPE::WRITE_ON_BLACKBOARD( MESSAGE_ADDR_TYPE MESSAGE_ADDR, MESSAGE_SIZE_TYPE LENGTH,RETURN_CODE_TYPE *RETURN_CODE)
		{
		...
		void APEX_BLACKBOARD_TYPE::READ_FROM_BLACKBOARD(MESSAGE_ADDR_TYPE MESSAGE_ADDR, MESSAGE_SIZE_TYPE* LENGTH,RETURN_CODE_TYPE *RETURN_CODE)
		{
			this->rw_lock.rdlock();
