...
#define __GET_SINGLETON_INSTANCE_AND_CREATE_IF_DOES_NOT_EXIST \
	APEX* partition_instance = (APEX::_pinstance==0)?APEX::getInstance():APEX::_pinstance; \
	if (partition_instance ==NULL) \
	{\
		*RETURN_CODE=INVALID_CONFIG;\
		return;\
...
namespace ACM_INTERNAL{
	//! the Partition class. All methods and members of this class are static. No Instance of this class is ever created or destroyed.
	class APEX
	{
	private:
		//! An internal struct. It holds necessary information for process scheduling.
		struct PROCESS_DESCRIPTOR{		
			SYSTEM_TIMESPEC ABSOLUTE_CRITICAL_TIME; //!< The ansolute time stamp used in scheduling
			ACM_INTERNAL::APEX_EVENT_TYPE SUSPENSION_EVENT; //!< It is used if the process calls suspend_self. Suspend call (not self) is implemented using priorities			
			ACM_INTERNAL::PROCESS* THE_PROCESS; //!< The real process.
		
			enum NEXT_SCHEDULING_INSTANCE_TYPE {RELEASE_PROCESS=0,CHECK_DEADLINE=1};
			NEXT_SCHEDULING_INSTANCE_TYPE NEXT_SCHEDULING_INSTANCE; //!<This tells wether the next critical scheduling instance is a release kind or check deadline kind
			SYSTEM_TIMESPEC ABSOLUTE_PREV_START_TIME; //!< Used to record previous start time. Only for history
			SYSTEM_TIMESPEC ABSOLUTE_NEXT_SCHEDULED_TIME; //!< Used to record next release time. Only used for periodic processes. For aperiodic processes. Next scheduled time= Next critical time
			...
			HM_DESCRIPTOR_ID_TYPE HM_DESCRIPTOR_ID; //!< Used to store the descriptor ID for the HM
			bool IS_HEALTH_MANAGER; //!< This flag is set to true for health managers.
			BLACKBOARD_ID_TYPE HM_RESPONSE_BOARD_ID; //!< Used to store the blackboard id for getting HM response back
			
			BLACKBOARD_ID_TYPE PROCESS_NOTIFICATION_RESPONSE_BOARD_ID; //!< Used to store the blackboard id for getting process response back

			
			PROCESS_DESCRIPTOR(ACM_INTERNAL::PROCESS* aep,PROCESS_NAME_TYPE Name)
				: SUSPENSION_EVENT(std::string(Name)+"SuspensionEvent"),
				THE_PROCESS(aep),
				NEXT_SCHEDULING_INSTANCE(RELEASE_PROCESS),
				INITIAL_DELAY_TIME(0),
				HM_DESCRIPTOR_ID(-1),
				IS_HEALTH_MANAGER(false),
			...
			inline std::string toString(){
				...
				temp<<"\t:NAME:"<<this->THE_PROCESS->get_name()<<"\t:PROCESS_STATE:"<<this->THE_PROCESS->get_string_state();
				temp<<"\t:Priority:"<<this->THE_PROCESS->get_priority()<<"\t:ABSOLUTE_CRITICAL_TIME:"<<convertTimeSpecToNano(ABSOLUTE_CRITICAL_TIME);
				temp<<"\t:ABSOLUTE_PREV_START_TIME:"<<convertTimeSpecToNano(ABSOLUTE_PREV_START_TIME);
				temp<<"\t:NEXT_SCHEDULING_INSTANCE:"<<((NEXT_SCHEDULING_INSTANCE==0)?"RELEASE_PROCESS":"CHECK_DEADLINE");
				return temp.str();
			}
			...
			~PROCESS_DESCRIPTOR(){
				...
				if (THE_PROCESS!=0) delete (THE_PROCESS);
			}
	...
	private:
		...
		static ACM_INTERNAL::MUTEX singletonInstanceMutex; //!<Used to protect the singleton instance
		ACM_INTERNAL::MUTEX PROCESS_TABLE_MUTEX; //!< Used to protect the access to process table while scheduling
		ACM_INTERNAL::APEX_EVENT_TYPE* APEX_SCHEDULING_EVENT; //!<Used by the scheduler
		
		enum  PRIORITY_LEVEL_TYPE {LOW_LEVEL=0,HIGH_LEVEL};
		PRIORITY_LEVEL_TYPE PRIORITY_LEVEL;
		static bool moduleManagerIsParent; //!< This flag is true if running inside a module manager. other wise it is false.
		static int sigIntCount;
		OPERATING_MODE_TYPE CURRENT_OPERATING_MODE; //!<Current Partition Operating Mode
		bool flagToTerminate; //!< This flag is set after signal SIGINT is received
		//! \name Internal Data Structures for holding the various entities. Id's are function of the vector index
		typedef std::vector<PROCESS_DESCRIPTOR*> PROCESS_DESCRIPTOR_VECTOR_TYPE;
		PROCESS_DESCRIPTOR_VECTOR_TYPE PROCESS_DESCRIPTOR_VECTOR; //!< All process descriptors
		PROCESS_DESCRIPTOR_VECTOR_TYPE PROCESS_SCHEDULING_SORTED_VECTOR;//<! This vector is especially used for sorting through schedules;
		typedef std::vector<ACM_INTERNAL::APEX_EVENT_TYPE*> EVENT_VECTOR_TYPE;
		EVENT_VECTOR_TYPE EVENT_VECTOR;
		typedef std::vector<ACM_INTERNAL::APEX_BLACKBOARD_TYPE*> BLACKBOARD_VECTOR_TYPE;
		...
		typedef std::map<PROCESS_ID_TYPE,ACM_INTERNAL::PROCESS::ThreadID*> PROCESS_ID_TO_PTHREAD_ID_MAP_TYPE;
		PROCESS_ID_TO_PTHREAD_ID_MAP_TYPE	PROCESS_ID_TO_PTHREAD_ID_MAP; //<!Used to map internal process id to actual pthread id. used for get_my_id
	...
	private:
		...
		static void CLEAR_VECTORS();//!<Used to delete all the internal data structures except PROCESS
		int	  InitPartitionScheduler(/*!out*/ RETURN_CODE_TYPE *RETURN_CODE);
		void  RunPartitionScheduler();
		void  DoSchedulabilityAnalysis();//<! This Method is a placeholder. Will be possiblly implemented in future.
		...
		/*!
		* This method is used to suspend the execution of this process till a resume is called by someone.
...
*/
	...
	private :
		...
		* 
		*/
		inline static bool PROCESS_DESCRIPTOR_COMP(const PROCESS_DESCRIPTOR* first, const PROCESS_DESCRIPTOR* second)  
		{
			assert(first!=0);
			assert(second!=0);
			...
			if((first->THE_PROCESS->get_state() == ACM_INTERNAL::PROCESS::DORMANT) && (second->THE_PROCESS->get_state() == ACM_INTERNAL::PROCESS::DORMANT))
				return false;
			...
			return compareTime(first->ABSOLUTE_CRITICAL_TIME, second->ABSOLUTE_CRITICAL_TIME);
		...
		inline static void APEX_HELPER_CLOCK_NANOSLEEP(SYSTEM_TIME_TYPE TIME_OUT){
			...
			if(_pinstance ==0){			
				...
				//LOGGER(INFO,"***************USING CONDITION VARIABLE INSIDE SCHEDULER FOR WAITING********");
		...
		//<! Will return true if it has timed out. otherwise if somebody has set the event it will return false.
		inline static bool APEX_SCHEDULER_WAIT_ABSOLUTE_TIME(SYSTEM_TIMESPEC TIME_OUT)
		{
			//LOGGER(INFO,"***************USING CONDITION VARIABLE INSIDE SCHEDULER FOR WAITING********");
			RETURN_CODE_TYPE returncode;
			_pinstance->APEX_SCHEDULING_EVENT->WAIT_AND_LOCK_EVENT(TIME_OUT,&returncode);
		...
		inline void sortProcessDescriptorVector()
		{
			...
			std::sort(this->PROCESS_SCHEDULING_SORTED_VECTOR.begin(),this->PROCESS_SCHEDULING_SORTED_VECTOR.end(),APEX::PROCESS_DESCRIPTOR_COMP);
		}
		...
		static void logProcessDescriptorVector(bool sortfirst=false){
			...
			std::stringstream temp;
			temp<<std::endl;
			for (unsigned int i=0;i<_pinstance->PROCESS_SCHEDULING_SORTED_VECTOR.size();i++){
				temp<<i<<") "<<_pinstance->PROCESS_SCHEDULING_SORTED_VECTOR[i]->toString();
				temp<<std::endl;						
			}
	...
	public:
		...
		*/
		...
		static void GET_PROCESS_STATUS (
			...
			/*!out*/ RETURN_CODE_TYPE *RETURN_CODE );
		static void GET_MY_NAME(/*!out*/  PROCESS_NAME_TYPE * PROCESS_NAME,/*!out*/RETURN_CODE_TYPE *RETURN_CODE );
	...
	static void GET_PROCESS_NOTIFICATION_BOARD_ID(/*!in */ PROCESS_ID_TYPE PROCESS_ID,
		/*!out*/BLACKBOARD_ID_TYPE *TARGET_BOARD_ID,/*!out*/RETURN_CODE_TYPE *RETURN_CODE);
		...
		*/
		...
		static void GET_TIME (
			...
			/*!out*/ RETURN_CODE_TYPE *RETURN_CODE );
		/*! Replenish is a misnomer. According to arinc spec. new_deadline=current_time+budget_time; effectively, new deadline can be less than current deadline
		*/
...
static void RAISE_APPLICATION_ERROR_AND_GET_RESPONSE (	/*!in */ ERROR_CODE_TYPE ERROR_CODE,
			/*!in */ MESSAGE_ADDR_TYPE MESSAGE_ADDR,
		...
		//! HEALTH Manager process is aperiodic. It runs with the highest prority in the partition. Waits on the buffer.
		static void CREATE_HEALTH_MANAGER_PROCESS(/*!in*/SYSTEM_ADDRESS_TYPE ENTRY_POINT,
			/*!in*/PROCESS_NAME_TYPE PROCESS_NAME,	
			/*!in*/STACK_SIZE_TYPE STACK_SIZE,
		...
		static void START_HM (
			...
			/*!out*/ RETURN_CODE_TYPE *RETURN_CODE );
		static void HM_GET_PROCESS_ERROR_MESSAGE(/*!out*/ERROR_STATUS_TYPE* ERROR_STATUS,
			/*!out*/BLACKBOARD_ID_TYPE* TARGET_BBOARD_ID,
			/*!out*/RETURN_CODE_TYPE* RETURN_CODE);
		static void HM_GET_MY_DESCRIPTOR_ID(/*!OUT*/HM_DESCRIPTOR_ID_TYPE *HM_DESCRIPTOR_ID,
			/*!out*/RETURN_CODE_TYPE *RETURN_CODE);
		static void REGISTER_PROCESS_WITH_HEALTH_MANAGER(/*!in*/PROCESS_ID_TYPE PROCESS_ID,
			/*!in*/HM_DESCRIPTOR_ID_TYPE HM_DESCRIPTOR_ID,
			/*!out*/BLACKBOARD_ID_TYPE *BLACKBOARD_ID, 
			/*!out*/ RETURN_CODE_TYPE *RETURN_CODE);
		static void HM_GET_MY_BUFFER_ID(/*!OUT*/BUFFER_ID_TYPE *BUFFER_ID,
			/*!out*/RETURN_CODE_TYPE *RETURN_CODE);
		static void HM_NOTIFY_PROCESS(/*!in*/PROCESS_ID_TYPE PROCESS_ID,/*!in*/HM_RESPONSE_TYPE *HM_RESPONSE,/*!out*/RETURN_CODE_TYPE *RETURN_CODE);
		static void GET_PROCESS_HM_BOARD_ID(/*!in */ PROCESS_ID_TYPE PROCESS_ID,/*!out*/BLACKBOARD_ID_TYPE *HM_BOARD_ID,/*!out*/RETURN_CODE_TYPE *RETURN_CODE);
		...
		static void READ_CONTENTS_TO_DATA_BUFFER(SAMPLING_PORT_ID_TYPE SAMPLING_PORT_ID,MESSAGE_ADDR_TYPE msg_ptr,MESSAGE_SIZE_TYPE  MAX_LENGTH,MESSAGE_SIZE_TYPE*  READ_LENGTH,RETURN_CODE_TYPE* RETURN_CODE);
		static void WRITE_CONTENTS_FROM_DATA_BUFFER(SAMPLING_PORT_ID_TYPE SAMPLING_PORT_ID,MESSAGE_ADDR_TYPE msg_ptr,MESSAGE_SIZE_TYPE  LENGTH,RETURN_CODE_TYPE* RETURN_CODE);
		...
		static void SEND_QUEUING_MESSAGE (
			...
			/*in */ SYSTEM_TIME_TYPE TIME_OUT,
			/*out*/ RETURN_CODE_TYPE *RETURN_CODE);
		static void MESSAGE_QUEUE_SEND_MESSAGE_FROM_BUFFER(QUEUING_PORT_ID_TYPE QUEUING_PORT_ID,
		MESSAGE_ADDR_TYPE msg_ptr,MESSAGE_SIZE_TYPE LENGTH, SYSTEM_TIME_TYPE TIME_OUT,RETURN_CODE_TYPE* RETURN_CODE);
 ...
 /// @cond IGNORE_DOXY
		...
		///*!Set Process Argument. This will set the corba request to the process identified by the id. 
		//This request should return an INVALID_MODE if the process in the READY_RUNNING state. This request is only valid for aperiodic processes.*/
