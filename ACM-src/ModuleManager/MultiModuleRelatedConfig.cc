...
void MODULE_MANAGER::PREPARE_PARENT_CHILD_INFORMATION() {
  ...
  std::pair<PropertyMapIteratorType, PropertyMapIteratorType> searchPair =
      this->PropertyMap.equal_range(SEARCH_STRING);
  if (searchPair.first == searchPair.second) {
    ...
    if (value.compare("TRUE") != 0) {
      LOGGER(INFO, "%s is  set to %s. Not running in synchronized mode",
             PRIMARY_KEY_SYNCHRONIZED_EXECUTION,
             (searchPair.first->second).c_str());
      return;
    }
  }

  LOGGER(INFO, "%s is  set to %s. running in synchronized mode",
         PRIMARY_KEY_SYNCHRONIZED_EXECUTION,
         (searchPair.first->second).c_str());
  /////////////////////////////////////////////////////
  ...
  if (this->isMulticast && this->isBroadcast) {
    LOGGER(FATAL, "Cannot specify both %s and %s",
           PRIMARY_KEY_MULTICAST_ADDRESS, PRIMARY_KEY_BROADCAST_ADDRESS);
    throw(-3);
  }
  ...
  if (!(this->isMulticast) && !(this->isBroadcast)) {
    LOGGER(
        FATAL,
        "Should specify either %s or %s. Cannot Proceed in synchronized mode",
        PRIMARY_KEY_MULTICAST_ADDRESS, PRIMARY_KEY_BROADCAST_ADDRESS);
    throw(-3);
  }
  ...
  if (searchPair.first != searchPair.second) {
    ...
    if (!from_string<unsigned int>(this->CONFIGURATION_PORT_NUM, tempvalue,
                                   std::dec) ||
        this->CONFIGURATION_PORT_NUM == 0) {
      LOGGER(FATAL, "Incorrect%s %s.", PRIMARY_KEY_CONFIGURATION_PORT,
             tempvalue.c_str());
      throw(-2);
    }
  } else {
    LOGGER(ERROR, "CONFIGURATION_PORT_NUM Not Defined. Cannot Use for "
  ...
  if (searchPair.first != searchPair.second) {
    ...
    // Found configuration port
    std::string tempvalue = searchPair.first->second;
    if (!from_string<unsigned int>(this->PARENT_PORT, tempvalue, std::dec) ||
        this->PARENT_PORT == 0) {
      LOGGER(FATAL, "Incorrect%s %s.", PRIMARY_KEY_PARENT_PORT,
             tempvalue.c_str());
      throw(-2);
    }
  } else {
    LOGGER(ERROR,
           "PARENT_PORT Not Defined. Cannot Use for synchronized start up");
    throw(-2);
  }
  ...
  if (searchPair.first == searchPair.second) {
    LOGGER(ERROR,
           "MODULE_NAME Not Defined. Cannot Use for synchronized start up");
    throw(-2);
  } else {
  ...
  if (searchPair.first == searchPair.second) {
    LOGGER(INFO,
           "Parent Module Not Set. This must be the system module manager");
    ...
    for (PropertyMapIteratorType iter = searchPair.first;
      ...
      std::string SEARCH_STRING_CHILD =
          temp.childname + ADDITVE_KEY_MM_HOSTNAME;
      ...
      if (searchPairHostname.first == searchPairHostname.second) {
        LOGGER(ERROR, "Could Not find entry for %s",
               SEARCH_STRING_CHILD.c_str());
        throw(-3);
      } else {
    ...
    LOGGER(INFO, "Parent Module  Set. This is not the system module manager. "
    ...
    searchPair = this->PropertyMap.equal_range(SEARCH_STRING);
    if (searchPair.first != searchPair.second) {
      LOGGER(ERROR, "Invalid COnfiguration. Both parent and Child Are set. "
                    "Ignoring Children Modules");
      throw(-3);
    }
  }
}
...
bool MODULE_MANAGER::GET_READY_MM_FOR_HYPERPERIOD() {
  ...
  if (this->isSystemModuleManager) {
    ...
    while (this->numchildrensyncremaining() > 0) {
      ...
      recvMsgSize = this->Recv_Socket->recvFrom(&(BUFFER[0]), SyncTypeSize,
                                                sourceAddress, sourcePort);
      if (recvMsgSize != SyncTypeSize)
        continue;
      else {
        ACM_INTERNAL::SocketSynchronizationMessageType::FROM_SERIAL_BUFFER(
            SocketSynchronizationMessage, BUFFER);
        if (SocketSynchronizationMessage.Sync !=
            SocketSynchronizationMessageType::BARRIER_READY)
          continue;
    ...
    ACM_INTERNAL::SocketSynchronizationMessageType::SERIAL_BUFFER_TYPE BUFFER;
    ACM_INTERNAL::SocketSynchronizationMessageType SocketSynchronizationMessage(
        ACM_INTERNAL::SocketSynchronizationMessageType::BARRIER_READY,
        this->MODULE_NAME, this->PARENT_NAME);
    ACM_INTERNAL::SocketSynchronizationMessageType::TO_SERIAL_BUFFER(
        BUFFER, SocketSynchronizationMessage);
    this->Send_Socket->sendTo(&BUFFER, SyncTypeSize, this->PARENT_HOST,
                              this->PARENT_PORT);
  }
...
void MODULE_MANAGER::DETERMINE_AVERAGE_NETWORK_LATENCY() {
  ...
  if (this->isSystemModuleManager) {
    ...
    // Child Sends ACK
    // Parent Sends Ack
    ACM_INTERNAL::SocketSynchronizationMessageType SocketSyncMessage(
        ACM_INTERNAL::SocketSynchronizationMessageType::NETWORK_LATENCY_READY,
        this->MODULE_NAME, this->PARENT_NAME);
    ACM_INTERNAL::SocketSynchronizationMessageType::SERIAL_BUFFER_TYPE BUFFER;
    ACM_INTERNAL::SocketSynchronizationMessageType::TO_SERIAL_BUFFER(
        BUFFER, SocketSyncMessage);

    for (std::map<std::string, child_url>::iterator iter =
             childrenModules.begin();
         iter != childrenModules.end(); iter++) {
      SocketSyncMessage.Sync =
          SocketSynchronizationMessageType::NETWORK_LATENCY_READY;
      ACM_INTERNAL::SocketSynchronizationMessageType::TO_SERIAL_BUFFER(
          BUFFER, SocketSyncMessage);
      this->Send_Socket->sendTo(&BUFFER, SyncTypeSize, iter->second.hostname,
                                iter->second.portnum);

      // Wait for Ready
      ACM_INTERNAL::SocketSynchronizationMessageType
          SocketSynchronizationMessage;
      LOGGER(INFO, "DETERMINE_AVERAGE_NETWORK_LATENCY: Sent "
                   "NETWORK_LATENCY_READY to %s. Wait for "
                   "NETWORK_LATENCY_CHILD_ACK.",
             iter->second.childname.c_str());
      int recvMsgSize = 0;
      string sourceAddress;      // Address of datagram source
      unsigned short sourcePort; // Port of datagram source
      ...
      while (!receivedProper) {
        ...
        while (recvMsgSize != SyncTypeSize) {
          recvMsgSize = this->Recv_Socket->recvFrom(&(BUFFER[0]), SyncTypeSize,
                                                    sourceAddress, sourcePort);
          if (recvMsgSize != SyncTypeSize) {
            LOGGER(ERROR, "DETERMINE_AVERAGE_NETWORK_LATENCY: Bad "
                          "NETWORK_LATENCY_READY from %s.",
                   iter->second.childname.c_str());
          }
        }
        ...
        ACM_INTERNAL::SocketSynchronizationMessageType::FROM_SERIAL_BUFFER(
            SocketSynchronizationMessage, BUFFER);
        if (SocketSynchronizationMessage.Sync ==
            SocketSynchronizationMessageType::NETWORK_LATENCY_CHILD_ACK)
          receivedProper = true;
      }
      ...
      if (SocketSynchronizationMessage.Sync ==
        ...
        LOGGER(INFO, "DETERMINE_AVERAGE_NETWORK_LATENCY: Received "
                     "NETWORK_LATENCY_CHILD_ACK ");
        LOGGER(INFO, "DETERMINE_AVERAGE_NETWORK_LATENCY: Sending "
                     "NEWORK_LATENCY_PARENT_ACK ");

        SocketSyncMessage.Sync =
            SocketSynchronizationMessageType::NEWORK_LATENCY_PARENT_ACK;
        ACM_INTERNAL::SocketSynchronizationMessageType::TO_SERIAL_BUFFER(
            BUFFER, SocketSyncMessage);
        this->Send_Socket->sendTo(&BUFFER, SyncTypeSize, iter->second.hostname,
                                  iter->second.portnum);
    ...
    SocketSyncMessage.Sync =
        ACM_INTERNAL::SocketSynchronizationMessageType::START;
    ACM_INTERNAL::SocketSynchronizationMessageType::TO_SERIAL_BUFFER(
        BUFFER, SocketSyncMessage);
    LOGGER(INFO, "SENDING Go After Latency");
    this->Send_Socket->sendTo(&BUFFER, SyncTypeSize, this->broadcastaddress,
                              this->CONFIGURATION_PORT_NUM);
  } else {
    ACM_INTERNAL::SocketSynchronizationMessageType syncrecv;
    // Wait to Receive ACK from parent
    ACM_INTERNAL::SocketSynchronizationMessageType::SERIAL_BUFFER_TYPE BUFFER;
    while (strcmp(syncrecv.MESSAGE_SRC_MODULE, this->PARENT_NAME.c_str()) !=
           0) {
      int recvMsgSize = 0;
      string sourceAddress;      // Address of datagram source
      unsigned short sourcePort; // Port of datagram source
      ...
      if (recvMsgSize != SyncTypeSize || errno == EAGAIN ||
          errno == EWOULDBLOCK) {
      ...
      if (strcmp(syncrecv.MESSAGE_SRC_MODULE, this->PARENT_NAME.c_str()) != 0) {
        LOGGER(ERROR, "RECEIVED WRONG ENTRY %s:%s", this->PARENT_NAME.c_str(),
               syncrecv.MESSAGE_SRC_MODULE);
        syncrecv.printout();
      }
    }
    ...
    if (syncrecv.Sync !=
        SocketSynchronizationMessageType::NETWORK_LATENCY_READY) {
      LOGGER(ERROR, "RECEIVED WRONG synchronization message type. expecting "
    ...
    while (strcmp(syncrecv.MESSAGE_SRC_MODULE, this->PARENT_NAME.c_str()) !=
      ...
      if (recvMsgSize != SyncTypeSize || errno == EAGAIN ||
          errno == EWOULDBLOCK) {
        LOGGER(ERROR,
               "Error in Receiving NEWORK_LATENCY_PARENT_ACK from Parent");
      }
      ACM_INTERNAL::SocketSynchronizationMessageType::FROM_SERIAL_BUFFER(
          syncrecv, BUFFER);
    ...
    if (syncrecv.Sync !=
        SocketSynchronizationMessageType::NEWORK_LATENCY_PARENT_ACK) {
      LOGGER(ERROR,
             "RECEIVED WRONG ENTRY. expected NEWORK_LATENCY_PARENT_ACK ");
      syncrecv.printout();
      return;
    } else {
      this->AVE_NETWORK_LATENCY_FROM_PARENT = reciept - current;
      LOGGER(INFO, "Network Latency in NS %lld",
             this->AVE_NETWORK_LATENCY_FROM_PARENT);
    }
    ...
    while (true) {
      ...
      while (strcmp(syncrecv.MESSAGE_SRC_MODULE, this->PARENT_NAME.c_str()) !=
        ...
        ACM_INTERNAL::SocketSynchronizationMessageType::FROM_SERIAL_BUFFER(
            syncrecv, BUFFER);
        if (strcmp(syncrecv.MESSAGE_SRC_MODULE, this->PARENT_NAME.c_str()) !=
            0) {
...
bool MODULE_MANAGER::SYNC_WITH_OTHER_MODULE_MANAGERS() {
  ...
  if (this->isSystemModuleManager) {
    ...
    // Broadcase
    ACM_INTERNAL::SocketSynchronizationMessageType SocketSyncMessage(
        ACM_INTERNAL::SocketSynchronizationMessageType::BARRIER_ACK,
        this->MODULE_NAME, this->PARENT_NAME);
    ACM_INTERNAL::SocketSynchronizationMessageType::TO_SERIAL_BUFFER(
        BUFFER, SocketSyncMessage);
...
void MODULE_MANAGER::SEND_HYPER_PERIOD_ECHO(bool Stop) {
  if (!(this->isSystemModuleManager))
    return;
  static ACM_INTERNAL::SocketSynchronizationMessageType::SERIAL_BUFFER_TYPE
      BUFFER;
  static ACM_INTERNAL::SocketSynchronizationMessageType
      SocketSynchronizationMessage(
          ACM_INTERNAL::SocketSynchronizationMessageType::START,
          this->MODULE_NAME, this->PARENT_NAME);

  if (Stop)
    SocketSynchronizationMessage.Sync =
        ACM_INTERNAL::SocketSynchronizationMessageType::STOP;
