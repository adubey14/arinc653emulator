/* Used to test the internal methods used by the Module Manager for sampling
 * port channels */
...
int main() {
  ...
  SAMPLING_PORT_HELPER SP("/temp1", 11, PORT_SOURCE_DEST, convertSecsToNano(5),
                          &RETURN_CODE, O_CREAT);
  ...
  char buf2[11];
  SP.READ_SAMPLING_MESSAGE((MESSAGE_ADDR_TYPE)&buf2[0], &LENGTH, &VALIDITY,
                           &RETURN_CODE);
  ...
  std::cout << "Last Message Validity is "
  ...
  SAMPLING_PORT_HELPER SP2("/temp2", 11, PORT_SOURCE_DEST, convertSecsToNano(5),
                           &RETURN_CODE, O_CREAT);
  assert(RETURN_CODE == NO_ERROR);
