...
enum TFPG_MONITOR_ALARM_TYPE {NO_DEFINED_ALARM=0,
...
//! COMPONENT_OPERATIONAL_STATE_TYPE enum captures the current operational state of a component.  COMPONENT_INACTIVE implies none of the Component processes can be started. COMPONENT_SEMIACTIVE implies the output processes (publisher/provides interface) cannot be started. COMPONENT_ACTIVE allows any component process to be started.
enum COMPONENT_OPERATIONAL_STATE_TYPE {COMPONENT_ACTIVE=0,
...
{
	enum HM_ACTION_ENUM_TYPE{REFUSE=0,IGNORE=1,SUSPEND=2,STOP=3,START=4, USE_PAST_DATA=5,USER_SPECIFIC_RESPONSE=6};
	...
	APEX_BYTE HM_MESSAGE[ERROR_RESPONSE_TYPE_MESSAGE_SIZE]; //! This can potentially carry application SPECIFIC DATA
...
{
	//! STOP_ALL makes Component INACTIVE,  START_ALL makes Component ACTIVE, DISABLE_OUTPUT makes Component SEMI_ACTIVE,USE_REQUIRE with REQUIRE_RECONFIG_INFO_TYPE as the HM_MESSAGE reconfigures the requires port
	enum SYSTEM_HM_ACTION_ENUM_TYPE{STOP_ALL=0,START_ALL=1, RESET_ALL=2,CHECKPOINT=3, RESTORE=4, DISABLE_OUTPUT=5,USE_REQUIRE=6};
	SYSTEM_TIME_TYPE TIMEOFMESSAGE;		
...
//! SYSTEM_HM_INPUT_TYPE - Component HM sends this information to the supervisor - SystemHM
