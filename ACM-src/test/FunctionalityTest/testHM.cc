...
void HMprocess(void)
{
	while(1){
		...
		HM_GET_PROCESS_ERROR_MESSAGE(&ERROR_STATUS,&TARGET_BBOARD_ID,&RETURN_CODE);
		std::cout<<ERROR_STATUS.LENGTH<<" bboard id "<<TARGET_BBOARD_ID<<std::endl;
		...
		HM_RESPONSE.HM_MESSAGE_LENGTH= (APEX_UNSIGNED)strlen((char*)HM_RESPONSE.HM_MESSAGE);
		HM_NOTIFY_PROCESS(/*!in*/ERROR_STATUS.FAILED_PROCESS_ID,/*!in*/&HM_RESPONSE,/*!out*/&RETURN_CODE);

		//DISPLAY_BLACKBOARD (TARGET_BBOARD_ID,(APEX_BYTE*)display_message,sizeof(display_message),&RETURN_CODE);
...
void anotherperiodic(void){
	...
	if(i==2 || i==4 || i==6 ||i==8 || i==10)
	{
		...
		RAISE_APPLICATION_ERROR_AND_GET_RESPONSE (	/*!in */ APPLICATION_ERROR,
			/*!in */ MESSAGE_ADDR_TYPE (MESSGAE),
			/*!in */ sizeof(MESSGAE),
			/*!in */ INFINITE_TIME_VALUE, //Added Timeout in response to the discussion with Gabor
			/*!out*/ &HM_RESPONSE,
			/*!out*/ &RETURN_CODE );
		...
		//RAISE_APPLICATION_ERROR (/*!in */ APPLICATION_ERROR,/*!in */ MESSAGE_ADDR_TYPE (MESSGAE),/*!in */ sizeof(MESSGAE),/*!out*/ &RETURN_CODE );
		/*
		MESSAGE_SIZE_TYPE length;
		...
		*/
		if(HM_RESPONSE.HM_MESSAGE_DISCRIMINATOR==STRING)
			LOGGER(APP,"FROM another periodic. reply from HM %s",(char*)HM_RESPONSE.HM_MESSAGE);
...
int main(int argc, char* argv []){
	...
	REGISTER_PROCESS_WITH_HEALTH_MANAGER(PERIODIC_PROCESS_ID_2,HM_DESCRIPTOR_ID,&BLACKBOARD_ID, /*out*/ &RETURN_CODE);
