...
namespace ACM_INTERNAL{
	...
	PROCESS::PROCESS_state PROCESS::get_state() const {
		return state;
	}
	...
	void PROCESS::waiting_on_period(/*out*/ACM_USER::RETURN_CODE_TYPE* RETURN_CODE)	
	{
		if (!this->isPeriodic())
		{
	...
	void PROCESS::set_state (PROCESS_state s){
		state=s;
	}
	void PROCESS::post_state_change(){
		sem_state.post();
	}
	...
	/*! We have to get rid of all the new statements and replace it with an own memory manager
	*/
	PROCESS::PROCESS(PROCESS_ATTRIBUTE_TYPE& attr,PROCESS_ID_TYPE unique_process_id): 
	process_id(unique_process_id),
		attribute(attr),
		thread_detached(PROCESS::NOT_DETACHED) ,
		state(DORMANT),
...

	{
		...
		std::string Message("From PROCESS's constructor: A null PROCESS attribute pointer passed to the constructor");
		...
		if (attribute.ENTRY_POINT==0 || attribute.ENTRY_POINT==NULL){
			//We are in a bad situation. Throw an execption
			std::string Message("From PROCESS's constructor: NULL ENTRY_POINT in PROCESS_attribute is Illegal");
	...
	PROCESS::~PROCESS(){
		...
		//LOGGER(ACM_USER::DEBUG,"Inside PROCESS %s Destructor. Thread has died. Wait Finished.",Name.c_str());	
	}
	...
	void PROCESS::restart(bool doMitigation){
		...
		this->terminate();	
		//LOGGER (ACM_USER::DEBUG,"Waiting for the ArincEmulatorThread to terminate");	
		void *status;
		this->wait( &status );	
		...
		if(result!=PROCESS::NO_ERROR){
			LOGGER( ERROR,"Problem while restarting %s: Return From create_thread: code is %d", this->attribute.NAME,result );
			return;
		}
		...
		if (this->isPeriodic() && doMitigation){
			...
			if ( RETURN_CODE != ACM_USER::NO_ERROR )
			{
				...
				LOGGER( ERROR,"APEX::START failed while restarting process %s:And the error code is %d", this->attribute.NAME,RETURN_CODE );
	...
	*/
	...
	void PROCESS::delayed_start (SYSTEM_TIME_TYPE timeout){
		if(this->get_state()==PROCESS::READY){		
			...
			//RESET Their Event
			RETURN_CODE_TYPE RETURN_CODE;
			APEX_HELPER_RESET_EVENT(this->attribute.NAME,&RETURN_CODE); //<- change this to write to blackboard.....
			this->sem_state.post(); //Post the state Change Message
		}
	}
	...
	void PROCESS::stop(bool restart){ //Will cause a restart of the thread
		...
		//this->setException(new ACM::ACM_TIMING_EXCEPTION_TYPE(std::string("Timing Failure in Process Named")+this->attribute_ptr->NAME));
	...
	void PROCESS::set_priority(long priority ){
		...
		if(schedRetValue!=0)
		{
			LOGGER(ERROR,"pthread_setschedparam at %d for thread_id %d. return code is %d",
	...
	void  PROCESS::Thread_cleanup_Handler (void* data)
	{
		...
		for(std::list<SEMAPHORE_ID_TYPE>::iterator iter=current->READLOCKS.begin();iter!=current->READLOCKS.end();iter++)
		{
			LOGGER(HMEVENT,"PROCESS %s stopped. But it still holds READ LOCK to sempahore id %ld. Unlocking it.",current->attribute.NAME,*iter);
		...
		for(std::list<SEMAPHORE_ID_TYPE>::iterator iter=current->WRITELOCKS.begin();iter!=current->WRITELOCKS.end();iter++)
		{
			LOGGER(HMEVENT,"PROCESS %s stopped. But it still holds WRITE LOCK to sempahore id %ld. Unlocking it.",current->attribute.NAME,*iter);
			SEMAPHORE_ID_TYPE SEM_ID=*iter;
			//iter++;
			SEM_ID_VEC.push_back(SEM_ID);
		...
		//	LOGGER(APP,"pthread Cleanp for process %s : id %ld",current->attribute.NAME,my_id);	std::cout<<PROCESS::self()<<std::endl;
		//std::cout<<"MY ID IS "<<my_id<<std::endl;

		// This is where we will call the unlock if required.
		//	LOGGER(ACM_USER::DEBUG,"pthread Cleanp for process"pthread Cleanp for process  %s. Specific cleanup code can can be called here.",current->attribute.NAME);
	...
	void PROCESS::run(){
		...
		do{
			sem_state.wait();//!Waiting for State to Change
			if (get_state()==PROCESS::RUNNING) { //State is Ready_Running. Do Some Work
				...
				if (this->delayed_start_flag)
				{
					...
					while ( clockresult != 0) 
					{
						clockresult = clock_nanosleep( CLOCK_REALTIME ,TIMER_ABSTIME, &(this->absolute_delay_start_time),&rem);
					}
				}
				...
				try
				{
					...
					strncpy((char*)&(PROCESS_RESPONSE.ERROR_RESPONSE.HM_MESSAGE[0]),ex.getMessage().c_str(),ERROR_RESPONSE_TYPE_MESSAGE_SIZE);
					PROCESS_RESPONSE.ERROR_RESPONSE.HM_MESSAGE_LENGTH=ERROR_RESPONSE_TYPE_MESSAGE_SIZE;//strlen(ex.getMessage().c_str());
					...
					PROCESS_RESPONSE.NO_ERROR=false;
					PROCESS_RESPONSE.ERROR_RESPONSE.HM_MESSAGE_DISCRIMINATOR=ACM_USER::STRING;
					strncpy((char*)&(PROCESS_RESPONSE.ERROR_RESPONSE.HM_MESSAGE[0]),ststring.c_str(),ERROR_RESPONSE_TYPE_MESSAGE_SIZE);
					PROCESS_RESPONSE.ERROR_RESPONSE.HM_MESSAGE_LENGTH=ERROR_RESPONSE_TYPE_MESSAGE_SIZE;//strlen(ex.getMessage().c_str());
				...
				// catch(...)
				// {
					// LOGGER(ACM_USER::ERROR,"Unknown Exception occured while doing the Run");
					// PROCESS_RESPONSE.NO_ERROR=false;
					// strncpy((char*)&(PROCESS_RESPONSE.ERROR_RESPONSE.HM_MESSAGE[0]),"Unknown Exception occured while doing the Run"
				...
				if( this->isPeriodic()){
					...
					//this->notificationMUTEX.unlock();
					RETURN_CODE_TYPE RETURN_CODE;
					APEX_HELPER_SET_EVENT(this->attribute.NAME,&RETURN_CODE);// Left for backward compatibility. We should use notification board now.
					this->set_state(PROCESS::DORMANT);
				}
				...
				//Check and update process response if there is an error_code in the HM blackboard.
...
#ifndef OPTIMAL
				APEX::GET_PROCESS_HM_BOARD_ID(this->process_id,&MY_HM_BOARD_ID,&RETURN_CODE);
				if(RETURN_CODE==ACM_USER::NO_ERROR)
				{
					//HM_BOARD is available
					BLACKBOARD_STATUS_TYPE MY_BOARD_STATUS;
					APEX::GET_BLACKBOARD_STATUS(MY_HM_BOARD_ID,&MY_BOARD_STATUS,&RETURN_CODE);
					if(MY_BOARD_STATUS.EMPTY_INDICATOR!=ACM_USER::EMPTY)
					{ //Read the board						
						MESSAGE_SIZE_TYPE HM_RESPONSE_LENGTH;
						APEX::READ_BLACKBOARD(MY_HM_BOARD_ID,0,(MESSAGE_ADDR_TYPE)&HM_RESPONSE,&HM_RESPONSE_LENGTH,&RETURN_CODE);
						if(RETURN_CODE==ACM_USER::NO_ERROR && HM_RESPONSE_LENGTH>0 && TIME_OF_START < HM_RESPONSE.TIMEOFMESSAGE)
						{
							memcpy(&(PROCESS_RESPONSE.ERROR_RESPONSE),&HM_RESPONSE,sizeof(HM_RESPONSE_TYPE));
							PROCESS_RESPONSE.NO_ERROR=false;
				...
				//					   /*!in */ (MESSAGE_SIZE_TYPE) sizeof(response),		
