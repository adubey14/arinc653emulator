...
void PROCESS_PERIODIC (void){
	...
	//Tests that no error is returned when correctly resuming a process
	SUSPEND(PROCESS_APERIODIC_ID, &PERIODIC_RETURN);
	RESUME(PROCESS_APERIODIC_ID, &PERIODIC_RETURN);
	if(PERIODIC_RETURN == NO_ERROR ) {
		LOGGER(APP,"Resume aperiodic test passed!");
		LOGGER(APP,"Return code is: %d" ,PERIODIC_RETURN);	
	}
	else {
		LOGGER(APP,"Resume aperiodic test failed.");
		LOGGER(APP,"Return code is: %d" ,PERIODIC_RETURN);	
	}

	//Tests that "NO_ACTION" is returned when the specified process is not suspended
	RESUME(PROCESS_APERIODIC_ID, &PERIODIC_RETURN);
	if(PERIODIC_RETURN == NO_ACTION ) {
		LOGGER(APP,"Already resumed test passed!");
		LOGGER(APP,"Return code is: %d" ,PERIODIC_RETURN);	
	}
	else {
		LOGGER(APP,"Already resumed test failed.");
		LOGGER(APP,"Return code is: %d" ,PERIODIC_RETURN);	
	}

	//Tests if the right return code is given when a process tries to resume a process that does not exist
	PROCESS_ID_TYPE PROCESS_NONEXISTENT_ID = 10;
	RESUME(PROCESS_NONEXISTENT_ID, &PERIODIC_RETURN);
...
void PROCESS_APERIODIC (void){
	...
	RESUME(PROCESS_APERIODIC_ID, &APERIODIC_RETURN);
	//Tests if the right return code is given when a process tries to resume itself
	if(APERIODIC_RETURN == INVALID_PARAM) {
		LOGGER(APP,"Don't resume self test passed!");
		LOGGER(APP,"Return code is: %d" ,APERIODIC_RETURN);	
	}
	else {
		LOGGER(APP,"Don't resume self test failed.");
		LOGGER(APP,"Return code is: %d" ,APERIODIC_RETURN);	
	}
	
	//Tests if the right return code is given when a process tries to resume a periodic process
	RESUME(PROCESS_PERIODIC_ID, &APERIODIC_RETURN);
	if(APERIODIC_RETURN == INVALID_MODE) {
		LOGGER(APP,"Don't resume periodic test passed!");
		LOGGER(APP,"Return code is: %d" ,APERIODIC_RETURN);	
	}
	else {
		LOGGER(APP,"Don't resume periodic test failed.");
		LOGGER(APP,"Return code is: %d" ,APERIODIC_RETURN);	
	}
