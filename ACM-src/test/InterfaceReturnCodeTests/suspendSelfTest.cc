...
void PROCESS_PERIODIC (void) {
	...
	if(RETURN_CODE == NO_ERROR) {
		...
		//Waits for the interval to time out and tests if the right return code is returned
...
PROCESS_PERIODIC_ATTR.BASE_PRIORITY=90; // This prioority should be more than the aperiodic priority. Since we are starting the aperiodic process, it is possible that it will take the resources away from the periodic process
