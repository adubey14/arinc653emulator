...
void PROCESS_APERIODIC (void){
	...
	//Tests that no error is returned when correctly setting an event
	SET_EVENT(SOME_EVENT_ID, &RETURN_CODE);		
	if(RETURN_CODE == NO_ERROR){
		LOGGER(APP,"Set event test Succeeded!");  
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);  
	}
	else{
		LOGGER(APP,"Set event test Failed."); 
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE); 
	}
	 
	EVENT_ID_TYPE NONEXISTENT_ID = 10;
	//Tests that the correct code is returned when atempting to set an event that does not exist
	SET_EVENT(NONEXISTENT_ID, &RETURN_CODE);
