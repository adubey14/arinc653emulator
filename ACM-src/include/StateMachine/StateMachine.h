...
namespace STATEMACHINE
{
	...
	public:
		...
		Transition(State* source, State* target, int priorityID, TRANSID id, bool setHistory=false);
	...
	public:
		...
		virtual void ApplyAndResetInputEvents()=0; // Resets current events. Apply input events as current event and reset input event list
		virtual void ApplyAndResetInternalEvents()=0;//Resets current events. Apply internal events as current event and reset input event list
		...
		virtual bool IsCurrentEventSet()=0; // Checks if there are any internal events set
		virtual void GetOutputEvents(map<string,set<Event*> >& events)=0; // Gets the list of Output Events generated 
		...
		virtual void clearTimeOutCalls(set<TRANSID>& setTimeOutTransIDs)=0;//clear any pending timeout calls
		virtual void EvaluateTimeOuts()=0;//evaluate new time-out calls
		virtual void GetTimeOutTransitionsToEvaluate(set<TRANSID>& timeOutTransitions) =0;//get the list of time out transitions
	...
	{
	public:
		StateMachine(string& name, DataSource* ds);//create state with null parent(region and state) and sets the statetype to OR
