...
int main(int argc, char **argv) {
  ...
  // SocketSynchronizationMessage(ACM_INTERNAL::SocketSynchronizationMessageType::START,"localhost","durip02",10);
  // SocketSynchronizationMessage.increment();

  // SocketSynchronizationMessage.printout();

  // ACM_INTERNAL::SocketSynchronizationMessageType::SERIAL_BUFFER_TYPE
  // SERIALBUFFER;

  // ACM_INTERNAL::SocketSynchronizationMessageType::TO_SERIAL_BUFFER(SERIALBUFFER,SocketSynchronizationMessage);

  // ACM_INTERNAL::SocketSynchronizationMessageType
  // SocketSynchronizationMessage2;
  ...
  try {
    ...
    if (argc == 1)
      LOGGER(WARNING, "Using Default configuration file %s",
             ACM_INTERNAL::MODULE_MANAGER::fileName.c_str());

    if (argc > 1) {
      std::string secondargument = argv[1];
      if (secondargument.compare("-?") == 0 ||
          secondargument.compare("-h") == 0 ||
          secondargument.compare("--help") == 0 ||
          secondargument.compare("-help") == 0) {
        cout << argv[0] << " <Partition Configuration File Name>\n or, -s for "
                           "printing configurations schema \n";
        return 0;
      }
      if (secondargument.compare("-s") == 0 ||
          secondargument.compare("--schema") == 0 ||
          secondargument.compare("-schema") == 0) {

        cout << argv[0]
             << "Here are the keys that can be used in a configuration file\n";
      ...
      if (argc > 2) {
        cout << "Ignoring additional commandline arguments. Only using "
             << argv[1] << " \n";
      }
    }
