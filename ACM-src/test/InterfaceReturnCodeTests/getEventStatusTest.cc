/*
Test of the GET_EVENT_STATUS service
*/
...
void PROCESS_APERIODIC (void){
	...
	EVENT_STATUS_TYPE GET_STATUS;
	//Tests that no error is returned when correctly getting an event's status
	GET_EVENT_STATUS(SOME_EVENT_ID,&GET_STATUS, &RETURN_CODE);		
	if(RETURN_CODE == NO_ERROR){
		LOGGER(APP,"Get event status test Succeeded!");  
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);  
	}
	else{
		LOGGER(APP,"Get event status test Failed."); 
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);
	}
	 
	EVENT_ID_TYPE NONEXISTENT_ID = 10;
	//Tests that the correct code is returned when attempting to get the status of an event that does not exist
	GET_EVENT_STATUS(NONEXISTENT_ID,&GET_STATUS, &RETURN_CODE);
