...
void PROCESS_APERIODIC (void){
	...
	RECEIVE_BUFFER(SOME_BUFFER_ID,convertSecsToNano(1),(APEX_BYTE*)(&myLoad),&length,&RETURN_CODE);
	...
	//Tests no error is returned when correctly when receiveing a message in a buffer
	...
	SEND_BUFFER(SOME_BUFFER_ID,(APEX_BYTE*)(&myLoad),sizeof(myLoad),convertSecsToNano(1),&RETURN_CODE);
	RECEIVE_BUFFER(SOME_BUFFER_ID,convertSecsToNano(315569261),(APEX_BYTE*)(&myLoad),&length,&RETURN_CODE);
	...
	//Tests that the correct code is returned when attempting to receive from a buffer that does not exist
	BUFFER_ID_TYPE NONEXISTENT_ID = 10;
	RECEIVE_BUFFER(NONEXISTENT_ID,convertSecsToNano(1),(APEX_BYTE*)(&myLoad),&length,&RETURN_CODE);
	if(RETURN_CODE == INVALID_PARAM){
