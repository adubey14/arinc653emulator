...
void PROCESS_APERIODIC(void) {
  ...
  DISPLAY_BLACKBOARD(BB_ID, (APEX_BYTE *)(&myLoad), sizeof(myLoad),
                     &RETURN_CODE);
  ...
  // Tests that the correct code is returned when attempting to display a
  // nonexistent blackboard
  BLACKBOARD_ID_TYPE NONEXISTENT_BB_ID = 10;
  DISPLAY_BLACKBOARD(NONEXISTENT_BB_ID, (APEX_BYTE *)(&myLoad), sizeof(myLoad),
                     &RETURN_CODE);
