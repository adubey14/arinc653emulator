...
void PROCESS_PERIODIC (void)
{
	...
	//Tests that no error is returned when correctly getting the partition's status
	if (RETURN_CODE == NO_ERROR)
	{
		LOGGER(APP,"Get partition status test passed!");
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);	
	}
	else
	{
		LOGGER(APP,"Get process status test failed.");
		LOGGER(APP,"Return code is: %d" ,RETURN_CODE);	
	}
