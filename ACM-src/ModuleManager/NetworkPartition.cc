...
static void FireChannels() {
  ...
  while (true) {
    ...
    RECEIVE_QUEUING_MESSAGE(
        ...
        /*out*/ (MESSAGE_ADDR_TYPE)&NETWORK_PARTITION_PETRI_NET_TOKEN_READ
            [0], // buf[0],
        /*out*/ &LENGTH,
        /*out*/ &RETURN_CODE);

    if (strcmp(NETWORK_PARTITION_PETRI_NET_TOKEN,
               NETWORK_PARTITION_PETRI_NET_TOKEN_READ) != 0)
      continue;
    ...
    for (unsigned counter = 0;
         counter < net_partition->QUEUEING_CHANNELS.size(); counter++) {
      if (main_instance->MAP_OF_QUEUEING_CHANNELS
              [net_partition->QUEUEING_CHANNELS[counter]] != 0) {
        main_instance
            ->MAP_OF_QUEUEING_CHANNELS[net_partition
                                           ->QUEUEING_CHANNELS[counter]]
            ->MOVE_MESSAGES(&RETURN_CODE);
        pthread_testcancel();
      }
    }
    for (unsigned counter = 0;
         counter < net_partition->SAMPLING_CHANNELS.size(); counter++) {
      if (main_instance->MAP_OF_SAMPLING_CHANNELS
              [net_partition->SAMPLING_CHANNELS[counter]] != 0) {
        main_instance
            ->MAP_OF_SAMPLING_CHANNELS[net_partition
                                           ->SAMPLING_CHANNELS[counter]]
            ->MOVE_MESSAGES(&RETURN_CODE);
...
int main(int argc, char **argv) {
  ...
  main_instance =
      ACM_INTERNAL::MODULE_MANAGER::Instance(); // create partition vectors
  ...
  for (unsigned int counter = 0;
       counter < main_instance->PARTITION_VECTOR.size(); counter++) {
    if (main_instance->PARTITION_VECTOR[counter].Name.compare(
            net_partition->name) == 0) {
      found = true;
      index = counter;
      break;
    }
  ...
  for (unsigned int i = 0; i < main_instance->PARTITION_VECTOR[index]
                                   .QUEUE_CHANNEL_SOURCE_VECTOR.size();
       i++)
    net_partition->QUEUEING_CHANNELS.push_back(
        main_instance->PARTITION_VECTOR[index].QUEUE_CHANNEL_SOURCE_VECTOR[i]);

  for (unsigned int i = 0; i < main_instance->PARTITION_VECTOR[index]
                                   .SAMPLING_CHANNEL_SOURCE_VECTOR.size();
       i++)
    net_partition->SAMPLING_CHANNELS.push_back(
        main_instance->PARTITION_VECTOR[index]
            .SAMPLING_CHANNEL_SOURCE_VECTOR[i]);

  for (unsigned int i = 0;
       i < main_instance->PARTITION_VECTOR[index]
               .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR.size();
       i++)
    net_partition->QUEUEING_CHANNELS.push_back(
        main_instance->PARTITION_VECTOR[index]
            .QUEUE_CHANNEL_INTER_MODULE_DST_VECTOR[i]);
  ...
  for (unsigned int i = 0;
    ...
    net_partition->SAMPLING_CHANNELS.push_back(
        main_instance->PARTITION_VECTOR[index]
            .SAMPLING_CHANNEL_INTER_MODULE_DST_VECTOR[i]);
  ...
  PROCESS_ID_TYPE NET_PART_PROCESS_ID = APEX_HELPER_CREATE_PROCESS(
